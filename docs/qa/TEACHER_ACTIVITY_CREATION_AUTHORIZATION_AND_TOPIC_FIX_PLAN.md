...
**Official plan document status:** Created and ready at `docs/qa/TEACHER_ACTIVITY_CREATION_AUTHORIZATION_AND_TOPIC_FIX_PLAN.md`. This document is the official implementation plan inside the repository.

---

## Mandatory Build Workflow

This block governs every aspect of implementation. Owner approval via Cursor Build/Implement means: execute the full approved plan from start to finish, under these rules.

**Scope and authority**
1. This plan document is the single source of truth for implementation. No additional chat-body instructions are required or expected.
2. Phases are execution order only — they are not stop-and-wait gates. Implement the full scope, then run QA, fix failures, rerun tests, and produce the closure report.

**SQL rules**
3. Migration files may be prepared (written to disk) only. Cursor must not execute SQL under any circumstances.
4. Owner applies SQL manually after reviewing the migration content.
5. No code that depends on a new column may be deployed until the owner confirms the migration has been applied, or explicitly approves committing with the migration pending.

**Git rules**
6. No commit, no push, and no staging until the owner approves after browser review.
7. Never use `git add .` or `git add -A`. Use explicit `git add <file>` per file, one at a time.

**Scope boundaries**
8. Do not touch any file outside the scope listed in the "Files Expected to Change" section.
9. Do not touch parent/guardian report files (`components/parent*`, `lib/parent-server`, `lib/guardian-server`).
10. Do not touch worksheet or PDF implementation files except for regression checks listed in Phase 9.
11. Do not touch simulation or seeding scripts.
12. Do not change broad visual design or layout outside the specific UI elements described per phase.
13. Do not change any Hebrew text outside the approved labels and error messages already written in this plan.
...
**Rollout order and rules:**
1. Cursor prepares the migration file only. Cursor does not run SQL under any circumstances.
2. Owner manually applies the migration in Supabase after reviewing the SQL content.
3. Code changes (8-B through 8-E) depend on this column existing — deploy only after owner confirms migration is applied.
4. If the migration has not yet been applied at the time of browser QA, the closure report must explicitly state: "Browser QA for SQL-dependent discussion behavior (explanation-only, answer_required=false, student completion without answer) cannot be fully completed until migration is applied."
5. No deployment, commit approval, or push until owner either (a) confirms the migration was applied, or (b) explicitly approves committing with the migration pending.
6. Existing single-question discussions continue to work regardless — `DEFAULT true` ensures no behavioral change for existing rows.
...
- Confirmation: the mandatory workflow rules (see "Mandatory Build Workflow" section) were followed throughout implementation.
- Deviation log: list any deviation from the plan that occurred during implementation, or state "no deviations".

### SQL-dependent browser QA status
State one of:
- "Owner confirmed migration `<NEXT_MIGRATION_NUMBER>_discussion_multi_question.sql` was applied. SQL-dependent browser QA (explanation-only discussion, answer_required=false, student completion without answer) is complete."
- "Migration has not been applied. SQL-dependent browser QA could not be completed. Owner must apply migration and re-verify items 13–16 of the Phase 9 checklist before approval."
