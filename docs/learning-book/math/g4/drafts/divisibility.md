# התחלקות — כללי 2, 3, 5, 6, 9 ו-10

## Metadata

| Field | Value |
|-------|-------|
| **learning_page_id** | `math:g4:divisibility` |
| **skill_id** | `math:kind:divisibility` |
| **subject** | math |
| **grade** | g4 |
| **age_band** | grades_3_4 |
| **page_type** | step_by_step_procedure |
| **approval_status** | launch_ready |
| **title_hebrew** | התחלקות — כללי 2, 3, 5, 6, 9 ו-10 `[DRAFT — not owner-approved]` |

**Source references:**
- `data/curriculum-spine/v1/skills.json`
- `docs/learning-book/MATH_LEARNING_BOOK_CURRICULUM_MAP.md`
- `docs/learning-book/MATH_LEARNING_PAGE_TEMPLATE.md`

**Content scope:** סימני התחלקות 2, 3, 5, 6, 9, 10. כולל 3, 6, 9 (חדש בכיתה ד׳). מספרים עד ~1,000.

---

## 1. מה לומדים?

היום נבדוק **מתי מספר מתחלק** ב-2, 3, 5, 6, 9 או 10 **בלי שארית** — לפי **כללים** מהירים.

---

## 2. הסבר

בכיתה ד׳ מרחיבים ל**ששה** מחלקים — כולל **3, 6, 9** (לפי **סכום הספרות**).

**÷2 / ÷5 / ÷10:** כמו בכיתה ג׳ — לפי **ספרת האחדות**.

**÷3 (או ÷9):** **סכום הספרות** מתחלק ב-3 (או ב-9).

**÷6:** חייב להתחלק **גם** ב-2 **וגם** ב-3.

---

## 3. דוגמה

**450**

ספרת אחדות: **0** → ÷2 ✓  ÷5 ✓  ÷10 ✓  
סכום ספרות: 4+5+0 = 9 → ÷3 ✓  ÷9 ✓  → **÷6 ✓**

450 ÷ 10 = 45

---

## 4. בואו נפתור

**שאלה:** האם **126** מתחלק ב-2? ב-3? ב-6? ב-9?

**שלב 1:** אחדות **6** (זוגי) → ÷2 **כן**.

**שלב 2:** 1+2+6 = **9** → ÷3 **כן**, ÷9 **כן**.

**שלב 3:** ÷2 ו-÷3 → **÷6 כן**.

**תשובה:** ÷2, ÷3, ÷6, ÷9 — **כולם כן**.

---

## 5. נסו בעצמכם

האם **234** מתחלק ב-2, ב-3, ב-5, ב-6, ב-9 וב-10? (כן/לא לכל אחד)

נסו לפתור בעצמכם.
בעמוד הבא נבדוק יחד את הדרך ואת התשובה.

---

## 6. בואו נבדוק יחד

**האם 234 מתחלק ב-2, ב-3, ב-5, ב-6, ב-9 וב-10?**

**שלב 1:** ÷2: ספרת אחדות **4** (זוגי) → **כן**.

**שלב 2:** ÷3: סכום ספרות 2+3+4 = **9** — מתחלק ב-3 → **כן**.

**שלב 3:** ÷5: ספרת אחדות **4** (לא 0 ולא 5) → **לא**.

**שלב 4:** ÷6: מתחלק גם ב-2 וגם ב-3 → **כן**.

**שלב 5:** ÷9: סכום ספרות **9** — מתחלק ב-9 → **כן**.

**שלב 6:** ÷10: ספרת אחדות **4** (לא 0) → **לא**.

**תשובה: ÷2 כן, ÷3 כן, ÷5 לא, ÷6 כן, ÷9 כן, ÷10 לא** ✓

---

## 7. בואו נתרגל!

עכשיו אתם יודעים לבדוק התחלקות ב-2, 3, 5, 6, 9 ו-10.
בתרגול תמצאו "מתחלק?" — השתמשו ב**כללים**!
