# Grade 1 English Learning Book — Hebrew Review Pack

> **Status:** All content in this pack is **draft** (`approval_status: draft`). Nothing here is owner-approved or production-ready.
>
> **Book naming:** Child-facing copy uses **אנגלית** — ספר אנגלית — כיתה א׳.
>
> **Draft coverage:** Grade 1 English has **10 / 10** draft pages authored. Source: `docs/learning-book/english/g1/drafts/`.
>
> **UI / runtime:** Grade 1 English Learning Book **UI is not wired to this content** — no registry, route, or practice CTA resolver.
>
> **Review focus:** Grade 1 exposure level, Hebrew explanations, short English examples on own lines, Section 5 / Section 6 alignment, Bidi/LTR safety.
>
> **Pack version:** Initial full-book review pack (June 2026). Generated from current source markdown.

---

## Summary

| Metric | Value |
|--------|-------|
| **Total pages** | 10 |
| **Batch A** (אוצר מילים — צבעים, מספרים, משפחה) | 3 |
| **Batch B** (אוצר מילים — חיות, רגשות, פעולות, בית ספר) | 4 |
| **Batch C** (תבניות בסיסיות — be, משפטים, כיתה) | 3 |
| **All pages** | `approval_status: draft` |

---

**Book:** ספר אנגלית — כיתה א׳  
**Pages:** 10  
**Sections per page:** 7  
**Source folder:** `docs/learning-book/english/g1/drafts/`

---

## Page 1: vocab_colors.md

| Field | Value |
|-------|-------|
| **File** | `vocab_colors.md` |
| **learning_page_id** | `english:g1:vocab_colors` |
| **skill_id** | `english:vocabulary:wordlist:colors` |
| **title_hebrew** | צבעים באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד מילים באנגלית לצבעים.
צבעים מסביבנו — בגדים, צעצועים וציורים.

---

---

---

### Section 2. הסבר

כל צבע יש לו שם באנגלית.
כששומעים את השם — מחברים אותו לצבע שאנחנו מכירים בעברית.

red — אדום
blue — כחול
green — ירוק
yellow — צהוב

---

---

---

### Section 3. דוגמה

דמיינו תפוח אדום.
באנגלית אומרים:

red

התפוח הוא red.

---

---

---

### Section 4. בואו נפתור

שאלה: מה הצבע של שמיים בהירים ביום יפה?

שלב 1: חושבים על הצבע — כחול.
שלב 2: בוחרים את המילה באנגלית:

blue

תשובה: blue

---

---

---

### Section 5. נסו בעצמכם

מה הצבע של תפוח אדום?

בחרו את המילה הנכונה באנגלית:

red

(רמז: התפוח הוא red.)

---

---

---

### Section 6. שימו לב!

קל לבלבל בין צבעים דומים.

❌ לקרוא לתפוח blue — זה לא נכון!

✓ תפוח אדום — red.

זכרו: red = אדום.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים כמה צבעים באנגלית.
בתרגול תמצאו שאלות על צבעים — red, blue, green ו-yellow.

---

## Page 2: vocab_numbers.md

| Field | Value |
|-------|-------|
| **File** | `vocab_numbers.md` |
| **learning_page_id** | `english:g1:vocab_numbers` |
| **skill_id** | `english:vocabulary:wordlist:numbers` |
| **title_hebrew** | מספרים 0–10 באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד מספרים באנגלית מ-0 עד 10.
מספרים עוזרים לספור חפצים ולדבר על כמות.

---

---

---

### Section 2. הסבר

כל מספר יש לו שם באנגלית.

zero — 0
one — 1
two — 2
three — 3
four — 4
five — 5
six — 6
seven — 7
eight — 8
nine — 9
ten — 10

---

---

---

### Section 3. דוגמה

יש חמש אצבעות ביד אחת.
באנגלית:

five

חמש = five.

---

---

---

### Section 4. בואו נפתור

שאלה: מה המספר 3 באנגלית?

שלב 1: 3 בעברית.
שלב 2: המילה באנגלית:

three

תשובה: three

---

---

---

### Section 5. נסו בעצמכם

כמה אצבעות ביד אחת? חמש.

מה המילה five באנגלית?

five

(רמז: 5 = five.)

---

---

---

### Section 6. שימו לב!

לפעמים מבלבלים בין מספרים קרובים.

❌ לקרוא ל-5 את המילה four — לא נכון!

✓ 5 = five.

five זה חמש.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים מספרים 0–10 באנגלית.
בתרגול תמצאו שאלות על מספרים כמו five, three ו-seven.

---

## Page 3: vocab_family.md

| Field | Value |
|-------|-------|
| **File** | `vocab_family.md` |
| **learning_page_id** | `english:g1:vocab_family` |
| **skill_id** | `english:vocabulary:wordlist:family` |
| **title_hebrew** | משפחה באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד מילים באנגלית לבני משפחה.
כולנו חלק ממשפחה — ויש לה שמות באנגלית.

---

---

---

### Section 2. הסבר

מילים בסיסיות:

mom — אמא
dad — אבא
sister — אחות
brother — אח

---

---

---

### Section 3. דוגמה

אמא שלי בבית.
באנגלית:

mom

אמא = mom.

---

---

---

### Section 4. בואו נפתור

שאלה: מה המילה לאבא באנגלית?

שלב 1: חושבים על אבא.
שלב 2:

dad

תשובה: dad

---

---

---

### Section 5. נסו בעצמכם

מי האישה במשפחה שאנחנו קוראים לה אמא?

המילה באנגלית:

mom

(רמז: אמא = mom.)

---

---

---

### Section 6. שימו לב!

קל לבלבל בין מילים דומות.

❌ לקרוא לאמא dad — זה אבא!

✓ אמא = mom.

זכרו: mom לאבא לא מתאים.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים מילים למשפחה באנגלית.
בתרגול תמצאו שאלות על mom, dad, sister ו-brother.

---

## Page 4: vocab_animals.md

| Field | Value |
|-------|-------|
| **File** | `vocab_animals.md` |
| **learning_page_id** | `english:g1:vocab_animals` |
| **skill_id** | `english:vocabulary:wordlist:animals` |
| **title_hebrew** | חיות באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד שמות של חיות באנגלית.
חיות יש בבית, בגינה ובגן חיות.

---

---

---

### Section 2. הסבר

מילים נפוצות:

cat — חתול
dog — כלב
bird — ציפור
fish — דג

---

---

---

### Section 3. דוגמה

חתול רך על הספה.
באנגלית:

cat

חתול = cat.

---

---

---

### Section 4. בואו נפתור

שאלה: מה המילה לכלב באנגלית?

שלב 1: כלב.
שלב 2:

dog

תשובה: dog

---

---

---

### Section 5. נסו בעצמכם

איזו חיה מיואות ואומרת meow?

המילה באנגלית:

cat

(רמז: חתול = cat.)

---

---

---

### Section 6. שימו לב!

לפעמים מערבבים בין חיות.

❌ לקרוא לחתול dog — זה כלב!

✓ חתול = cat.

cat זה חתול.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים שמות חיות באנגלית.
בתרגול תמצאו שאלות על cat, dog, bird ו-fish.

---

## Page 5: vocab_emotions.md

| Field | Value |
|-------|-------|
| **File** | `vocab_emotions.md` |
| **learning_page_id** | `english:g1:vocab_emotions` |
| **skill_id** | `english:vocabulary:wordlist:emotions` |
| **title_hebrew** | רגשות באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד מילים באנגלית לרגשות.
כולנו מרגישים שמחים או עצובים לפעמים.

---

---

---

### Section 2. הסבר

שני רגשות ראשונים:

happy — שמח
sad — עצוב

---

---

---

### Section 3. דוגמה

ילד מחייך ושמח.
באנגלית:

happy

שמח = happy.

---

---

---

### Section 4. בואו נפתור

שאלה: מה המילה לשמח באנגלית?

שלב 1: שמח.
שלב 2:

happy

תשובה: happy

---

---

---

### Section 5. נסו בעצמכם

ילד מחייך גדול. הוא שמח.

המילה באנגלית:

happy

(רמז: שמח = happy.)

---

---

---

### Section 6. שימו לב!

שמח ועצוב הם הפכים.

❌ לקרוא לילד שמח sad — לא נכון!

✓ שמח = happy.

happy זה שמח.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים רגשות באנגלית.
בתרגול תמצאו שאלות על happy ו-sad.

---

## Page 6: vocab_actions.md

| Field | Value |
|-------|-------|
| **File** | `vocab_actions.md` |
| **learning_page_id** | `english:g1:vocab_actions` |
| **skill_id** | `english:vocabulary:wordlist:actions` |
| **title_hebrew** | פעולות באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד מילים באנגלית לפעולות.
פעולות — מה שאנחנו עושים כל יום.

---

---

---

### Section 2. הסבר

פעלים פשוטים:

run — לרוץ
jump — לקפוץ
read — לקרוא
eat — לאכול

---

---

---

### Section 3. דוגמה

ילד רץ במגרש.
באנגלית:

run

לרוץ = run.

---

---

---

### Section 4. בואו נפתור

שאלה: מה המילה לרוץ באנגלית?

שלב 1: לרוץ.
שלב 2:

run

תשובה: run

---

---

---

### Section 5. נסו בעצמכם

ילד רץ מהר במגרש.

המילה באנגלית:

run

(רמז: לרוץ = run.)

---

---

---

### Section 6. שימו לב!

כל פעולה יש לה מילה משלה.

❌ לקרוא לריצה jump — זה לקפוץ!

✓ לרוץ = run.

run זה לרוץ.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים פעולות באנגלית.
בתרגול תמצאו שאלות על run, jump, read ו-eat.

---

## Page 7: vocab_school.md

| Field | Value |
|-------|-------|
| **File** | `vocab_school.md` |
| **learning_page_id** | `english:g1:vocab_school` |
| **skill_id** | `english:vocabulary:wordlist:school` |
| **title_hebrew** | בית ספר באנגלית `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד מילים באנגלית לדברים בכיתה.
בבית ספר משתמשים בהם כל יום.

---

---

---

### Section 2. הסבר

מילים בכיתה:

book — ספר
pen — עט
desk — שולחן כתיבה
teacher — מורה

---

---

---

### Section 3. דוגמה

ספר על השולחן.
באנגלית:

book

ספר = book.

---

---

---

### Section 4. בואו נפתור

שאלה: מה המילה לספר באנגלית?

שלב 1: ספר.
שלב 2:

book

תשובה: book

---

---

---

### Section 5. נסו בעצמכם

יש ספר על השולחן בכיתה.

המילה באנגלית:

book

(רמז: ספר = book.)

---

---

---

### Section 6. שימו לב!

ספר ועט — שני דברים שונים.

❌ לקרוא לספר pen — זה עט!

✓ ספר = book.

book זה ספר.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים מילים מהכיתה באנגלית.
בתרגול תמצאו שאלות על book, pen, desk ו-teacher.

---

## Page 8: grammar_be.md

| Field | Value |
|-------|-------|
| **File** | `grammar_be.md` |
| **learning_page_id** | `english:g1:grammar_be` |
| **skill_id** | `english:pool:grammar:be_basic` |
| **title_hebrew** | I am / You are — היכרות `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד שתי תבניות קצרות באנגלית.
הן עוזרות להגיד מי אנחנו.

---

---

---

### Section 2. הסבר

שתי תבניות קבועות:

I am — אני …
You are — אתה … / את …

דוגמה:
I am happy — אני שמח.
You are sad — אתה עצוב.

---

---

---

### Section 3. דוגמה

ילד שמח.

I am happy

אני שמח.

---

---

---

### Section 4. בואו נפתור

שאלה: איך אומרים "אני שמח" באנגלית?

שלב 1: מתחילים ב-I am.
שלב 2: מוסיפים happy.

I am happy

תשובה: I am happy

---

---

---

### Section 5. נסו בעצמכם

איך אומרים "אני שמח"?

I am happy

(רמז: I am = אני.)

---

---

---

### Section 6. שימו לב!

חשוב לשמור על הסדר.

❌ happy I am — לא נכון!

✓ I am happy — כן!

תמיד I am ואז המילה.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים את I am ו-You are.
בתרגול תמצאו תבניות קצרות כמו I am happy.

---

## Page 9: sentence_base.md

| Field | Value |
|-------|-------|
| **File** | `sentence_base.md` |
| **learning_page_id** | `english:g1:sentence_base` |
| **skill_id** | `english:pool:sentence:base` |
| **title_hebrew** | משפטים קצרים — בסיס `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד לחבר מילים למשפט קצר באנגלית.
משפט קצר — 2 או 3 מילים.

---

---

---

### Section 2. הסבר

לוקחים מילים שלמדנו — ומחברים.

I am happy — אני שמח.
I am sad — אני עצוב.
You are happy — אתה שמח.

---

---

---

### Section 3. דוגמה

ילד שמח.

I am happy

משפט של 3 מילים.

---

---

---

### Section 4. בואו נפתור

שאלה: בנו משפט "אני עצוב".

שלב 1: I am
שלב 2: sad

I am sad

תשובה: I am sad

---

---

---

### Section 5. נסו בעצמכם

בנו משפט: אני שמח.

I am happy

(רמז: I am ואז happy.)

---

---

---

### Section 6. שימו לב!

משפט צריך סדר נכון.

❌ happy am I — לא משפט!

✓ I am happy — נכון.

I am happy זה "אני שמח".

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם יודעים לבנות משפטים קצרים.
בתרגול תמצאו משפטים כמו I am happy.

---

## Page 10: translation_classroom.md

| Field | Value |
|-------|-------|
| **File** | `translation_classroom.md` |
| **learning_page_id** | `english:g1:translation_classroom` |
| **skill_id** | `english:pool:translation:classroom` |
| **title_hebrew** | ביטויי כיתה `[DRAFT — not owner-approved]` |
| **approval_status** | `draft` |

### Section 1. מה לומדים?

היום נלמד ביטויים קצרים באנגלית לכיתה.
ביטויים שימושיים כשפוגשים חברים ומורים.

---

---

---

### Section 2. הסבר

ביטויים נפוצים:

Hello — שלום
Thank you — תודה
Please — בבקשה

---

---

---

### Section 3. דוגמה

נכנסים לכיתה ואומרים:

Hello

שלום.

---

---

---

### Section 4. בואו נפתור

שאלה: איך אומרים "שלום" באנגלית?

Hello

תשובה: Hello

---

---

---

### Section 5. נסו בעצמכם

פוגשים חבר בבוקר. מה אומרים?

Hello

(רמז: שלום = Hello.)

---

---

---

### Section 6. שימו לב!

Hello ו-Thank you — שני ביטויים שונים.

❌ לומר Thank you כשנכנסים — זה תודה, לא שלום!

✓ שלום = Hello.

Hello בבוקר בכיתה.

---

---

---

### Section 7. בואו נתרגל!

עכשיו אתם מכירים ביטויי כיתה.
בתרגול תמצאו מילים כמו Hello ו-Thank you.

---
