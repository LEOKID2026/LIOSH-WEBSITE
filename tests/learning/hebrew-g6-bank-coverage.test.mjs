...
describe("hebrew G6 bank coverage (Phase 5G)", () => {
  it("G6 literacy pool items are grade-6 MCQs with integrity", () => {
    assert.ok(HEBREW_G6_LITERACY_POOL.length >= 150);
...
describe("hebrew G6 text quality (Phase 5G)", () => {
...
describe("hebrew G3/G4/G5 completed topics remain PROFESSIONAL_READY (Phase 5G guard)", () => {
  ...
  it("G4 all four literacy topics stay at or above 50/40/30 after G6 wiring", () => {
    for (const topic of ["comprehension", "reading", "grammar", "vocabulary"]) {
      for (const [level, min] of [
        ["easy", 50],
        ["medium", 40],
  ...
  it("G5 all four literacy topics stay at or above 50/40/30 after G6 wiring", () => {
    for (const topic of ["comprehension", "reading", "grammar", "vocabulary"]) {
      for (const [level, min] of [
        ["easy", 50],
        ["medium", 40],
