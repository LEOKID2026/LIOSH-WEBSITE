...
function assertMcqIntegrity(rows) {
  for (const row of rows) {
    assert.equal(row.minGrade, 4);
    assert.equal(row.maxGrade, 4);
    ...
    const preview = finalizeHebrewMcq({ ...row }, row.topic, row.levels[0], "g4");
    const aud = auditMcqQuality(preview);
    assert.equal(aud.failures.length, 0, aud.failures.join("; "));
  }
}

describe("hebrew G4 bank coverage (Phase 5C + 5D)", () => {
  it("G4 literacy pool items are grade-4 MCQs with integrity", () => {
    assert.ok(HEBREW_G4_LITERACY_POOL.length >= 400);
    assertMcqIntegrity(HEBREW_G4_LITERACY_POOL);
  });

  it("G4 comprehension reaches 50/40/30 rich-pool thresholds", () => {
    for (const [level, min] of [
      ["easy", 50],
      ["medium", 40],
      ["hard", 30],
    ]) {
      const count = countByTopicLevel("g4", "comprehension", level);
      assert.ok(count >= min, `comprehension ${level}=${count}`);
    }
  });

  it("G4 reading reaches 50/40/30 rich-pool thresholds", () => {
    for (const [level, min] of [
      ["easy", 50],
      ["medium", 40],
      ["hard", 30],
    ]) {
      const count = countByTopicLevel("g4", "reading", level);
      assert.ok(count >= min, `reading ${level}=${count}`);
    }
  });

  it("G4 grammar reaches 50/40/30 rich-pool thresholds", () => {
    for (const [level, min] of [
      ["easy", 50],
      ["medium", 40],
      ["hard", 30],
    ]) {
      const count = countByTopicLevel("g4", "grammar", level);
      assert.ok(count >= min, `grammar ${level}=${count}`);
    }
  });

  it("G4 vocabulary reaches 50/40/30 rich-pool thresholds", () => {
    for (const [level, min] of [
      ["easy", 50],
      ["medium", 40],
      ["hard", 30],
    ]) {
      const count = countByTopicLevel("g4", "vocabulary", level);
      assert.ok(count >= min, `vocabulary ${level}=${count}`);
    }
  });

  it("G4 writing/speaking stay practice-only (not promoted)", () => {
    const writing = getTopicLaunchRow("hebrew", "g4", "writing");
    const speaking = getTopicLaunchRow("hebrew", "g4", "speaking");
    assert.equal(writing?.launchLevel, "PRACTICE_ONLY");
    assert.equal(speaking?.launchLevel, "PRACTICE_ONLY");
  });
});

describe("hebrew G4 text quality (Phase 5C + 5D)", () => {
  it("G4 literacy pool has no text-quality blockers", () => {
    const failures = assertNoTextQualityBlockers(HEBREW_G4_LITERACY_POOL, "g4");
    assert.equal(failures.length, 0, failures.slice(0, 8).join("\n"));
  });

  it("G4 pool has no gender-slash or סימון patterns", () => {
    const critical = [];
    for (const row of HEBREW_G4_LITERACY_POOL) {
...
describe("hebrew G3/G4 completed topics remain PROFESSIONAL_READY (Phase 5D guard)", () => {
  ...
  it("G4 comprehension/reading stay at or above 50/40/30 after grammar/vocabulary wiring", () => {
