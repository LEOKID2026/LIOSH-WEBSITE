/**
 * Regenerate docs/learning-book/GEOMETRY_GRADE_1_HEBREW_REVIEW_PACK.md from G1 geometry drafts.
 * Run: node scripts/build-geometry-g1-hebrew-review-pack.mjs
 */
import fs from "fs";
...
} from "./lib/geometry-g1-draft-manifest.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DRAFTS_DIR = path.join(__dirname, "../docs/learning-book/geometry/g1/drafts");
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/GEOMETRY_GRADE_1_HEBREW_REVIEW_PACK.md"
);
...
function validatePages() {
  const errors = [];
  for (const pageId of GEOMETRY_G1_PAGE_ORDER) {
...
const batchSummary = GEOMETRY_G1_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 1 Geometry Learning Book — Hebrew Review Pack
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${GEOMETRY_G1_PAGE_ORDER.length} |
${batchSummary}
...
**Pages:** ${GEOMETRY_G1_PAGE_ORDER.length}  
**Sections per page:** 7  
...
const pageBlocks = GEOMETRY_G1_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${GEOMETRY_G1_PAGE_ORDER.length} pages).`);
