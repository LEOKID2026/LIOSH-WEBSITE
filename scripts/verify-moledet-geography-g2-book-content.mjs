/**
 * Verify Grade 2 Moledet / Geography learning book draft content (documentation only).
 * Run: node scripts/verify-moledet-geography-g2-book-content.mjs
 */
import fs from "fs";
...
import {
  ...
  MOLEDET_GEOGRAPHY_G2_ALIGNMENT_ANCHORS,
  MOLEDET_GEOGRAPHY_G2_PAGE_META,
...
const FULL_ISRAEL_MAP_RE = /מפת ישראל|מחוזות|גבולות|קואורדינט/i;
...
for (const pageId of MOLEDET_GEOGRAPHY_G2_PAGE_ORDER) {
  ...
  const expectedId = `geography:g2:${pageId}`;
  ...
  const meta = MOLEDET_GEOGRAPHY_G2_PAGE_META[pageId];
  ...
  if (pageId !== "mg_g2_israel_basics" && FULL_ISRAEL_MAP_RE.test(childFacing)) {
    errors.push(`${pageId}: G2 body contains full Israel-map complexity`);
  }
  if (pageId === "mg_g2_israel_basics" && /מפת ישראל/.test(childFacing)) {
    errors.push(`${pageId}: israel_basics must not teach full Israel map`);
  }
  ...
  const anchors = MOLEDET_GEOGRAPHY_G2_ALIGNMENT_ANCHORS[pageId] || [];
...
if (errors.length) {
  console.error(
    "G2 Moledet/Geography content verification FAILED:\n" +
      errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}
...
console.log(
  `G2 Moledet/Geography content verification PASSED: ${MOLEDET_GEOGRAPHY_G2_PAGE_ORDER.length} pages.`
);
console.log("- 7 sections each");
console.log("- draft metadata + geography:g2:{pageId} ids");
console.log("- מולדת וגאוגרפיה wording; no English geography in body");
console.log("- israel_basics: concepts only, no full country map");
