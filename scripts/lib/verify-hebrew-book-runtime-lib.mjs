/**
 * Shared Hebrew grade book runtime verification.
 */
import {
...
const FAKE_PRACTICE_RE =
  /hebrew-master\?|fromBook=|resolveHebrew|\/learning\/hebrew-master\?topic=/i;
 ...
 * }} spec
 */
export function verifyHebrewBookRuntime(spec) {
  const errors = [];
  const label = `hebrew/${spec.grade}`;

  const entry = getLearningBookEntry("hebrew", spec.grade);
  ...
  const tile = getLearningBookTileTitle("hebrew", spec.grade);
  const expectedLine2 = `כיתה ${getGradeShortLabel(spec.grade)}`;
  if (tile.line1 !== "ספר עברית" || tile.line2 !== expectedLine2) {
  ...
  for (const pageId of spec.pageOrder) {
    ...
    if (page.metadata?.subject !== "hebrew") {
      errors.push(`${label}/${pageId}: metadata subject must be hebrew`);
    }
    if (page.metadata?.approval_status !== "approved") {
      errors.push(`${label}/${pageId}: approval_status must be approved`);
    }
    const expectedLearningPageId = `hebrew:${spec.grade}:${pageId}`;
    if (
...
export function assertHebrewMasterPath() {
  if (getLearningBookMasterPath("hebrew") !== "/learning/hebrew-master") {
    return ["hebrew master path must be /learning/hebrew-master"];
  }
  return [];
}
 ...
 */
export function checkHebrewLearningPageIdCollisions(specs) {
  const errors = [];
  /** @type {Map<string, string>} */
  const seen = new Map();
  for (const spec of specs) {
    for (const pageId of spec.pageOrder) {
      const id = `hebrew:${spec.grade}:${pageId}`;
      if (seen.has(id)) {
