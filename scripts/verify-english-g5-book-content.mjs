/**
 * Verify Grade 5 English learning book draft content (documentation only).
 * Run: node scripts/verify-english-g5-book-content.mjs
 */
import fs from "fs";
...
import {
  ...
  ENGLISH_G5_ALIGNMENT_ANCHORS,
  ENGLISH_G5_PAGE_META,
...
const INTERNAL_KEY_RE = /english:(pool|grammar|vocabulary|g5)|spine_layer|skill_id/i;
...
for (const pageId of ENGLISH_G5_PAGE_ORDER) {
  ...
  const expectedId = `english:g5:${pageId}`;
  ...
  const meta = ENGLISH_G5_PAGE_META[pageId];
  ...
  const anchors = ENGLISH_G5_ALIGNMENT_ANCHORS[pageId] || [];
...
if (errors.length) {
  console.error(
    "G5 English content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}

console.log(`G5 English content verification PASSED: ${ENGLISH_G5_PAGE_ORDER.length} pages.`);
console.log("- 7 sections each");
console.log("- draft metadata + english:g5:{pageId} ids");
