/**
 * Verify Grade 5 Math learning book draft content (documentation only).
 * Run: node scripts/verify-math-g5-book-content.mjs
 */
import fs from "fs";
...
for (const pageId of MATH_G5_PAGE_ORDER) {
  ...
  const expectedId = `math:g5:${pageId}`;
...
if (errors.length) {
  console.error("G5 content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n"));
  process.exit(1);
}

console.log(`G5 content verification PASSED: ${MATH_G5_PAGE_ORDER.length} pages.`);
console.log("- 7 sections each");
console.log("- draft metadata + math:g5:{pageId} ids");
