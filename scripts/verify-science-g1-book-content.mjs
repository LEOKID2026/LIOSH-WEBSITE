/**
 * Verify Grade 1 Science learning book draft content (documentation only).
 * Run: node scripts/verify-science-g1-book-content.mjs
 */
import fs from "fs";
...
import {
  ...
  SCIENCE_G1_ALIGNMENT_ANCHORS,
  SCIENCE_G1_PAGE_META,
} from "./lib/science-g1-draft-manifest.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DRAFTS_DIR = path.join(__dirname, "../docs/learning-book/science/g1/drafts");
...
for (const pageId of SCIENCE_G1_PAGE_ORDER) {
  ...
  const expectedId = `science:g1:${pageId}`;
  ...
  const meta = SCIENCE_G1_PAGE_META[pageId];
...
if (errors.length) {
  console.error(
    "G1 Science content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}
