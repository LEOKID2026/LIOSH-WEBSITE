/**
 * Regenerate docs/learning-book/GEOMETRY_GRADE_2_HEBREW_REVIEW_PACK.md from G2 geometry drafts.
 * Run: node scripts/build-geometry-g2-hebrew-review-pack.mjs
 */
import fs from "fs";
...
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/GEOMETRY_GRADE_2_HEBREW_REVIEW_PACK.md"
);
...
const batchSummary = GEOMETRY_G2_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 2 Geometry Learning Book — Hebrew Review Pack
...
>
> **Draft coverage:** Grade 2 Geometry has **${GEOMETRY_G2_PAGE_ORDER.length} / ${GEOMETRY_G2_PAGE_ORDER.length}** draft pages authored. Source: \`docs/learning-book/geometry/g2/drafts/\`.
>
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${GEOMETRY_G2_PAGE_ORDER.length} |
${batchSummary}
...
**Pages:** ${GEOMETRY_G2_PAGE_ORDER.length}  
**Sections per page:** 7  
...
const pageBlocks = GEOMETRY_G2_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${GEOMETRY_G2_PAGE_ORDER.length} pages).`);
