/**
 * Verify Grade 5 Hebrew learning book draft content (documentation only).
 * Run: node scripts/verify-hebrew-g5-book-content.mjs
 */
import fs from "fs";
...
const FAKE_PRACTICE_RE =
  /forceKind|fromBook=|hebrew-master\?|resolveHebrew|getHebrewG5Practice|practice_mapping/i;
...
const g5Spine = allSkills.filter(
  (s) => s.subject === "hebrew" && s.minGrade <= 5 && s.maxGrade >= 5
);

const errors = [];
const markdownNotes = [];

if (g5Spine.length !== EXPECTED_G5_COUNT) {
  errors.push(
    `Spine G5 Hebrew count ${g5Spine.length} !== expected ${EXPECTED_G5_COUNT}`
  );
}

if (HEBREW_G5_PAGE_ORDER.length !== EXPECTED_G5_COUNT) {
  errors.push(
    `Manifest page count ${HEBREW_G5_PAGE_ORDER.length} !== expected ${EXPECTED_G5_COUNT}`
  );
}

const spineIds = new Set(g5Spine.map((s) => s.skill_id));
const manifestSkillIds = HEBREW_G5_PAGE_ORDER.map((p) => HEBREW_G5_PAGE_META[p].skillId);
for (const id of manifestSkillIds) {
  if (!spineIds.has(id)) {
    errors.push(`Manifest skill_id not in G5 spine: ${id}`);
  }
}
for (const row of g5Spine) {
  if (!manifestSkillIds.includes(row.skill_id)) {
    errors.push(`Spine G5 skill missing from manifest: ${row.skill_id}`);
  }
}
...
for (const pageId of HEBREW_G5_PAGE_ORDER) {
  ...
  const anchors = HEBREW_G5_ALIGNMENT_ANCHORS[pageId] || [];
...
if (extra.length !== EXPECTED_G5_COUNT) {
  errors.push(`Draft folder has ${extra.length} .md files (expected ${EXPECTED_G5_COUNT} + README)`);
}

if (errors.length) {
  console.error(
    "G5 Hebrew content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}

console.log(`G5 Hebrew content verification PASSED: ${HEBREW_G5_PAGE_ORDER.length} pages.`);
console.log(`- Reconciled with skills.json: ${g5Spine.length} G5 Hebrew spine rows`);
console.log("- 7 sections each; age_band grades_5_6");
