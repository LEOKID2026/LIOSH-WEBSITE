/**
 * Verify Grade 1 Geometry learning book draft content (documentation only).
 * Run: node scripts/verify-geometry-g1-book-content.mjs
 */
import fs from "fs";
...
const FAKE_PRACTICE_RE =
  /forceKind|fromBook=|geometry-master\?|resolveGeometryG1|getGeometryG1Practice/i;
...
for (const pageId of GEOMETRY_G1_PAGE_ORDER) {
  ...
  const expectedId = `geometry:g1:${pageId}`;
  ...
  const anchors = GEOMETRY_G1_ALIGNMENT_ANCHORS[pageId] || [];
...
if (errors.length) {
  console.error(
    "G1 Geometry content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}

console.log(`G1 Geometry content verification PASSED: ${GEOMETRY_G1_PAGE_ORDER.length} pages.`);
console.log("- 7 sections each");
console.log("- draft metadata + geometry:g1:{pageId} ids");
console.log("- גאומטריה naming; no גאומטריה / English geometry in body");
