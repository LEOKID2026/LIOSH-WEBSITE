/**
 * Verify Grade 2 Hebrew / עברית learning book draft content (documentation only).
 * Run: node scripts/verify-hebrew-g2-book-content.mjs
 */
import fs from "fs";
...
import {
  ...
  HEBREW_G2_ALIGNMENT_ANCHORS,
  HEBREW_G2_PAGE_META,
} from "./lib/hebrew-g2-draft-manifest.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.join(__dirname, "..");
const DRAFTS_DIR = path.join(ROOT, "docs/learning-book/hebrew/g2/drafts");
...
const INTERNAL_KEY_RE =
  /hebrew:(g\d|rich)|skill_id|word_level_early|spell_word_early|binary_fact_early|gender_number_early|word_context_early|social_reply_early|patternFamily|minGrade|maxGrade/i;
...
function spineG2HebrewSkills() {
  const spineRaw = JSON.parse(fs.readFileSync(SPINE_PATH, "utf8"));
  const allSkills = spineRaw.skills ?? spineRaw;
  return allSkills.filter((s) => s.subject === "hebrew" && s.minGrade <= 2 && s.maxGrade >= 2);
}

const errors = [];
const markdownNotes = [];

const spineSkills = spineG2HebrewSkills();
const spineSkillIds = new Set(spineSkills.map((s) => s.skill_id));
const manifestSkillIds = new Set(
  HEBREW_G2_PAGE_ORDER.map((id) => HEBREW_G2_PAGE_META[id]?.skillId).filter(Boolean)
);

if (HEBREW_G2_PAGE_ORDER.length !== spineSkills.length) {
  errors.push(
    `Page count ${HEBREW_G2_PAGE_ORDER.length} != spine G2 Hebrew count ${spineSkills.length}`
  );
}
...
for (const skillId of manifestSkillIds) {
  if (!spineSkillIds.has(skillId)) {
    errors.push(`Manifest skill not in G2 spine: ${skillId}`);
  }
}
...
for (const pageId of HEBREW_G2_PAGE_ORDER) {
  ...
  const expectedId = `hebrew:g2:${pageId}`;
  ...
  const anchors = HEBREW_G2_ALIGNMENT_ANCHORS[pageId] || [];
...
if (errors.length) {
  console.error(
    "G2 Hebrew content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}

console.log(`G2 Hebrew content verification PASSED: ${HEBREW_G2_PAGE_ORDER.length} pages.`);
console.log("- 7 sections each");
console.log("- draft metadata + hebrew:g2:{pageId} ids");
console.log("- spine G2 skill count matches manifest");
