/**
 * Verify Grade 3 Moledet / Geography learning book draft content (documentation only).
 * Run: node scripts/verify-moledet-geography-g3-book-content.mjs
 */
import fs from "fs";
...
import {
  ...
  MOLEDET_GEOGRAPHY_G3_ALIGNMENT_ANCHORS,
  MOLEDET_GEOGRAPHY_G3_PAGE_META,
...
for (const pageId of MOLEDET_GEOGRAPHY_G3_PAGE_ORDER) {
  ...
  const expectedId = `geography:g3:${pageId}`;
  ...
  const meta = MOLEDET_GEOGRAPHY_G3_PAGE_META[pageId];
  ...
  if (COORDINATE_RE.test(childFacing)) {
    errors.push(`${pageId}: G3 body contains coordinate-grid content (G5 scope)`);
  }
  ...
  const anchors = MOLEDET_GEOGRAPHY_G3_ALIGNMENT_ANCHORS[pageId] || [];
...
if (errors.length) {
  console.error(
    "G3 Moledet/Geography content verification FAILED:\n" +
      errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}
...
console.log(
  `G3 Moledet/Geography content verification PASSED: ${MOLEDET_GEOGRAPHY_G3_PAGE_ORDER.length} pages.`
);
console.log("- 7 sections each; grades_3_4 age band");
console.log("- geography:g3:{pageId} ids");
console.log("- no coordinate grid; Section 5/6 anchors present");
