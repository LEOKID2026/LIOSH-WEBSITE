/**
 * Regenerate docs/learning-book/ENGLISH_GRADE_2_HEBREW_REVIEW_PACK.md from G2 English drafts.
 * Run: node scripts/build-english-g2-hebrew-review-pack.mjs
 */
import fs from "fs";
...
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/ENGLISH_GRADE_2_HEBREW_REVIEW_PACK.md"
);
...
const batchSummary = ENGLISH_G2_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 2 English Learning Book — Hebrew Review Pack

> **Status:** All content in this pack is **draft** (\`approval_status: draft\`). Nothing here is owner-approved or production-ready.
>
> **Book naming:** Child-facing copy uses **אנגלית** — ספר אנגלית — כיתה ב׳.
>
> **Draft coverage:** Grade 2 English has **${ENGLISH_G2_PAGE_ORDER.length} / ${ENGLISH_G2_PAGE_ORDER.length}** draft pages authored. Source: \`docs/learning-book/english/g2/drafts/\`.
>
> **UI / runtime:** Grade 2 English Learning Book **UI is not wired to this content** — no registry, route, or practice CTA resolver.
>
> **Review focus:** Grade 2 foundation level, continuing vocab depth, Hebrew explanations, Section 5 / Section 6 alignment, Bidi/LTR safety.
>
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${ENGLISH_G2_PAGE_ORDER.length} |
${batchSummary}
| **All pages** | \`approval_status: draft\` |

---

**Book:** ספר אנגלית — כיתה ב׳  
**Pages:** ${ENGLISH_G2_PAGE_ORDER.length}  
**Sections per page:** 7  
**Source folder:** \`docs/learning-book/english/g2/drafts/\`
...
const pageBlocks = ENGLISH_G2_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${ENGLISH_G2_PAGE_ORDER.length} pages).`);
