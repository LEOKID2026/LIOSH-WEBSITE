/**
 * Verify all Grade 6 Math book pages parse with sections 1–7 and clean titles.
 * Run: node scripts/verify-math-g6-book.mjs
 */
import fs from "fs";
...
const DRAFT_MARKER = /\[DRAFT|not owner-approved|math:g6:|approval_status|`/i;

try {
  const registryFlat = MATH_G6_BOOK_BATCHES.flatMap((b) => b.pages);
  if (registryFlat.length !== 44) {
    throw new Error(`Expected 44 pages in registry, got ${registryFlat.length}`);
  }
  if (registryFlat.join(",") !== MATH_G6_PAGE_ORDER.join(",")) {
    throw new Error("MATH_G6_PAGE_ORDER does not match batch flat order");
  }
  const unique = new Set(MATH_G6_PAGE_ORDER);
  if (unique.size !== MATH_G6_PAGE_ORDER.length) {
    throw new Error("Duplicate page IDs in MATH_G6_PAGE_ORDER");
  }

  const pages = MATH_G6_PAGE_ORDER.map((pageId) => {
  ...
  console.log("Registry order:", MATH_G6_PAGE_ORDER.join(", "));
