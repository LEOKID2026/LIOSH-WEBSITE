/**
 * Regenerate docs/learning-book/SCIENCE_GRADE_5_HEBREW_REVIEW_PACK.md from G5 science drafts.
 * Run: node scripts/build-science-g5-hebrew-review-pack.mjs
 */
import fs from "fs";
...
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/SCIENCE_GRADE_5_HEBREW_REVIEW_PACK.md"
);

function validatePages() {
  const errors = [];
  for (const pageId of SCIENCE_G5_PAGE_ORDER) {
  ...
  if (fs.existsSync(path.join(DRAFTS_DIR, "plants.md"))) {
    errors.push("plants.md must not exist in G5 (spine maxGrade 3)");
  }
  if (errors.length) {
...
const batchSummary = SCIENCE_G5_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 5 Science Learning Book — Hebrew Review Pack
...
>
> **Draft coverage:** Grade 5 Science has **${SCIENCE_G5_PAGE_ORDER.length} / ${SCIENCE_G5_PAGE_ORDER.length}** draft pages. **No plants page** (spine maxGrade 3).
>
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${SCIENCE_G5_PAGE_ORDER.length} |
${batchSummary}
...
const pageBlocks = SCIENCE_G5_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${SCIENCE_G5_PAGE_ORDER.length} pages).`);
