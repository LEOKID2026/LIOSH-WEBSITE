/**
 * Regenerate docs/learning-book/ENGLISH_GRADE_6_HEBREW_REVIEW_PACK.md from G6 English drafts.
 * Run: node scripts/build-english-g6-hebrew-review-pack.mjs
 */
import fs from "fs";
...
} from "./lib/english-g6-draft-manifest.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DRAFTS_DIR = path.join(__dirname, "../docs/learning-book/english/g6/drafts");
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/ENGLISH_GRADE_6_HEBREW_REVIEW_PACK.md"
);
...
function validatePages() {
  const errors = [];
  for (const pageId of ENGLISH_G6_PAGE_ORDER) {
...
const batchSummary = ENGLISH_G6_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 6 English Learning Book — Hebrew Review Pack
...
>
> **Draft coverage:** Grade 6 English has **${ENGLISH_G6_PAGE_ORDER.length} / ${ENGLISH_G6_PAGE_ORDER.length}** draft pages authored. Source: \`docs/learning-book/english/g6/drafts/\`.
>
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${ENGLISH_G6_PAGE_ORDER.length} |
${batchSummary}
...
**Pages:** ${ENGLISH_G6_PAGE_ORDER.length}  
**Sections per page:** 7  
...
const pageBlocks = ENGLISH_G6_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${ENGLISH_G6_PAGE_ORDER.length} pages).`);
