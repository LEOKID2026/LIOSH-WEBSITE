/**
 * Verify Grade 1 Moledet / Geography learning book draft content (documentation only).
 * Run: node scripts/verify-moledet-geography-g1-book-content.mjs
 */
import fs from "fs";
...
import {
  ...
  MOLEDET_GEOGRAPHY_G1_ALIGNMENT_ANCHORS,
  MOLEDET_GEOGRAPHY_G1_PAGE_META,
...
const NATIONAL_MAP_RE = /מפת ישראל|מחוזות|גבולות|קואורדינט/i;
...
for (const pageId of MOLEDET_GEOGRAPHY_G1_PAGE_ORDER) {
  ...
  const expectedId = `geography:g1:${pageId}`;
  ...
  const meta = MOLEDET_GEOGRAPHY_G1_PAGE_META[pageId];
  ...
  if (NATIONAL_MAP_RE.test(childFacing)) {
    errors.push(`${pageId}: G1 body contains national-map complexity`);
  }
  ...
  const anchors = MOLEDET_GEOGRAPHY_G1_ALIGNMENT_ANCHORS[pageId] || [];
...
if (errors.length) {
  console.error(
    "G1 Moledet/Geography content verification FAILED:\n" +
      errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}
...
console.log(
  `G1 Moledet/Geography content verification PASSED: ${MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.length} pages.`
);
console.log("- 7 sections each");
console.log("- draft metadata + geography:g1:{pageId} ids");
console.log("- מולדת וגאוגרפיה wording; no English geography in body");
console.log("- no national-map complexity in G1");
