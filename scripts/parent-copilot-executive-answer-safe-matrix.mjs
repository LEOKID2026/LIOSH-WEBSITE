...
function expectedPatternForQuestion(q) {
  if (q === "מה המקצוע החזק?")
    return /המקצוע החזק ביותר|מקצוע אחד עם תרגול|לא .*מספיק תרגול.*שני מקצועות|לא נדרגים כאן מקצועות|אין מספיק נתונים להשוואה בין מקצועות|תורגל רק|התחומים החזקים יחסית/;
  if (q === "מה המקצוע החלש?")
  ...
  if (q === "באיזה מקצוע הכי קשה?")
    return /המקצוע שבו הכי «קשה»|מקצוע אחד עם תרגול|מקצוע אחד עם נתונים|«הכי קשה» מתייחס|לא .*מספיק תרגול.*שני מקצועות|לא נדרגים כאן מקצועות|אין מספיק נתונים להשוואה בין מקצועות|תורגל רק|דורש חיזוק/;
  if (q === "יש עוד מקצועות?")
  ...
  if (q === "מה הכי בולט בתקופה?")
    return /לפי שורות הסיכום|בולטים במיוחד|אין כרגע בדוח מספיק תרגול|הכי גבוהים|ממוצע|דירוג הדיוק|התקדמות|מגמה|בולט|majorTrends|סיכום התקופה/;
  if (q === "חשבון מול אנגלית")
    return /גבוה יותר|על אותו קו דיוק|חסרים מספיק נתוני תרגול|צריך לציין בבירור שני מקצועות|מעט נתוני תרגול|אין עדיין מספיק מידע|מה שעובד יחסית טוב|ההשוואה מבוססת/;
  ...
  if (q === "מה השתפר?") return /סימני שיפור|אין שורת מגמה מפורשת|מה השתפר/;
