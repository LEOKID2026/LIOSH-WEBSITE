/**
 * Verify all Grade 2 Math book pages parse with sections 1–7 and clean titles.
 * Run: node scripts/verify-math-g2-book.mjs
 */
import fs from "fs";
...
const DRAFT_MARKER = /\[DRAFT|not owner-approved|math:g2:|approval_status|`/i;

try {
  const registryFlat = MATH_G2_BOOK_BATCHES.flatMap((b) => b.pages);
  if (registryFlat.length !== 22) {
    throw new Error(`Expected 22 pages in registry, got ${registryFlat.length}`);
  }
  if (registryFlat.join(",") !== MATH_G2_PAGE_ORDER.join(",")) {
    throw new Error("MATH_G2_PAGE_ORDER does not match batch flat order");
  }
  const unique = new Set(MATH_G2_PAGE_ORDER);
  if (unique.size !== MATH_G2_PAGE_ORDER.length) {
    throw new Error("Duplicate page IDs in MATH_G2_PAGE_ORDER");
  }

  const pages = MATH_G2_PAGE_ORDER.map((pageId) => {
    ...
    if (raw.includes("כיתה ב'")) {
      throw new Error(`${pageId}.md contains forbidden כיתה ב'`);
    }
  ...
  console.log("Registry order:", MATH_G2_PAGE_ORDER.join(", "));
