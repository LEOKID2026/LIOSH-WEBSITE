/**
 * Verify Grade 4 Hebrew / עברית learning book draft content (documentation only).
 * Run: node scripts/verify-hebrew-g4-book-content.mjs
 */
import fs from "fs";
...
function spineG4HebrewSkills() {
  const spineRaw = JSON.parse(fs.readFileSync(SPINE_PATH, "utf8"));
  const allSkills = spineRaw.skills ?? spineRaw;
  return allSkills.filter((s) => s.subject === "hebrew" && s.minGrade <= 4 && s.maxGrade >= 4);
}

const errors = [];
const markdownNotes = [];

const spineSkills = spineG4HebrewSkills();
const spineSkillIds = new Set(spineSkills.map((s) => s.skill_id));
const manifestSkillIds = new Set(
  HEBREW_G4_PAGE_ORDER.map((id) => HEBREW_G4_PAGE_META[id]?.skillId).filter(Boolean)
);

if (HEBREW_G4_PAGE_ORDER.length !== spineSkills.length) {
  errors.push(
    `Page count ${HEBREW_G4_PAGE_ORDER.length} != spine G4 Hebrew count ${spineSkills.length}`
  );
}
...
for (const skillId of manifestSkillIds) {
  if (!spineSkillIds.has(skillId)) {
    errors.push(`Manifest skill not in G4 spine: ${skillId}`);
  }
}
...
for (const pageId of HEBREW_G4_PAGE_ORDER) {
  ...
  if (HEBREW_G4_RICH_BAND_PAGE_IDS.has(pageId) && !raw.includes("G4 depth")) {
    errors.push(`${pageId}: rich band page missing G4 depth metadata note`);
  }

  const learningPageId = readMetadataField(raw, "learning_page_id").replace(/^`|`$/g, "");
  const expectedId = `hebrew:g4:${pageId}`;
...
if (errors.length) {
  console.error(
    "G4 Hebrew content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}

console.log(`G4 Hebrew content verification PASSED: ${HEBREW_G4_PAGE_ORDER.length} pages.`);
console.log("- 7 sections each");
console.log("- draft metadata + hebrew:g4:{pageId} ids");
console.log("- spine G4 skill count matches manifest");
console.log(`- ${HEBREW_G4_RICH_BAND_PAGE_IDS.size} rich 3–4 band pages with G4 depth metadata`);
