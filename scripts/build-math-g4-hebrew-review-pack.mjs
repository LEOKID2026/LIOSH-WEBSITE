/**
 * Regenerate docs/learning-book/MATH_GRADE_4_HEBREW_REVIEW_PACK.md from G4 drafts.
 * Run: node scripts/build-math-g4-hebrew-review-pack.mjs
 */
import fs from "fs";
...
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/MATH_GRADE_4_HEBREW_REVIEW_PACK.md"
);
...
const batchSummary = MATH_G4_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 4 Math Learning Book — Hebrew Review Pack
...
>
> **Draft coverage:** Grade 4 has **${MATH_G4_PAGE_ORDER.length} / ${MATH_G4_PAGE_ORDER.length}** draft pages authored (Batches A–G). Source: \`docs/learning-book/math/g4/drafts/\`.
>
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${MATH_G4_PAGE_ORDER.length} |
${batchSummary}
...
**Pages:** ${MATH_G4_PAGE_ORDER.length}  
**Sections per page:** 7  
...
const pageBlocks = MATH_G4_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${MATH_G4_PAGE_ORDER.length} pages).`);
console.log(
  `Validation: all ${MATH_G4_PAGE_ORDER.length} files exist, 7 sections each, draft status, DRAFT titles, grades_3_4, no מתמטיקה in bodies.`
);
