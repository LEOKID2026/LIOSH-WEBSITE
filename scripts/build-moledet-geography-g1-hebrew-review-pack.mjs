/**
 * Regenerate docs/learning-book/MOLEDET_GEOGRAPHY_GRADE_1_HEBREW_REVIEW_PACK.md from G1 drafts.
 * Run: node scripts/build-moledet-geography-g1-hebrew-review-pack.mjs
 */
import fs from "fs";
...
import {
  MOLEDET_GEOGRAPHY_G1_PAGE_ORDER,
  MOLEDET_GEOGRAPHY_G1_BOOK_BATCHES,
} from "./lib/moledet-geography-g1-draft-manifest.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DRAFTS_DIR = path.join(__dirname, "../docs/learning-book/moledet-geography/g1/drafts");
const OUT_PATH = path.join(
  __dirname,
  "../docs/learning-book/MOLEDET_GEOGRAPHY_GRADE_1_HEBREW_REVIEW_PACK.md"
);
...
function validatePages() {
  const errors = [];
  for (const pageId of MOLEDET_GEOGRAPHY_G1_PAGE_ORDER) {
    ...
    if (readMetadataField(raw, "grade") !== "g1") {
      errors.push(`${pageId}.md: grade must be g1`);
    }
...
const batchSummary = MOLEDET_GEOGRAPHY_G1_BOOK_BATCHES.map(
  (b) => `| **Batch ${b.id.toUpperCase()}** (${b.titleHe}) | ${b.pages.length} |`
).join("\n");

const header = `# Grade 1 Moledet / Geography Learning Book — Hebrew Review Pack
...
>
> **Draft coverage:** Grade 1 Moledet/Geography has **${MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.length} / ${MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.length}** draft pages authored. Source: \`docs/learning-book/moledet-geography/g1/drafts/\`.
>
...
| Metric | Value |
|--------|-------|
| **Total pages** | ${MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.length} |
${batchSummary}
...
**Pages:** ${MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.length}  
**Sections per page:** 7  
...
const pageBlocks = MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.map((pageId, index) => {
...
console.log(`Wrote ${OUT_PATH} (${MOLEDET_GEOGRAPHY_G1_PAGE_ORDER.length} pages).`);
