#!/usr/bin/env node
/**
 * Launch Readiness — Coverage Matrix MVP (E2).
 *
 * Reads reports/virtual-student-daily/<date>/run-summary.json only.
 * Writes reports/launch-readiness/<date>/coverage-summary.json + .md
 *
 * Usage:
 *   npm run qa:launch:coverage -- --date 2026-05-23
 *   node scripts/launch-readiness/build-coverage-matrix.mjs 2026-05-23
 *
 * Exit 0 when report is written (even if overallStatus is not_run/warn/fail).
 */
...
async function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/launch-readiness/build-coverage-matrix.mjs --date YYYY-MM-DD"
    );
    process.exit(0);
  }
  ...
  const relSource = path
    .relative(repoRoot, nightlyPath)
    .split(path.sep)
    .join("/");

  console.log(`launch-readiness/coverage: date=${args.date} source=${relSource}`);

  const nightlyRead = await readRunSummary(nightlyPath);
  ...
  const mdPath = path.join(outDir, "coverage-summary.md");

  await writeFile(jsonPath, JSON.stringify(report, null, 2), "utf8");
  await writeFile(mdPath, buildCoverageMarkdown(report), "utf8");

  console.log("");
  console.log("=========== Launch Readiness Coverage Matrix ===========");
  ...
  console.log(`subjects      : ${Object.keys(report.subjects).length}`);
  console.log(`grades w/data : ${Object.values(report.grades).filter((g) => g.studentsCount > 0).length}`);
...
main().catch((err) => {
  console.error("launch-readiness/coverage FAILED:", err?.stack || err);
  process.exit(1);
});
