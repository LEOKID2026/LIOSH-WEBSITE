/**
 * Verify Grade 3 Hebrew learning book draft content (documentation only).
 * Run: node scripts/verify-hebrew-g3-book-content.mjs
 */
import fs from "fs";
...
const FAKE_PRACTICE_RE =
  /forceKind|fromBook=|hebrew-master\?|resolveHebrew|getHebrewG3Practice|practice_mapping/i;
const RAW_MARKDOWN_RE = /\*\*[^*]+\*\*/;
const HIGH_RISK_RE = /```|^\s*\|.*\|.*\|/m;
const INTERNAL_KEY_RE =
  /hebrew:g|hebrew:rich|skill_id|learning_page_id|approval_status|:rich:|patternFamily/i;
...
const g3Spine = allSkills.filter(
  (s) => s.subject === "hebrew" && s.minGrade <= 3 && s.maxGrade >= 3
);

const errors = [];
const markdownNotes = [];

if (g3Spine.length !== EXPECTED_G3_COUNT) {
  errors.push(
    `Spine G3 Hebrew count ${g3Spine.length} !== expected ${EXPECTED_G3_COUNT}`
  );
}

if (HEBREW_G3_PAGE_ORDER.length !== EXPECTED_G3_COUNT) {
  errors.push(
    `Manifest page count ${HEBREW_G3_PAGE_ORDER.length} !== expected ${EXPECTED_G3_COUNT}`
  );
}

const spineIds = new Set(g3Spine.map((s) => s.skill_id));
const manifestSkillIds = HEBREW_G3_PAGE_ORDER.map((p) => HEBREW_G3_PAGE_META[p].skillId);
for (const id of manifestSkillIds) {
  if (!spineIds.has(id)) {
    errors.push(`Manifest skill_id not in G3 spine: ${id}`);
  }
}
for (const row of g3Spine) {
  if (!manifestSkillIds.includes(row.skill_id)) {
    errors.push(`Spine G3 skill missing from manifest: ${row.skill_id}`);
  }
}
...
for (const pageId of HEBREW_G3_PAGE_ORDER) {
  ...
  const anchors = HEBREW_G3_ALIGNMENT_ANCHORS[pageId] || [];
...
if (extra.length !== EXPECTED_G3_COUNT) {
  errors.push(`Draft folder has ${extra.length} .md files (expected ${EXPECTED_G3_COUNT} + README)`);
}

if (errors.length) {
  console.error(
    "G3 Hebrew content verification FAILED:\n" + errors.map((e) => `  - ${e}`).join("\n")
  );
  process.exit(1);
}

console.log(`G3 Hebrew content verification PASSED: ${HEBREW_G3_PAGE_ORDER.length} pages.`);
console.log(`- Reconciled with skills.json: ${g3Spine.length} G3 Hebrew spine rows`);
console.log("- 7 sections each; age_band grades_3_4");
