#!/usr/bin/env node
/**
 * Science Content Sync — embed approved export txt into website markdown source.
 * Source: exports/audio-text/books/science/science-gX/pages/page-NNN.txt
 * Target: docs/learning-book/science/gX/drafts/{topic}.md sections 1–7
 */
...
/** @returns {Map<string, Map<number, string>>} mdRelPath -> sectionNumber -> txt body */
function collectSectionUpdates() {
  /** @type {Map<string, Map<number, string>>} */
  const byFile = new Map();
  /** @type {Array<{ txt: string, md: string, section: number, grade: string, pageNum: number, topicId: string }>} */
  const mapping = [];

  for (const grade of GRADES) {
    const entry = getLearningBookEntry("science", grade);
    if (!entry) throw new Error(`Missing science catalog entry: ${grade}`);
    ...
    for (const page of entry.loader.loadAllPages()) {
      for (const section of page.sections) {
        ...
        const txtPath = path.join(TXT_ROOT, txtRel.replace(`science-${grade}/`, `science-${grade}/`));
        const fullTxtPath = path.join(TXT_ROOT, `science-${grade}`, "pages", `page-${padPageNum(pageNumber)}.txt`);
        ...
        mapping.push({
          grade,
          pageNum: pageNumber,
          txt: `science-${grade}/pages/page-${padPageNum(pageNumber)}.txt`,
          md: mdRel,
    ...
    if (pageNumber !== EXPECTED_PAGES[grade]) {
      throw new Error(`science-${grade}: expected ${EXPECTED_PAGES[grade]} pages, got ${pageNumber}`);
    }
  }
...
fs.writeFileSync(
  path.join(ROOT, "docs/learning-book/science/science-content-sync-report.json"),
