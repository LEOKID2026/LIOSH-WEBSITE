...
/**
 * @param {{ value: number, index: number, onRemove?: () => void, dragging?: boolean }} props
 */
function MoneyChip({ value, index, onRemove, dragging = false }) {
...
export default function LeoSupermarketGame({ backHref = "/dev/learning-game-prototypes" }) {
  ...
  const registerHint =
    step === "product" ? "גררו או לחצו על המוצר הנכון" : "מוצרים נבחרו — חשבו את העודף";

  const changeHint = step === "change" ? "גררו או לחצו מטבעות לעודף" : "קודם בחרו מוצר";
        ...
        <div className={styles.playArea}>
          ...
          <div className={styles.checkoutRow}>
            ...
            >
              ...
              <p className={styles.zoneHint}>{registerHint}</p>
              <div className={styles.zoneItems}>
                {selectedProductIds.length === 0 ? (
                  ...
                  selectedProductIds.map((id) => {
                    ...
                    if (!p) return null;
                    return (
                      <div key={id} className={styles.miniProduct}>
                        <GroceryItemVisual product={p} variant="register-mini" showPrice={false} />
                      </div>
                    );
                  })
                )}
              ...
              <div className={styles.checkoutFooter}>
                <div className={styles.priceRow}>
                  <span className={styles.checkoutAmount}>
                    מחיר: {step === "change" ? formatShekel(customer.total) : "—"}
                  </span>
                  <span className={styles.checkoutAmountPaid}>
                    שילם: {step === "change" ? formatShekel(customer.paid) : "—"}
                  </span>
                </div>
              </div>
            </div>
