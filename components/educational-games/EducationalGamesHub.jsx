...
import { listEducationalHubCardMetadata } from "../../lib/educational-games/educational-games-metadata.client.js";

export default function EducationalGamesHub() {
  const { theme } = useStudentTheme();
  const { GH } = useGamesHubUi();
  const { state, playableGames, enabledGames, isGuest } = useStudentGameAccess();
  const metadataCards = listEducationalHubCardMetadata();

  const accessGames =
    state === "ready"
      ? isGuest
        ? enabledGames("educational")
        : playableGames("educational")
      : [];

  const games =
    accessGames.length > 0
      ? accessGames
      : metadataCards.map((row) => ({
          gameKey: row.gameKey,
          titleHe: row.titleHe,
          blurbHe: row.blurbHe,
          emoji: row.emoji,
  ...
  return (
    <Layout studentTheme={theme} studentShell="home">
      <GameAccessGuard category="educational" shellFirst>
        <main className={GH.pageWrap} dir="rtl">
          <div className={`${GH.container} space-y-4`}>
            ...
            <section className="grid sm:grid-cols-2 gap-3 md:gap-4">
              {state === "loading" && accessGames.length === 0
                ? null
                : games.length === 0
                  ? (
                  ...
                  : games.map((row) => {
                      const locked = isGuest && state === "ready" && !row.playable;
                      const cardBody = (
                        <>
