...
for (const item of replacements) {
  ...
  const target = path.join(BOOKS_ROOT, book, "pages", page);
  const rel = path.relative(ROOT, target).replaceAll("\\", "/");
  if (!rel.startsWith(`exports/audio-text/books/${book}/pages/`)) {
    throw new Error(`Blocked path outside expected scope: ${rel}`);
  }
  if (!fs.existsSync(target)) throw new Error(`Missing page file: ${rel}`);
