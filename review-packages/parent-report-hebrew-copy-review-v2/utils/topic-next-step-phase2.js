...
/**
 * ריכוך ניסוח הורה כשהמנוע מרסן מסקנה ברורה.
...
export function mergePhase7SoftHebrewCopy(copy, restraint, rootCause) {
  ...
  if (cs === "withheld" || level === "insufficient") {
    addParent.push("עדיין מוקדם לקבוע מסקנה ברורה — עדיף תרגול קצר ושגרתי באותה רמה.");
...
export function buildPhase7RecommendationFields(p) {
  ...
  const whatWouldIncreaseConfidenceHe =
    alt.length > 0
      ? alt.join(" ")
      : "עוד שאלות בתקופה שנבחרה, כיוון דיוק ברור יותר, ופחות תלות ברמזים — יגבירו את הביטחון במסקנה.";
...
export function buildWhatCouldChangeThisHe(p) {
  ...
  const parts = [];
  parts.push(`איסוף יותר מ־${Math.max(12, Number(q) || 0)} שאלות בתקופה שנבחרה,`);
