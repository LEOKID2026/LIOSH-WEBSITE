...
function buildInterpretationSlot(envelope, cannotConcludeYet, seed, q = 0, acc = 0) {
  if (cannotConcludeYet || envelope === "WE0") {
    return pickVariant(seed, [
      "עדיין מוקדם לקבוע כאן מסקנה ברורה — נמשיך לעקוב אחרי התרגול.",
...
function buildUncertaintySlot(hedgeLevel, seed) {
  if (hedgeLevel === "mandatory") {
    return pickVariant(seed, [
      ...
      "עדיין מוקדם לקבוע מסקנה — כדאי לאסוף עוד נתוני תרגול ואז לבדוק שוב.",
      "בשלב הזה עדיין מוקדם לקבוע מסקנה סופית; נמשיך לאסוף עוד נתוני תרגול לפני שמסיקים מסקנה.",
    ]);
  }
  if (hedgeLevel === "light") {
    return pickVariant(seed, [
