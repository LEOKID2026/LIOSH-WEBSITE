...
function buildMajorTrendsHe(subjects, subjectCoverage) {
  ...
  if (!out.length) {
    out.push("עדיין אין שתי מגמות חוצות־מקצועות ברורות — התמונה תתבהר אחרי עוד תרגול בתקופה.");
  }
  if (out.length === 1) {
...
function pickMainHomeRecommendationHe(subjects, subjectCoverage, summary, topFocusAreasHe, topStrengthsAcrossHe) {
  ...
  return "שני מפגשים קצרים בשבוע, דגש על קריאת המשימה לפני תשובה — עד שיתקבל עוד תרגול בתקופה.";
}
...
function buildCautionNoteHe(crossRisks, subjects, dominantRiskId) {
  ...
  if (dominantRiskId === "none_sparse" || dominantRiskId === "none_observed") {
    return "עדיין מעט נתון — לא לקבוע שינוי דרמטי בבית לפני שיתקבל עוד תרגול.";
  }
...
function buildHomeFocusHe(subjects, topStrengthsAcrossHe, topFocusAreasHe, summary) {
  ...
  if (q < 18) {
    return "עדיין מעט חומר בתקופה — שני מפגשים קצרים בשבוע יעזרו לחדד את התמונה בפעם הבאה.";
  }
  if (acc >= 78 && q >= 35) {
...
function buildCrossSubjectInsights(baseReport, subjects) {
  ...
  if (sparse.length) {
    bulletsHe.push(
      `ב${sparse.map((s) => s.subjectLabelHe).join(", ")} עדיין יש מעט נתון — התמונה תתבהר אחרי עוד תרגול.`
    );
  }
...
function buildNextPeriodGoals(subjects) {
  ...
  if (!itemsHe.length) {
    itemsHe.push("כשיתקבל עוד תרגול בתקופה, יתווסף כאן כיוון קונקרטי — עד אז עדיף לא לעמיס יעדים מלאכותיים.");
  }
  return { itemsHe };
}
