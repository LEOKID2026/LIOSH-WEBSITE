...
import {
  deleteDraftStudentActivity,
  getTeacherStudentActivityDetail,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      const detail = await getTeacherStudentActivityDetail(
        ctx.serviceRole,
        ctx.teacherId,
        activityId
      );
    ...
    if (req.method === "DELETE") {
      ...
      const deleted = await deleteDraftStudentActivity(
        ctx.serviceRole,
        ctx.teacherId,
        activityId
      );
    ...
    safeApiLog("teacher/student-activities/[activityId]", err);
