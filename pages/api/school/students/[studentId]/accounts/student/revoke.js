...
import { revokeSchoolStudentAccess } from "../../../../../../../lib/school-server/school-account-management.server.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await revokeSchoolStudentAccess({
    ...
    await maybeWriteOperatorCredentialAudit(ctx, {
      studentId: String(studentId),
      actionType: "credential_revoke_student",
    });

    return res.status(200).json({ data: result.data });
  } catch (_e) {
    safeApiLog("school_student_revoke_error", {});
