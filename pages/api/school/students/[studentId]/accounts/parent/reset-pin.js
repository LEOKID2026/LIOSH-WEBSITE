...
import { rotateSchoolParentPin } from "../../../../../../../lib/school-server/school-account-management.server.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await rotateSchoolParentPin({
    ...
    await maybeWriteOperatorCredentialAudit(ctx, {
      studentId: String(studentId),
      actionType: "credential_reset_parent_pin",
    });

    return res.status(200).json({ data: result.data });
  } catch (_e) {
    safeApiLog("school_parent_reset_pin_error", {});
