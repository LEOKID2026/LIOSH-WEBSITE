...
import { setSchoolParentBlocked } from "../../../../../../../lib/school-server/school-account-management.server.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await setSchoolParentBlocked(
      {
    ...
    await maybeWriteOperatorCredentialAudit(ctx, {
      studentId: String(studentId),
      actionType: "credential_unblock_parent",
    });

    return res.status(200).json({ data: result.data });
  } catch (_e) {
    safeApiLog("school_parent_unblock_error", {});
