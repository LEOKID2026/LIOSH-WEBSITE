...
import { unlinkSchoolParentFromStudent } from "../../../../../../../lib/school-server/school-account-management.server.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await unlinkSchoolParentFromStudent({
    ...
    await maybeWriteOperatorCredentialAudit(ctx, {
      studentId: String(studentId),
      actionType: "parent_unlink",
    });

    return res.status(200).json({ data: result.data });
  } catch (_e) {
    safeApiLog("school_parent_unlink_error", {});
