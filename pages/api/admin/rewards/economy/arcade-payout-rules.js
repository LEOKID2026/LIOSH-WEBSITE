...
import {
  listArcadePayoutRulesAdmin,
  updateArcadePayoutRuleAdmin,
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const rows = await listArcadePayoutRulesAdmin(ctx.serviceRole);
  ...
  if (req.method === "PUT") {
    ...
    const result = await updateArcadePayoutRuleAdmin(ctx.serviceRole, ctx.adminUserId, id, patch || {});
