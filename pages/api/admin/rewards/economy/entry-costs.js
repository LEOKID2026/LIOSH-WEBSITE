...
import {
  listEntryCostOptionsAdmin,
  updateEntryCostOptionAdmin,
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const rows = await listEntryCostOptionsAdmin(ctx.serviceRole);
  ...
  if (req.method === "PUT") {
    ...
    const result = await updateEntryCostOptionAdmin(ctx.serviceRole, ctx.adminUserId, id, patch || {});
