...
import {
  listMonthlyTiersAdmin,
  updateMonthlyTierAdmin,
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const rows = await listMonthlyTiersAdmin(ctx.serviceRole);
  ...
  if (req.method === "PUT") {
    ...
    const result = await updateMonthlyTierAdmin(ctx.serviceRole, ctx.adminUserId, id, patch || {});
