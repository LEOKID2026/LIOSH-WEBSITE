...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await ctx.serviceRole
      .from("teacher_limits")
      .update({
        feature_flags: parsed.featureFlags,
    ...
    await writeAdminAuditRow(ctx.serviceRole, {
      ...
      action: "set_feature",
      beforeState: { feature_flags: beforeRow.limits.feature_flags },
      afterState: { feature_flags: data.feature_flags },
    });
    ...
    safeApiLog("admin_teacher_features_error", { route: "admin/teachers/[teacherId]/features" });
