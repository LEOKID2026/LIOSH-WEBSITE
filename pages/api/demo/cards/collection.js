...
import { getDemoCardsCollectionView } from "../../../../lib/rewards/server/reward-cards.server.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const view = await getDemoCardsCollectionView(supabase, gradeLevel);
    return res.status(200).json({ ok: true, ...view });
  } catch (_err) {
    return res.status(500).json({ ok: false, error: "demo_collection_load_failed" });
  }
}
