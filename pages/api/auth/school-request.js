...
import {
  parseSchoolRegistrationBody,
  submitSchoolRegistrationRequest,
...
export default async function handler(req, res) {
  ...
  const parsed = parseSchoolRegistrationBody(req.body);
  if (!parsed.ok) {
  ...
  try {
    ...
    const result = await submitSchoolRegistrationRequest(serviceRole, parsed.payload);
    ...
    return res.status(201).json({ data: { submitted: true, schoolId: result.schoolId } });
  } catch (_e) {
    safeApiLog("school_registration_request_error", {});
