---
name: Teacher Portal Master Plan
...
todos:
  ...
  - id: phase4-teacher-login
    ...
    status: pending
  - id: phase5-teacher-dashboard
    content: "Phase 5: Implement /teacher/dashboard with link-only student management honoring configurable limit (default 20)."
    status: pending
  - id: phase6-teacher-student-report
    content: "Phase 6: Implement /teacher/student/[studentId] reusing the existing diagnostic engine via a new teacher-side ownership gate."
    status: pending
  - id: phase7-class-report
    content: "Phase 7: Implement /teacher/class/[classId] with lib/teacher-server/class-aggregate.server.js cohort rollups."
    status: pending
  - id: phase8-guardian-access
    content: "Phase 8: Implement teacher-issued parent/guardian access tables, APIs, magic-link bootstrap, revoke/audit, and pages/parent/guardian-view/[studentId].js (no Hebrew text in this phase)."
    status: pending
  - id: phase9-qa-security
    content: "Phase 9: Cross-tenant QA, RLS tests, regression sweep on parent/student flows, sign-off doc docs/security/TEACHER_PORTAL_SECURITY_PLAN.md."
    status: pending
  - id: deferred-q2-schema-decision
    content: "Deferred: complete impact audit of all students.parent_id consumers, then choose between virtual-parent (short-term), nullable-parent_id (preferred long-term), or teacher_only_students (last resort)."
    status: pending
isProject: false
---
...
- **Phase 9 — QA / security gates.** Cross-tenant tests (teacher A ↔ teacher B, teacher ↔ parent, guardian-session ↔ other students), full regression sweep on parent + student dashboards, RLS unit tests, audit completeness checks, IDOR matrix. Consult [`docs/security/PARENT_STUDENT_OWNERSHIP_BOUNDARY_PLAN.md`](docs/security/PARENT_STUDENT_OWNERSHIP_BOUNDARY_PLAN.md) and [`docs/security/SUPABASE_RLS_SERVICE_ROLE_AUDIT_PLAN.md`](docs/security/SUPABASE_RLS_SERVICE_ROLE_AUDIT_PLAN.md). Sign-off written to a new `docs/security/TEACHER_PORTAL_SECURITY_PLAN.md`. **No commit, no push.** Stop and wait for owner approval.
...
## File-creation summary (this phase only — Phase 0 deliverable)

- Create folder: `docs/teacher-portal/`. **Created/updated locally only — no commit, no push.**
- Create file: `docs/teacher-portal/TEACHER_PORTAL_MASTER_PLAN.md` — the content above, in English, technical identifiers preserved. **Created/updated locally only — no commit, no push.** The owner commits manually.
