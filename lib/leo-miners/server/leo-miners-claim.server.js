...
async function claimMinersCoins(supabase, studentId, parsed, state, config, soloRules, opts = {}) {
  const pendingPoints = Number(state.mining_points_pending || 0);
  if (pendingPoints <= 0) {
    return minersErrorResult(
  ...
  if (!coinResult.duplicate) {
    await decrementMinersPendingRewards(supabase, studentId, conversion.pointsUsed, 0);
  }
...
async function claimMinersDiamondsChest(
  ...
  const pendingDiamonds = Math.floor(Number(state.diamonds_pending || 0));
  ...
  if (!diamondResult.duplicate) {
    await decrementMinersPendingRewards(supabase, studentId, 0, conversion.diamondsUsed);
  }
