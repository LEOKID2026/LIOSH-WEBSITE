...
function calculateLeoSupermarketCoins(diff, metrics, rules) {
  ...
  if (metrics?.completedAllCustomers === true && completeBonus > 0) {
    coins += completeBonus;
    breakdown.push(`30 לקוחות: +${completeBonus}`);
  }
...
function calculateLeoContinuousCoins(diff, metrics, rules, progressLabel) {
  const progress = clampInt(metrics?.successfulQuestions, 0, 200);
  const zero = zeroProgressCoinResult(progress);
  if (zero) {
    return { ...zero, displayLevelHe: difficultyLabelHe(diff) };
  }
  ...
  if (progress >= 25 && milestoneBonus > 0) {
    coins += milestoneBonus;
    breakdown.push(`אבן דרך 25: +${milestoneBonus}`);
  }
...
function calculateLeoNumberPathCoins(diff, metrics, rules) {
  const progress = clampInt(metrics?.successfulTasks, 0, 12);
  const zero = zeroProgressCoinResult(progress);
  if (zero) {
    return { ...zero, displayLevelHe: difficultyLabelHe(diff) };
  }
  ...
  const maxCoins = clampInt(rules?.maxCoins, 1, 1000000);
  const accuracy = clampFloat(metrics?.accuracy, 0, 1);
  const bestStreak = clampInt(metrics?.bestStreak, 0, 12);
  ...
  if (metrics?.completedAllTasks === true && completeBonus > 0) {
    coins += completeBonus;
    breakdown.push(`12 משימות: +${completeBonus}`);
  }
...
function validateLeoSupermarketMetrics(metrics, diff, score) {
  ...
  for (const [val, max, label] of [
    [customersTotal, 30, "סה״כ לקוחות"],
    [customersReached, 30, "לקוחות שהגיעו"],
    [customersCompleted, 30, "לקוחות שטופלו"],
    [correctCustomers, 30, "לקוחות נכונים"],
    [wrongProducts, 30, "טעויות מוצר"],
    [wrongChange, 30, "טעויות עודף"],
    [timeoutMistakes, 30, "פסילות זמן"],
    [mistakes, 30, "פסילות"],
    [bestStreak, 30, "רצף"],
    [durationSec, 3600, "משך"],
  ]) {
...
function buildLeoSupermarketFinishPresentation(metrics) {
  const correct = clampInt(metrics?.correctCustomers, 0, 30);
  return {
    subtitleHe: `כל הכבוד! שירתת ${correct} לקוחות ועזרת לליאו במכולת`,
    statsLines: [
      { label: "לקוחות שהגעת אליהם", value: String(clampInt(metrics?.customersReached, 0, 30)) },
      { label: "לקוחות שטופלו", value: String(clampInt(metrics?.customersCompleted, 0, 30)) },
      { label: "לקוחות נכונים", value: String(correct) },
      { label: "דיוק", value: `${Math.round(clampFloat(metrics?.accuracy, 0, 1) * 100)}%` },
      { label: "פסילות", value: String(clampInt(metrics?.mistakes, 0, 30)) },
      { label: "רצף הכי טוב", value: String(clampInt(metrics?.bestStreak, 0, 30)) },
    ],
  };
}
...
function buildLeoNumberPathFinishPresentation(metrics) {
  const successful = clampInt(metrics?.successfulTasks, 0, 12);
  return {
    subtitleHe: `כל הכבוד! השלמתם ${successful} משימות במסלול המספרים`,
    statsLines: [
      { label: "משימות שהגעתם אליהן", value: String(clampInt(metrics?.tasksReached, 0, 12)) },
      { label: "משימות שהצליחו", value: String(successful) },
      { label: "טעויות", value: String(clampInt(metrics?.mistakes, 0, 36)) },
      { label: "דיוק", value: `${Math.round(clampFloat(metrics?.accuracy, 0, 1) * 100)}%` },
      { label: "רצף הכי טוב", value: String(clampInt(metrics?.bestStreak, 0, 12)) },
    ],
  };
}
...
function validateLeoNumberPathMetrics(metrics, diff, score) {
  ...
  for (const [val, max, label] of [
    [tasksTotal, 12, "סה״כ משימות"],
    [tasksReached, 12, "משימות שהגיעו"],
    [successfulTasks, 12, "משימות שהצליחו"],
    [failedAttempts, 36, "ניסיונות כושלים"],
    [mistakes, 36, "טעויות"],
    [bestStreak, 12, "רצף"],
    [durationSec, 3600, "משך"],
  ]) {
