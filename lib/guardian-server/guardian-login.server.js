 ...
 */
async function verifyGuardianCredentials(serviceRole, usernameNormalized, pinNormalized) {
  const { data: accessRow, error } = await serviceRole
    .from("student_guardian_access")
    .select("id, student_id, pin_hash, is_active, revoked_at, expires_at, created_by_teacher_id")
  ...
  const accessState = computeGuardianAccessState(accessRow);
  if (accessState === "expired") {
    return { ok: false, status: 403, code: "access_expired" };
  }
  if (accessState === "revoked") {
    return { ok: false, status: 403, code: "access_revoked" };
  }

  const pinHash = hashStudentSecret(pinNormalized);
  if (pinHash !== accessRow.pin_hash) {
    return { ok: false, status: 401, code: "invalid_credentials" };
  }
...
export async function guardianLogin(input) {
  ...
  if (input.inviteToken) {
    ...
    const cred = await verifyGuardianCredentials(serviceRole, username, pin);
    if (!cred.ok) {
