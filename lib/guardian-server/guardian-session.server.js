...
function portalFlagsRelaxedForDev() {
  return process.env.NODE_ENV !== "production";
}

function envFlagOrDevDefault(name, productionDefault) {
  const raw = process.env[name];
  if (raw != null && String(raw).trim() !== "") {
    return String(raw).trim().toLowerCase() === "true";
  }
  if (portalFlagsRelaxedForDev()) return true;
  return productionDefault;
}

export function isGuardianPortalEnabled() {
  return envFlagOrDevDefault("GUARDIAN_PORTAL_ENABLED", false);
}

export function isGuardianPortalUiCopyEnabled() {
  return envFlagOrDevDefault("GUARDIAN_PORTAL_UI_COPY_ENABLED", false);
}
...
export async function resolveAuthenticatedGuardian(req) {
  ...
  const { data: accessRow, error: accessErr } = await serviceRole
    ...
    .select(
      "id, student_id, created_by_teacher_id, login_username, is_active, revoked_at, expires_at, created_at"
    )
