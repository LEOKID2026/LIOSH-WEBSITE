...
export async function computeStudentLearningDerived(supabase, studentId) {
  ...
  // Independent DB reads — same queries/limits as before; parallel to cut wall time.
  const [
    answersResult,
    sessionsResult,
    monthlyAggregate,
    parentAttemptsResult,
    parentVisitMinutesBySubject,
  ] = await Promise.all([
    supabase
      .from("answers")
    ...
    sumStudentLearningCreditedMinutesInIsraelMonth(supabase, studentId, startIso, endIso, {
      applyEconomyMonthlyCap: false,
    }),
    supabase
      .from("parent_activity_attempts")
      .select(
        "is_correct, answered_at, time_spent_ms, question_snapshot, parent_assigned_activities!inner(subject)"
      )
      .eq("student_id", studentId)
      .limit(MAX_ANSWER_ROWS_AGG),
    sumParentActivityVisitMsBySubjectInRange(supabase, studentId, startIso, endIso),
  ]);

  const { data: answers, error: aErr } = answersResult;
  if (aErr) throw aErr;

  const { data: sessions, error: sErr } = sessionsResult;
  if (sErr) throw sErr;

  const { data: parentAttempts, error: parentAttemptsErr } = parentAttemptsResult;
  ...
  const monthlyMinutesIsraelMonth = monthlyAggregate.minutes;
