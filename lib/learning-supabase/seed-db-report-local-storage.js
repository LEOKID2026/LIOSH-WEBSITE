...
import {
  estimatePracticeDurationSeconds,
...
export function seedLocalStorageFromDbReportInput(store, dbInput) {
  ...
  for (const sid of REPORT_AGG_SUBJECTS) {
    ...
    for (const [topicKey, rawTopic] of Object.entries(topics)) {
      ...
      const durationSec = sanitizeReportDurationSeconds(
        durationSecRaw > 0 ? durationSecRaw : estimatePracticeDurationSeconds(total),
        { answerCount: total }
      ).seconds;
      ...
      if (!Number.isFinite(activityMs) && total > 0 && Number.isFinite(rangeEndMs)) {
        activityMs = rangeEndMs;
      }
      const session = {
