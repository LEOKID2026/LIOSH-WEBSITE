...
/** Max single session duration (1 hour), aligned with educational/solo game caps. */
export const MAX_LEARNING_SESSION_SECONDS = 60 * 60;

/** Minimum session duration credited for persistence/rewards (5 seconds). */
...
/**
 * Cap a stored duration value for aggregation (monthly persistence).
 * @param {number} durationSeconds
 */
export function capStoredSessionDurationSeconds(durationSeconds) {
  const ds = Math.floor(Number(durationSeconds) || 0);
  if (ds <= 0) return 0;
