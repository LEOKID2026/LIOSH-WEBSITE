...
/**
 * Add whole calendar days to an Israel civil date string (YYYY-MM-DD).
 * Uses UTC noon arithmetic so day length / DST does not shift the civil date.
 * @param {string} israelDateStr
 * @param {number} [days=1]
 * @returns {string}
 */
export function addIsraelCalendarDays(israelDateStr, days = 1) {
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(israelDateStr || "").trim());
  if (!m) throw new Error("invalid_israel_date");
  const y = Number(m[1]);
  const mo = Number(m[2]);
  const d = Number(m[3]);
  const utcNoon = Date.UTC(y, mo - 1, d + Number(days || 0), 12, 0, 0);
...
/**
 * Inclusive Israel calendar date range → UTC ISO bounds for DB filters.
 * start = midnight Asia/Jerusalem on fromYmd (inclusive)
 * endExclusive = midnight Asia/Jerusalem on the day after toYmd (exclusive)
 * @param {string} fromYmd
 * @param {string} toYmd
 * @returns {{ fromIso: string, toIsoExclusive: string, fromYmd: string, toYmd: string, endExclusiveYmd: string, timeZone: string }}
 */
export function getIsraelInclusiveDateRangeUtcBounds(fromYmd, toYmd) {
  const from = String(fromYmd || "").trim();
  const to = String(toYmd || "").trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to)) {
    throw new Error("invalid_israel_date_range");
  }
  const endExclusiveYmd = addIsraelCalendarDays(to, 1);
  return {
    fromIso: getIsraelMidnightUtc(from).toISOString(),
    toIsoExclusive: getIsraelMidnightUtc(endExclusiveYmd).toISOString(),
    fromYmd: from,
    toYmd: to,
    endExclusiveYmd,
    timeZone: ISRAEL_TZ,
