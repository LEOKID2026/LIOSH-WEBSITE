...
import { getIsraelDateString } from "./israel-calendar.server";
import { isRewardEconomySettingsEnabled } from "../rewards/reward-feature-flags.js";
import { getDailyMissionsForGradeBand } from "../rewards/server/reward-economy.server.js";
import { LEGACY_MISSION_POOL, LEGACY_MISSION_REWARD_COINS } from "../rewards/legacy-economy.js";

// Re-export for consumers that import from this module
export { getIsraelDateString } from "./israel-calendar.server";

// ── Constants ──────────────────────────────────────────────────────────────
const MISSION_REWARD_COINS   = LEGACY_MISSION_REWARD_COINS;
const MISSION_COIN_REASON     = "mission_complete";
const MISSION_COIN_SOURCE_TYPE = "mission_complete";

// ── Static mission pool (legacy fallback) ────────────────────────────────────
const MISSION_POOL = LEGACY_MISSION_POOL;
...
function buildFreshDailyState(gradeBand, date, pool) {
  const missions = pool ?? MISSION_POOL[gradeBand] ?? MISSION_POOL.g34;
  return {
    date,
    missions: missions.map((m) => ({
      ...
      completed:   false,
      coinAwarded: false,
      rewardCoins: m.rewardCoins ?? MISSION_REWARD_COINS,
    })),
    subjectsSeen: [],
  };
}
...
/**
 * Resolve mission pool for grade band (legacy sync or economy async).
 * @param {import("@supabase/supabase-js").SupabaseClient} [supabase]
 */
export async function resolveMissionPool(supabase, gradeBand) {
  if (!isRewardEconomySettingsEnabled()) {
    return MISSION_POOL[gradeBand] ?? MISSION_POOL.g34;
  }
...
export function ensureTodayMissions(challenges, gradeLevel, today, poolOverride) {
  ...
  const pool = poolOverride ?? MISSION_POOL[band] ?? MISSION_POOL.g34;
  return {
...
async function awardMissionCoins(supabase, studentId, date, missionIds, missions) {
  ...
  for (const missionId of missionIds) {
    ...
    const amount = Math.floor(Number(mission?.rewardCoins ?? MISSION_REWARD_COINS));
