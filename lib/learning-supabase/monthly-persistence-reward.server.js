...
import { LEGACY_MONTHLY_PERSISTENCE_TIERS } from "../rewards/legacy-economy.js";

export const MONTHLY_PERSISTENCE_TIERS = LEGACY_MONTHLY_PERSISTENCE_TIERS.map((t) => ({ ...t }));
...
/**
 * Resolve tiers (legacy or admin DB).
 * @param {import("@supabase/supabase-js").SupabaseClient} [supabase]
 */
export async function resolveMonthlyTiers(supabase) {
  if (!isRewardEconomySettingsEnabled()) {
    return MONTHLY_PERSISTENCE_TIERS;
  }
  return getMonthlyPersistenceTiersFromSettings(supabase);
}

/**
 * Highest tier reached (non-cumulative). Returns null below minimum threshold.
 * @param {number} activeMinutes
 * @param {{ minutes: number, coins: number }[]} [tiers]
 */
export function resolveMonthlyPersistenceTier(activeMinutes, tiers = MONTHLY_PERSISTENCE_TIERS) {
