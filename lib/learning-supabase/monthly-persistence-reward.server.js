 ...
 *
 * Source of truth: learning_sessions (status=completed) + parent activity credited minutes.
...
import { capStoredSessionDurationSeconds } from "./learning-session-finish.server.js";
...
export async function sumCompletedSessionMinutesForIsraelMonth(
  ...
  for (const row of data || []) {
    totalSeconds += capStoredSessionDurationSeconds(row.duration_seconds);
  }

  let sessionMinutes = Math.round((totalSeconds / 60) * 100) / 100;
  ...
  let activeMinutes = Math.round((sessionMinutes + (parentMinutes.minutes || 0)) * 100) / 100;

  try {
    const economy = await requireEconomyConfig(supabase);
    const monthlyCap = Math.floor(Number(economy?.globalCaps?.monthlyMinutesCap) || 0);
    if (monthlyCap > 0 && activeMinutes > monthlyCap) {
      activeMinutes = monthlyCap;
    }
  } catch {
