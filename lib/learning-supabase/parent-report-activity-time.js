...
import { getIsraelInclusiveDateRangeUtcBounds } from "./israel-calendar.server.js";
...
/**
 * Inclusive parent-report period bounds in ms (Asia/Jerusalem civil days → UTC).
 * Must match resolveParentReportRangeUtcBounds / DB fetches in report-data-aggregate.
 * @param {Date} fromDate
 * @param {Date} toDate
 */
export function reportRangeBoundsMs(fromDate, toDate) {
  ...
  return {
    fromMs: Date.parse(bounds.fromIso),
    toMsExclusive: Date.parse(bounds.toIsoExclusive),
  };
}
