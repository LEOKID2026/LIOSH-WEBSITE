...
function sanitizeDiagnosticMistakeRow(item) {
  ...
  const out = {
    ...
    // Diagnostic evidence rows use selectedAnswer/correctAnswer; keep both aliases.
    expectedAnswer: safeString(item.expectedAnswer ?? item.correctAnswer, 300),
    userAnswer: safeString(item.userAnswer ?? item.selectedAnswer, 300),
    selectedAnswer: safeString(item.selectedAnswer ?? item.userAnswer, 300),
    correctAnswer: safeString(item.correctAnswer ?? item.expectedAnswer, 300),
    ...
    misconceptionTag: safeString(item.misconceptionTag, 120),
    classifierRuleId: safeString(item.classifierRuleId, 160),
    evidenceReason: safeString(item.evidenceReason, 200),
  ...
  if (item.answerEvidence && typeof item.answerEvidence === "object" && !Array.isArray(item.answerEvidence)) {
    out.answerEvidence = item.answerEvidence;
  }
