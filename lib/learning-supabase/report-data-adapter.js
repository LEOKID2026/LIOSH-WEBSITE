  ...
  for (const subject of SUBJECTS) {
    ...
    for (const topicKey of Object.keys(topicsIn)) {
      ...
      for (const [gradeBucket, gradeInRaw] of gradeEntries) {
        ...
        const topicDurRaw = Math.max(0, Math.floor(safeNumber(topicIn.durationSeconds)));
        const durationSeconds = sanitizeReportDurationSeconds(
          gradeDurRaw > 0 ? gradeDurRaw : topicDurRaw,
          { answerCount: topicTotal }
        ).seconds;
        const gradeTimeMsSum = Math.max(0, Math.floor(safeNumber(gradeIn.timeMsSum)));
        const topicTimeMsSum = Math.max(0, Math.floor(safeNumber(topicIn.timeMsSum)));
        const timeMsSum = gradeTimeMsSum > 0 ? gradeTimeMsSum : topicTimeMsSum;
        const gradeTimeMsCount = Math.max(0, Math.floor(safeNumber(gradeIn.timeMsCount)));
        const topicTimeMsCount = Math.max(0, Math.floor(safeNumber(topicIn.timeMsCount)));
        const timeMsCount = gradeTimeMsCount > 0 ? gradeTimeMsCount : topicTimeMsCount;
