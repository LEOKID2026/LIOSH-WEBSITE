  ...
  for (const subject of SUBJECTS) {
    ...
    for (const topicKey of Object.keys(topicsIn)) {
      ...
      for (const [gradeBucket, gradeInRaw] of gradeEntries) {
        ...
        const gradeDur = Math.max(0, Math.floor(safeNumber(gradeIn.durationSeconds)));
        const topicDur = Math.max(0, Math.floor(safeNumber(topicIn.durationSeconds)));
        topics[adapterTopicKey] = {
          ...
          durationSeconds: gradeDur > 0 ? gradeDur : topicDur,
