...
import { evaluateMonthlyPersistenceReward } from "./monthly-persistence-reward.server.js";
...
/**
 * Fast path — learning state row + today's missions + economy config (cached globally).
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 * @param {{ studentId: string, student?: { full_name?: string, grade_level?: string | null } }} auth
 * @param {{ timer?: ReturnType<typeof createStudentHomeProfileTimer> }} [ctx]
 */
...
/**
 * Heavy path — derived aggregates, account snapshot, monthly persistence, missions init,
 * and awaited achievement/card grants (reliable serverless — not fire-and-forget).
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 * @param {{
 *   studentId: string,
 *   displayName: string,
 *   gradeLevel?: string,
 *   normalized?: ReturnType<typeof normalizeLearningProfileRow>,
 *   row?: Awaited<ReturnType<typeof ensureStudentLearningStateRow>>,
 * }} params
 * @param {{ timer?: ReturnType<typeof createStudentHomeProfileTimer> }} [ctx]
 */
export async function loadStudentHomeAnalyticsPayload(supabase, params, ctx = {}) {
  ...
  const achievementGrants = await runStudentHomeAchievementGrants(supabase, studentId);
 ...
 * @param {string} studentId
 */
export async function runStudentHomeAchievementGrants(supabase, studentId) {
  try {
    const result = await evaluateAndGrantAchievementCards(supabase, studentId);
...
export async function buildLegacyStudentHomeProfilePayload(supabase, auth) {
  ...
  const analytics = await loadStudentHomeAnalyticsPayload(
    supabase,
    {
      studentId,
      displayName,
      gradeLevel: auth.student?.grade_level || "",
      row,
      normalized,
    },
    { timer }
  ...
  return {
    ...
    challenges: analytics.challenges ?? summary.challenges,
    ...
    achievementGrants: analytics.achievementGrants,
