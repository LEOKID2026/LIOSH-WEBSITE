...
/**
 * Returns the UTC instant that corresponds to 00:00:00 today in Asia/Jerusalem.
 *
 * Owner decision (2026-05-22): All student-facing daily logic (caps, missions, streaks)
 * must use Asia/Jerusalem calendar days — never UTC. Israel is UTC+2 (winter / standard
 * time) or UTC+3 (summer / DST). This helper handles both correctly.
 *
 * Algorithm:
 *   1. Format today's date in Israel timezone → "YYYY-MM-DD" (en-CA locale).
 *   2. Read Israel's current UTC offset from Intl → e.g. "GMT+3" → +180 min.
 *   3. Israel midnight UTC = parse("YYYY-MM-DDT00:00:00Z") − offsetMs.
 *
 * Example (summer, UTC+3):
 *   israelDate = "2026-05-22"
 *   offset     = +180 min
 *   result     = 2026-05-21T21:00:00Z  (May 22 00:00 Israel = May 21 21:00 UTC) ✓
 */
function getTodayIsraelMidnightUtc() {
  const now = new Date();

  const israelDate = new Intl.DateTimeFormat("en-CA", {
    timeZone: "Asia/Jerusalem",
  }).format(now); // e.g. "2026-05-22"
  ...
  // Parse "GMT+3" / "GMT+2" / "GMT+3:30" etc.
  ...
  return new Date(new Date(israelDate + "T00:00:00Z").getTime() - totalOffsetMs);
}
