...
export function applyBridgeProvenanceToGeneratedReport(report, dbInput, apiPayload) {
  ...
  const serverDurationSec = Math.max(0, Math.floor(Number(apiSummary.totalDurationSeconds) || 0));
  ...
  if (report.summary && typeof report.summary === "object") {
    ...
    for (const [subjectId, fields] of Object.entries(SUMMARY_SUBJECT_FIELDS)) {
      ...
      const questions = safeCount(source.diagnosticAnswers ?? source.answers ?? source.total);
      const correct = safeCount(source.diagnosticCorrect ?? source.correct);
  ...
  if (serverDurationSec > 0) {
    const mins = safeMinutesFromSeconds(serverDurationSec);
    if (report.summary && typeof report.summary === "object") {
