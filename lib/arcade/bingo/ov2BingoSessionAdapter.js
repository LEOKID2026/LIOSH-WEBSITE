...
export async function fetchOv2BingoLiveRoundSnapshot(roomId, _opts) {
  ...
  try {
    ...
    if (!rpcOkTruthy(b.ok) || b.snapshot == null || typeof b.snapshot !== "object") return null;
    return normalizeOv2BingoAuthoritativeSnapshot(b.snapshot, { viewerParticipantKey });
  } catch {
    return null;
  }
}
...
export async function openOv2BingoSession(roomId, hostParticipantKey) {
  ...
  const snap = await fetchOv2BingoLiveRoundSnapshot(roomId, { viewerParticipantKey: host });
  if (snap) return { ok: true, snapshot: snap, idempotent: true };
