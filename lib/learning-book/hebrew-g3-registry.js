...
export const HEBREW_G3_PAGE_ORDER = HEBREW_G3_BOOK_BATCHES.flatMap(
  (batch) => batch.pages
);
...
export const HEBREW_G3_BOOK_META = Object.freeze({
  subject: "hebrew",
  grade: "g3",
  routeBase: "/learning/book/hebrew/g3",
...
export function getHebrewG3PageNeighbors(pageId) {
  const index = HEBREW_G3_PAGE_ORDER.indexOf(pageId);
  if (index === -1) {
    return { prev: null, next: null, index: -1 };
  }
  return {
    prev: index > 0 ? HEBREW_G3_PAGE_ORDER[index - 1] : null,
    next:
      index < HEBREW_G3_PAGE_ORDER.length - 1
        ? HEBREW_G3_PAGE_ORDER[index + 1]
        : null,
    index,
  };
}
...
export function isValidHebrewG3PageId(pageId) {
  return HEBREW_G3_PAGE_ORDER.includes(pageId);
}
