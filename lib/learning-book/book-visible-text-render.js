 ...
 */
function flattenContentRuns(text, sourceText, sourceOffset = 0) {
  const runs = splitHebrewMathRuns(text);
  const scopedSource = sourceText || text;
  const offsetRuns = runs.map((run) => {
    const relStart = run.start ?? 0;
    const relEnd = run.end ?? relStart + run.value.length;
    return {
      ...run,
      start: sourceOffset + relStart,
      end: sourceOffset + relEnd,
    };
  });
  return flattenRunsToVisibleText(offsetRuns, scopedSource);
}
 ...
 * @param {number} [sourceOffset]
 */
function flattenProseSegment(text, sourceText, sourceOffset = 0) {
  const tokens = parseInlineMarkdown(text);
  let out = "";
  let searchFrom = sourceOffset;

  for (const token of tokens) {
    const idx = sourceText.indexOf(token.value, searchFrom);
    const tokenStart = idx >= 0 ? idx : searchFrom;
    out += flattenContentRuns(token.value, sourceText, tokenStart);
    searchFrom = tokenStart + token.value.length;
  }
 ...
 * @param {string} text
 */
function flattenFormulaBody(text) {
  return splitFormulaTokens(text)
...
function flattenMixedBodyInner(text) {
  ...
  if (isFormulaLikeBody(input)) {
    return flattenFormulaBody(input);
  }
  ...
  for (let i = 0; i < segments.length; i += 1) {
    ...
    if (i > 0) {
      out += interRunGapText(input, segments[i - 1].end, seg.start);
    }
    if (seg.type === "math") {
      out += stripStrayMarkdown(seg.value);
    } else {
      out += flattenProseSegment(seg.value, input, seg.start);
    }
  }
