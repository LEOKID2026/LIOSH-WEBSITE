...
export const HEBREW_G5_PAGE_ORDER = HEBREW_G5_BOOK_BATCHES.flatMap(
  (batch) => batch.pages
);
...
export const HEBREW_G5_BOOK_META = Object.freeze({
  subject: "hebrew",
  grade: "g5",
  routeBase: "/learning/book/hebrew/g5",
...
export function getHebrewG5PageNeighbors(pageId) {
  const index = HEBREW_G5_PAGE_ORDER.indexOf(pageId);
  if (index === -1) {
    return { prev: null, next: null, index: -1 };
  }
  return {
    prev: index > 0 ? HEBREW_G5_PAGE_ORDER[index - 1] : null,
    next:
      index < HEBREW_G5_PAGE_ORDER.length - 1
        ? HEBREW_G5_PAGE_ORDER[index + 1]
        : null,
    index,
  };
}
...
export function isValidHebrewG5PageId(pageId) {
  return HEBREW_G5_PAGE_ORDER.includes(pageId);
}
