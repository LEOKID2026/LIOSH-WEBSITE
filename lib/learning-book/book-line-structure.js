...
const KNOWN_LABEL_PREFIX =
  /^(?:שאלה|שלב\s*\d+|תשובה|דוגמה|רמז|זכרו|טיפ|קל\s*לטעות|עשרות|אחדות|סה״כ|סה"כ|מפרקים|מחברים|היה|הוציא|נשאר|מה\s+(?:יודעים|מבקשים|עושים)|חשבו|בדקו|ספרו|מצאו)\s*:/u;

const STRUCTURAL_BOLD_LABEL =
  /^(\*\*(?:שאלה|שלב\s*\d+|תשובה|דוגמה|רמז|זכרו|טיפ|קל\s*לטעות)[^*\n]*:\*\*)\s+(.*)$/su;
...
export function parseBookLineStructure(line) {
  ...
  if (colonIndex > 0 && colonIndex <= 72) {
    ...
    if (body && isPlausibleDisplayLabel(rawLabel)) {
      ...
      const bodyHasMath = /\d/.test(body) && /[+−\-=×÷?_]/.test(body);
      if (labelLooksKnown || (HEBREW.test(label) && bodyHasMath)) {
        return { label, body };
      }
    }
  }
