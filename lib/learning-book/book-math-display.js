...
function extendMathRunEnd(stripped, start) {
  if (isThousandsGroupedNumberAt(stripped, start)) {
    const end = extendThousandsGroupedNumberEnd(stripped, start);
  ...
  if (THOUSANDS_GROUPED_NUMBER.test(candidate.trim())) {
    return end;
  }
...
export function findInlineMathRuns(text) {
  ...
  for (let i = 0; i < stripped.length; i += 1) {
    if (isThousandsGroupedNumberAt(stripped, i)) {
      const end = extendThousandsGroupedNumberEnd(stripped, i);
      if (end != null && end > i) {
