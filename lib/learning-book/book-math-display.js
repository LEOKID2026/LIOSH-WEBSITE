...
function canBridgeHebrewInMathRun(stripped, start) {
  ...
  if (/^(?:קטן|גדול|שווה|צעדים|ימינה|שמאלה|ואז|ו|עוד)$/u.test(segment)) {
    return false;
  }

  const before = stripped.slice(Math.max(0, start - 8), start).trimEnd();
  if (/\d\s*$/.test(before)) return true;

  if (/^(?:מאה|עשר(?:ות|ה)?|אחד(?:ות|ה)?|אלפים|מיליון)$/u.test(segment)) {
    return true;
  }
  ...
  return MATH_OPERATOR.test(stripped[j]) || DIGIT.test(stripped[j]);
}
