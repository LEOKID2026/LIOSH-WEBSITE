...
const MATH_RUN_PATTERNS = [
  ...
  // Coin amounts
  /\d+\s*₪/g,
  // Number-line rows inside diagrams only (digit — digit — digit), not prose "10 — word"
  /(?:\d+[\s—–\-]+){2,}\d+/g,
];
...
export function splitHebrewMathRuns(text) {
  ...
  for (const m of matches) {
    if (m.start > last) {
      parts.push(
        ...splitBareDigits(input.slice(last, m.start), last)
      );
    }
    parts.push({ type: "math", value: m.value, start: m.start, end: m.end });
    last = m.end;
  }

  if (last < input.length) {
    parts.push(...splitBareDigits(input.slice(last), last));
  }

  if (!parts.length) {
    parts.push(...splitBareDigits(input, 0));
  }

  if (!parts.length) {
    parts.push({ type: "text", value: input, start: 0, end: input.length });
  }
