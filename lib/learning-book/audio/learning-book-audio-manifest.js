...
/** @typedef {{ subject: string, grade: string, pageIds: readonly string[], sectionsPerPage: number, cacheVersion: string, mode: "flat-page"|"section", expectedPages?: number }} BookAudioScope */
...
export function resolveBookGlobalPageNumber(subject, grade, pageId, sectionNumber) {
  ...
  if (!isBookSectionAudioSlot(subject, grade, pageId, sectionNumber)) return null;
  const lookup = _FLAT_PAGE_LOOKUPS.get(`${scope.subject}:${scope.grade}`);
  if (!lookup) return null;
  const n = lookup.get(`${String(pageId || "").trim()}:${Number(sectionNumber)}`);
