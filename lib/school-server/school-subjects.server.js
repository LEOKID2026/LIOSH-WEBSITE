...
export function filterReportByPermittedSubjects(reportPayload, permittedSubjects) {
  if (!permittedSubjects || permittedSubjects.size === 0) {
    if (permittedSubjects && permittedSubjects.size === 0) {
      ...
      zeroed.teacherGuidanceBlock = buildStudentTeacherGuidance(zeroed);
  ...
  reconciled.teacherGuidanceBlock = buildStudentTeacherGuidance(reconciled);
...
function applyDailyActivityFilterFromSubjectBreakdown(payload, permittedSubjects) {
  ...
  const breakdown = payload._dailyBySubject;
  ...
  // _dailyBySubject is a server-only aggregation helper. Strip it from the
  // payload so the API response cannot leak per-subject day-level breakdowns
  // for subjects that are outside the caller's permitted scope.
  delete payload._dailyBySubject;
}
