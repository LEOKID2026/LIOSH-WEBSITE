...
export async function verifyStudentVisibleToSchool(serviceRole, schoolId, studentId) {
  ...
  if (!visible.studentIds.includes(studentId)) {
 ...
 * @param {string} studentId
 */
export async function resolveSchoolReportTeacherForStudent(serviceRole, schoolId, studentId) {
  const scope = await loadSchoolScope(serviceRole, schoolId);
  if (!scope.ok) return scope;
...
export async function loadSchoolClassInScope(serviceRole, schoolId, classId) {
  ...
  const ownedBySchoolTeacher = scope.teacherIds.includes(data.teacher_id);
  const taggedToSchool = data.school_id === schoolId;

  if (!ownedBySchoolTeacher && !taggedToSchool) {
    return { ok: false, status: 403, code: "class_not_in_school" };
  }
