...
export async function requireParentApiContext(res, authHeader, options = {}) {
  const ctx = await requirePersonaApiContext(res, authHeader, "parent");
  if (!ctx.ok) return ctx;

  const settingsResult = await loadParentAccountSettings(ctx.serviceRole, ctx.userId);
 ...
 * @param {{ requireGrant?: 'student_access_admin'|'student_data_viewer' }} [options]
 */
export async function requireSchoolOperatorApiContext(res, authHeader, schoolId, options = {}) {
  ...
  const serviceRole = getLearningSupabaseServiceRoleClient();
  const operatorCheck = await hasActivePersonaEntitlement(serviceRole, auth.userId, "school_operator");
  if (!operatorCheck.ok) {
  ...
  const membershipVerify = await verifyTeacherMembershipInSchool(serviceRole, schoolId, auth.userId);
