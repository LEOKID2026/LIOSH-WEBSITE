...
function resolveApprovedCopy(patternRaw, topicTitle) {
  ...
  if (/כיוון הפוך|הוספה במקום חיסור|השלמה לעשר/i.test(pl)) {
    return {
      ...
      requiresEvidenceTag: "add_instead_of_sub",
    };
  }
...
function buildRegularReportTopicMeaningHe(row, title, pattern) {
  ...
  if (pattern) {
    ...
    if (!gatedPattern && /כיוון הפוך|הוספה במקום חיסור|omitted|מחובר/i.test(pattern)) {
      return genericStableTopicCopyHe(title, Number(row?.questions) || 0, Number(row?.accuracy) || 0).whatItMeans;
    }
    ...
    if (approved?.requiresEvidenceTag && !pipeline.allowsSpecificDiagnosis) {
      return genericLowVolumeTopicCopyHe(title, Number(row?.questions) || 0).whatItMeans;
    }
...
export function buildApprovedTopicCopyHe(row, registeredGradeKey) {
  ...
  const q = Number(row.questions) || 0;
  const acc = Math.round(Number(row.accuracy) || 0);
  const engineFinding = stripParentTopicSectionPrefixHe(resolveTopicParentFindingHe(row) || "");
  if (engineFinding) {
    const bundle = buildTopicParentReportBundleHe(row);
    return {
      title,
      whatWeSee: q > 0 ? `נפתרו ${q} שאלות, והדיוק עומד על ${acc}%.` : "",
      whatItMeans: engineFinding,
      homeAction:
        bundle.homeRecommendation ||
        "לתרגל כמה שאלות קצרות ולבקש מהילד להסביר את הדרך.",
      prominent: null,
      strength: null,
      focusLine: engineFinding,
    };
  }
