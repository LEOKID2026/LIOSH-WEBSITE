...
import { buildTopicOwnerCopySlots } from "../../utils/learning-pattern-decision/resolve-topic-owner-copy.js";
...
function patternFromRow(row) {
  ...
  const contract =
    lpd?.engineDecisionContract && typeof lpd.engineDecisionContract === "object"
      ...
      : null;
  return clean(slots?.detectedPattern || contract?.detectedPattern || "");
}
...
export function buildApprovedTopicCopyHe(row, registeredGradeKey) {
  ...
  if (pattern) {
    ...
    return {
      ...
      whatItMeans: approved.meaning,
      homeAction: approved.home,
      prominent: approved.prominent,
      strength: null,
      focusLine: approved.focusLine || approved.meaning,
    };
  }
  ...
  if (q > 0 && acc < 72) {
    return {
      title,
      whatWeSee: `נפתרו ${q} שאלות, והדיוק עומד על ${acc}%.`,
      whatItMeans: "יש טעויות שחוזרות בתרגול — כדאי לעקוב אחרי דרך הפתרון.",
