...
assert.equal(topicMaps.mathOperations.addition.timeMinutes, 33);
assert.equal(topicMaps.mathOperations.addition.mode, "למידה");
assert.equal(topicMaps.mathOperations.addition.level, "בינוני");
...
assert.equal(topicMaps.geometryTopics.shapes.timeMinutes, 27);
...
assert.equal(orphanMaps.geometryTopics["shapes_basic::grade:g1"].timeMinutes, 149);
