    ...
    if (evRec.confirmed || evRec.state === "confirmed") {
      statements.push({
        ...
        evidenceRef: {
          ...
          examples,
          patternLayer: "primary_dominant",
        },
      });
    } else if (evRec.state === "recurring" || (evRec.evidenceCount >= 3 && evRec.reasonCode === "low_occurrence_ratio")) {
      const observedHe =
        tag === "omitted_addend"
          ? `ב-${evRec.evidenceCount} מתוך ${evRec.relevantQuestions} תרגילים עם שלושה מחוברים חוברו רק שני המחוברים הראשונים.`
          : tag === "add_instead_of_sub"
            ? `ב-${evRec.evidenceCount} מתוך ${evRec.relevantQuestions} תרגילי חיסור, התשובה שווה לחיבור המחוסר והמחסר.`
            : `לא הופיע דפוס מרכזי אחד, אבל חזר אותו סוג של טעות: "${tax?.patternHe || tag}" (${evRec.evidenceCount} פעמים).`;
      statements.push({
        ...
        evidenceRef: {
          ...
          occurrenceRatio: evRec.occurrenceRatio,
          examples,
          patternLayer: "secondary_observed",
        },
      });
    } else if (tag === "omitted_addend") {
      statements.push({
        kind: "OBSERVED_PATTERN",
