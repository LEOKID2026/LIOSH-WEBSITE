...
Object.assign(RULE_LOADERS, {
  ...
  // Generator grammar sampling often tags a distractor as grammar_agreement_error while
  // classifyAnswerEvidence returns writing_pattern_error/null — pin a stable bank row.
  "H-02": {
    subject: "hebrew",
    ...
    kind: "mcq_bank",
    bank: () => {
      const row = HEBREW_RICH_POOL.find(
        (r) => r.topic === "grammar" && r.patternFamily === "verb_agreement"
      );
      if (!row) return null;
      return enrichBankMcq(
        {
