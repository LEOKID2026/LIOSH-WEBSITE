...
function topicRowFromDbInputTopic(topicKey, topic, displayNameFn) {
  ...
  const timeMinutes = Math.round(safeFloor(topic.durationSeconds) / 60);
  ...
  return {
    ...
    modeKey: topic.dominantMode || null,
    gradeKey: topic.contentGradeLevel || null,
    levelKey: topic.dominantLevel || null,
    latestActivityAt: topic.latestActivityAt || null,
    latestActivityMs: Number.isFinite(Number(topic.latestActivityMs)) ? Number(topic.latestActivityMs) : null,
    lastAnswerAt: topic.lastAnswerAt || null,
    lastAnswerMs: Number.isFinite(Number(topic.lastAnswerMs)) ? Number(topic.lastAnswerMs) : null,
  };
}
...
export function syncReportVisiblePracticeFromServer(report, sources = {}) {
  ...
  report.rawMetricStrengthsHe = deriveRawMetricStrengthLinesHe(report.summary);
