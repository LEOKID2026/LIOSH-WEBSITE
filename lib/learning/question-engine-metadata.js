...
export function questionRecordToEngineInput(record) {
  ...
  const params =
    record.params && typeof record.params === "object" && !Array.isArray(record.params)
      ? record.params
      : {};
  const choices = resolveChoicesArray(record);
  let correctAnswer =
  ...
  return {
    ...
    answers: record.answers ?? record.options ?? record.choices,
    options: record.options ?? record.answers ?? record.choices,
    correctAnswer,
    params,
...
export function buildQuestionEngineMetadataFromQuestion(question, options = {}) {
  ...
  const choices = resolveChoicesArray(q);
