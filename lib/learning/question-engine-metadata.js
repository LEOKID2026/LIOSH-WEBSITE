...
import { classifyAnswerEvidence } from "./classifiers/classify-answer-evidence.js";
import { normalizeAnswerEvidence } from "./answer-evidence-contract.js";
import { pickPersistableMathClassifierParams } from "./math-classifier-params.js";
import {
  applyMcqEvidenceTaggingToQuestion,
  enrichMcqChoicesWithEvidenceTags,
} from "./mcq-option-evidence-tagging.js";
...
/**
 * Map live/runtime answer UI mode → engine questionType.
 * Used to isolate student typed/numeric input from generator distractor arrays.
 *
 * @param {unknown} answerMode
 * @returns {"mcq"|"numeric"|"open"|null}
 */
export function mapAnswerModeToQuestionType(answerMode) {
  const m = strOrNull(answerMode)?.toLowerCase();
  if (!m) return null;
  if (["choice", "mcq", "multiple_choice", "select"].includes(m)) return "mcq";
  if (["numeric", "number", "typed", "input"].includes(m)) return "numeric";
  if (["open", "text", "typing", "freeform", "free_text"].includes(m)) return "open";
  return null;
}

/**
 * @param {Record<string, unknown>|null|undefined} question
 * @param {{ answerMode?: string|null }} [options]
 *   When `answerMode` is set (student interactive runtime), it wins over the
 *   presence of internal generator `answers`/`choices` distractor lists.
 *   Worksheet / print / genuine MCQ UIs omit answerMode and keep choices→mcq.
 */
export function detectQuestionTypeFromRecord(question, options = {}) {
  if (!question || typeof question !== "object") return "unknown";

  // 1) Explicit runtime presentation from caller (student typed vs choice UI)
  const fromOptions = mapAnswerModeToQuestionType(options?.answerMode);
  if (fromOptions) return fromOptions;

  // 2) Explicit type on the record (worksheets / authored MCQ)
  ...
  // 3) Record / params answerMode (when dual-written by student runtime)
  const params =
    question.params && typeof question.params === "object" && !Array.isArray(question.params)
      ? question.params
      : {};
  const fromRecordMode =
    mapAnswerModeToQuestionType(question.answerMode) ||
    mapAnswerModeToQuestionType(params.answerMode) ||
    mapAnswerModeToQuestionType(params.runtimeAnswerMode);
  if (fromRecordMode) return fromRecordMode;

  // 4) Default: choices array ⇒ mcq (worksheet generator / print / MCQ UI)
...
export function buildQuestionEngineMetadataFromQuestion(question, options = {}) {
  ...
  const answerMode =
    strOrNull(options.answerMode) ||
    strOrNull(q.answerMode) ||
    strOrNull(q.params && typeof q.params === "object" ? q.params.answerMode : null);
  const questionType = detectQuestionTypeFromRecord(q, { answerMode });
  const choices = resolveChoicesArray(q) ?? (Array.isArray(q.answers) ? q.answers : null);
  // Only expose MCQ choice lists when the active runtime/presentation is MCQ.
  // Generator may still attach distractor `answers` for worksheets — ignore for typed.
  ...
  if (questionType === "mcq") {
    ...
    selectedIndex = indices.selectedOptionIndex;
    correctIndex = indices.correctOptionIndex;
  ...
  if (selectedAnswer && selectedCell && questionType === "mcq") {
  ...
  const structuredParams = pickPersistableMathClassifierParams(
    q.params && typeof q.params === "object" && !Array.isArray(q.params) ? q.params : {},
    q,
  );
  if (!strOrNull(structuredParams.kind)) {
    const gk = strOrNull(diag.kind) ?? strOrNull(q.kind);
    if (gk) structuredParams.kind = gk;
  }
  ...
  const engine = {
    ...
    difficulty:
      ...
      strOrNull(q.params?.difficulty) ??
      null,
    // Persist structured generator params so answer API can classify without UI changes
...
export function normalizeQuestionEnginePayload(raw) {
  ...
  const answerEvidence = normalizeAnswerEvidence(raw.answerEvidence);
  if (answerEvidence) out.answerEvidence = answerEvidence;
...
export function extractRecentMistakeEngineFields(questionEngine) {
  ...
  if (strOrNull(questionEngine.misconceptionTag)) {
    out.misconceptionTag = questionEngine.misconceptionTag;
  }
  if (strOrNull(questionEngine.distractorFamily)) {
    out.distractorFamily = questionEngine.distractorFamily;
  }
