...
/** @typedef {{ id: string, difficulty: string, pickCount: number, validItems: string[], exactMatch?: boolean, title?: string }} LabExperiment */
 ...
 * @param {Record<string, string[]>} [shelfByDifficulty]
 * @returns {{ ok: boolean, issues: { id: string, difficulty: string, problems: string[] }[] }}
 */
export function auditLeoLabValidItems(
  experimentsByDifficulty = EXPERIMENTS_BY_DIFFICULTY,
  shelfByDifficulty = SHELF_BY_DIFFICULTY,
) {
  ...
  for (const [difficulty, experiments] of Object.entries(experimentsByDifficulty)) {
    const shelf = shelfByDifficulty[difficulty] ?? [];
...
for (const [difficulty, experiments] of Object.entries(EXPERIMENTS_BY_DIFFICULTY)) {
  const shelfSet = new Set(SHELF_BY_DIFFICULTY[difficulty] ?? []);
  ...
  for (const exp of experiments) {
    ...
    for (const itemId of exp.validItems) {
      assert.ok(
        shelfSet.has(itemId),
        `${exp.id}: validItem "${itemId}" not on ${difficulty} shelf`,
      );
    }
