...
const SUBJECT_UI = {
  ...
  geometry: { labelHe: "גאומטריה", href: "/learning/geometry-master" },
...
export function buildStudentHomeView({ student, homePayload }) {
  ...
  const analyticsPending =
    homePayload?.analyticsPending === true || !derived || !snap;
  ...
  const gradeKeyForSubjects = normalizeGradeLevelToKey(gradeLevelRaw);
  const moledetAllowedForStudent = isMoledetGeographyGradeAllowed(gradeKeyForSubjects);
  ...
  if (analyticsPending) {
    ...
    for (const sk of STUDENT_HOME_SUBJECT_ORDER) {
      if (sk === "moledet_geography" && !moledetAllowedForStudent) continue;
      ...
      subjects.push({
        ...
        href: meta.href,
        level: null,
        levelDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        stars: null,
        starsScopeHe: STUDENT_TRUTH_LABELS_HE.cumulative,
        bestScore: 0,
        bestStreak: 0,
        accuracyPct: null,
        accuracyDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        accuracyState: StudentDisplayTruthState.unavailable,
        answersTotal: 0,
        answersDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        answersState: StudentDisplayTruthState.unavailable,
        correctTotal: 0,
        sessionMinutesRounded: null,
        sessionMinutesDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        progressIndicatorPct: null,
        progressIndicatorState: StudentDisplayTruthState.unavailable,
        hasServerSlice: false,
      });
    }
    ...
    return {
      identity: {
        ...
        coinBalance: coinTruth.value,
        coinBalanceDisplayHe: coinTruth.displayHe,
        coinBalanceState: coinTruth.state,
        avatarEmoji,
        avatarCustomDataUrl,
      ...
      accountStats: {
        ...
        overallAccuracyDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        overallAccuracyState: StudentDisplayTruthState.unavailable,
        questionsAnswered: 0,
        questionsAnsweredDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        questionsAnsweredState: StudentDisplayTruthState.unavailable,
        correctAnswers: 0,
        learningMinutesLifetimeRounded: null,
        learningMinutesLifetimeDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        learningMinutesLifetimeScopeHe: STUDENT_TRUTH_LABELS_HE.cumulative,
        learningMinutesThisMonth: null,
        learningMinutesThisMonthDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        learningMinutesThisMonthState: StudentDisplayTruthState.unavailable,
        learningMinutesFilterNoteHe: STUDENT_TRUTH_LABELS_HE.allSessionsWithDuration,
        monthlyGoalMinutes: economyConfig?.goalMinutes != null ? Number(economyConfig.goalMinutes) : null,
      },
      monthlyJourney: {
        yearMonth: "",
        minutesThisMonth: null,
        minutesDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        minutesState: StudentDisplayTruthState.unavailable,
        filterNoteHe: STUDENT_TRUTH_LABELS_HE.allSessionsWithDuration,
        goalMinutes: economyConfig?.goalMinutes != null ? Number(economyConfig.goalMinutes) : null,
        progressPct: null,
        progressState: StudentDisplayTruthState.unavailable,
        celebrationShownForMonth: false,
        minutesRemaining: null,
        scopeLabelHe: STUDENT_TRUTH_LABELS_HE.periodThisMonth,
        encouragementHe: STUDENT_TRUTH_LABELS_HE.loading,
      },
      subjects,
      badges: [],
      recommendations: [
        {
          ...
          descriptionHe: "בחרו נושא מהרשימה למטה כדי להמשיך ללמוד.",
          href: "/learning",
          ctaHe: "מעבר לעמוד הלימודים",
          truthState: StudentDisplayTruthState.estimated,
          hintHe: STUDENT_TRUTH_LABELS_HE.heuristicHint,
        },
      ],
      dailyMissions,
      monthlyPersistence: {
        currentMinutes: null,
        currentMinutesDisplayHe: STUDENT_TRUTH_LABELS_HE.loading,
        currentMinutesState: StudentDisplayTruthState.unavailable,
        filterNoteHe: STUDENT_TRUTH_LABELS_HE.completedSessionsOnly,
        minutesFilterMismatch: false,
        loadError: false,
        economyConfigLoadError,
        tiers: monthlyPersistenceTiers,
        completedTiers: [],
        nextTier: null,
        progressToNextTierPct: null,
        minutesToNextTier: null,
        nextTierEncouragementHe: STUDENT_TRUTH_LABELS_HE.loading,
      },
      meta: {
        hasHomeDashboardPayload: true,
        analyticsPending: true,
        subjectsKeys: [...STUDENT_HOME_SUBJECT_ORDER],
        monthlyPersistenceLoadError: false,
        minutesFilterMismatch: false,
  ...
  const derivedMonthMinutes = finiteNumberOrNull(
  ...
  const minutesThisMonth =
    derivedMonthMinutes != null ? Math.round(derivedMonthMinutes * 100) / 100 : null;
  const monthlyMinutesTruth = classifyMonthlyMinutes(minutesThisMonth, {
    serverConfirmed: true,
    filterNoteHe: STUDENT_TRUTH_LABELS_HE.allSessionsWithDuration,
  });
  const persistenceMinutesTruth = classifyMonthlyMinutes(persistenceActiveMinutes, {
    serverConfirmed: monthlyPersistenceStatus != null && !monthlyPersistenceLoadError,
    filterNoteHe: STUDENT_TRUTH_LABELS_HE.completedSessionsOnly,
  });
  const minutesFilterMismatch =
    minutesThisMonth != null &&
    persistenceActiveMinutes != null &&
    Math.abs(minutesThisMonth - persistenceActiveMinutes) > 0.05;
  const progressPct =
  ...
  const monthlyPersistence = {
    ...
    currentMinutesDisplayHe: monthlyPersistenceLoadError
      ...
      : persistenceMinutesTruth.displayHe !== STUDENT_TRUTH_LABELS_HE.noData
        ? persistenceMinutesTruth.displayHe
        : monthlyMinutesTruth.displayHe,
    currentMinutesState: monthlyPersistenceLoadError
      ? StudentDisplayTruthState.unavailable
      : persistenceActiveMinutes != null
        ? persistenceMinutesTruth.state
        : monthlyMinutesTruth.state,
    filterNoteHe: persistenceActiveMinutes != null
      ? STUDENT_TRUTH_LABELS_HE.completedSessionsOnly
      : STUDENT_TRUTH_LABELS_HE.allSessionsWithDuration,
    minutesFilterMismatch,
