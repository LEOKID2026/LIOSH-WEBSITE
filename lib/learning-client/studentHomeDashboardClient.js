...
export function buildStudentHomeView({ student, homePayload }) {
  ...
  const completedTiers =
    persistencePanelMinutes != null
      ? MONTHLY_PERSISTENCE_TIERS.filter((t) => persistencePanelMinutes >= t.minutes)
      : [];
  const nextTierObj =
    persistencePanelMinutes != null
      ? MONTHLY_PERSISTENCE_TIERS.find((t) => persistencePanelMinutes < t.minutes) ?? null
      : null;
