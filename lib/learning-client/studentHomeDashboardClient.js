...
export function buildStudentHomeView({ student, homePayload }) {
  ...
  const monthlyPersistence = {
    ...
    minutesToNextTier: nextTierObj
      ? Math.max(0, Math.round((nextTierObj.minutes - currentMonthMinutes) * 10) / 10)
      : 0,
    nextTierEncouragementHe: nextTierObj
      ? `עוד ${Math.max(0, Math.ceil(nextTierObj.minutes - currentMonthMinutes))} דקות לפרס הבא`
