import { normalizeReportBucketKey } from "../../utils/diagnostic-engine-v2/topic-taxonomy-bridge.js";
...
export async function fetchStudentActionDecisions() {
  const res = await fetch("/api/student/action-decisions", {
    credentials: "same-origin",
    cache: "no-store",
  });
  const json = await res.json().catch(() => ({}));
  if (!res.ok || !json.ok || !Array.isArray(json.decisions)) {
    const msg = json?.error ? String(json.error) : `HTTP ${res.status}`;
    throw new Error(msg);
  }
  return json;
...
export function findStudentActionDecision(response, subjectId, topicKey) {
  const decisions = Array.isArray(response?.decisions) ? response.decisions : [];
  const canonicalTopic = normalizeReportBucketKey(topicKey).normalizedBucketKey;
  return (
    decisions.find(
      (decision) =>
        decision?.target?.subject === subjectId &&
        normalizeReportBucketKey(decision?.target?.topic).normalizedBucketKey ===
          canonicalTopic,
    ) ||
    null
  );
