...
const REQUIRES_EVENTS = "דורש מעקב אירועים";
...
function unavailable(label, source, error) {
  return metric(label, null, source, {
    status: "unavailable",
    note: error?.message || "מקור נתונים חסר",
  });
}
...
function buildBooksWorksheetsRewards(bookSessions, bookVisits, worksheetStatuses, coinRows, activeStudentIds, events) {
  ...
  return {
    ...
    rewards: [
      ...
      events
        ? metric("אירועי reward_earned", eventCount(events, "reward_earned"), "analytics_events.reward_earned", {
            status: eventCount(events, "reward_earned") ? "available" : "empty",
            note: eventCount(events, "reward_earned") ? null : "אין נתונים עדיין",
          })
        : requiresEvents("אירועי reward_earned"),
  ...
  return {
    cards: [
      ...
      metric("סיכומי מקור דוח", null, "aggregateParentReportPayload", {
        status: "partial",
        note: "Phase 1 מציג בדיקת מקור גולמית; השוואת payload מלאה תתווסף בשער אמת ממוקד",
      }),
    ],
    suspicious,
  };
  ...
  if (!events) {
    return {
      ...
      steps: steps.map((step) => ({
        ...
        source: fallbackCounts[step.event] != null ? step.fallbackSource || "existing DB truth" : "analytics_events",
      })),
    };
  }
...
function buildAccountAnalytics(authUsersResult, parentProfiles, teacherProfiles, entitlements, range) {
  ...
  return {
    cards: [
      authUsers ? metric("סה״כ חשבונות Auth", authUsers.length, "auth.users") : unavailable("סה״כ חשבונות Auth", "auth.users", authUsersResult.error),
      ...
      metric("חשבונות admin", entitlementRows.filter((ent) => ent.persona === "admin").length, "account_persona_entitlements persona=admin"),
      ...
      metric("הרשאות persona ללא פרופיל", missingProfileEntitlements, "account_persona_entitlements + profiles", { status: missingProfileEntitlements ? "available" : "empty", note: missingProfileEntitlements ? null : "אין נתונים עדיין" }),
...
function buildTeacherAnalytics(teacherProfiles, entitlements, teacherClasses, teacherStudents, studentActivities, classroomActivities, worksheetActivities, events, range) {
  ...
  return {
    cards: [
      ...
      metric("מורים עם הרשאת private_teacher", privateTeacherIds.size, "account_persona_entitlements persona=private_teacher"),
...
export async function getAdminAnalyticsDashboard(serviceRole, query = {}) {
  ...
  const studentsResult = await fetchAll(
    serviceRole,
    "students",
    "id,parent_id,grade_level,is_active,created_at",
  ...
  const overview = [
    ...
    allEntitlements.ok
      ? metric("הורים עם הרשאת parent", allEntitlements.rows.filter((ent) => ent.persona === "parent").length, "account_persona_entitlements")
      : unavailable("הורים עם הרשאת parent", "account_persona_entitlements", allEntitlements.error),
    ...
    metric("אורך מפגש ממוצע", sessions.length ? round(totalMinutes / sessions.length, 1) : 0, "learning_sessions.duration_seconds", { unit: "minutes" }),
