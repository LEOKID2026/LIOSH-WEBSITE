...
const AGGREGATE_DIMENSIONS = [
  { key: "daily", by: "day", labelKey: "day" },
  { key: "topPages", by: "requestPath", labelKey: "requestPath" },
  { key: "referrers", by: "referrerHostname", labelKey: "referrerHostname" },
  { key: "devices", by: "deviceType", labelKey: "deviceType" },
  { key: "browsers", by: "browserName", labelKey: "browserName" },
  { key: "countries", by: "country", labelKey: "country" },
];
 ...
 */
function normalizeAggregateRows(data, labelKey) {
  ...
  return rows
    .map((rawRow) => {
      ...
      const label =
        ...
        row.name ??
        "—";
      const pageviews = safeNum(row.pageviews ?? row.pageViews ?? row.count ?? row.value);
      const visitors = safeNum(row.visitors ?? row.visitorCount ?? row.uniqueVisitors);
      return {
        label: String(label),
        date: row.day || row.date || null,
        pageviews,
        value: pageviews,
    ...
    const aggregateResults = await Promise.all(
      AGGREGATE_DIMENSIONS.map(async (dimension) => {
        ...
        return {
          key: dimension.key,
          ok: true,
          rows: normalizeAggregateRows(result.data, dimension.labelKey),
        };
      })
    );
