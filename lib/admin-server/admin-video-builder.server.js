...
async function renderSceneSegment(scene, outPath, width, height, mediaAsset, watermarkAsset = null) {
  ...
  if (watermarkAsset?.type === "image" && watermarkAsset.url) {
    const wmPath = path.join(process.cwd(), "public", String(watermarkAsset.url).replace(/^\//, ""));
    const wmStat = existsSync(wmPath) ? await fs.stat(wmPath).catch(() => null) : null;
...
async function mergeSegments(segmentPaths, scenes, defaultTransition, workDir, outPath) {
  ...
  await runFfmpeg(
    [...inputArgs, "-filter_complex", parts.join(";"), "-map", "[outv]", "-y", outPath],
    workDir
  );
}
...
async function attachAudioTracks(videoPath, project, workDir, outPath) {
  ...
  if (settings.voiceoverAssetId) {
    ...
    const voicePath = voiceAsset?.url
      ? path.join(process.cwd(), "public", String(voiceAsset.url).replace(/^\//, ""))
      : null;
  ...
  if (settings.backgroundMusicAssetId) {
    ...
    const musicPath = musicAsset?.url
      ? path.join(process.cwd(), "public", String(musicAsset.url).replace(/^\//, ""))
      : null;
  ...
  if (mixLabels.length === 1) {
    await runFfmpeg(
      [...inputs, "-filter_complex", filters.join(";"), "-map", "0:v", "-map", mixLabels[0], "-c:v", "copy", "-c:a", "aac", "-y", outPath],
      workDir
    );
    return;
  }
  ...
  await runFfmpeg(
    [...inputs, "-filter_complex", filters.join(";"), "-map", "0:v", "-map", "[aout]", "-c:v", "copy", "-c:a", "aac", "-y", outPath],
    workDir
  );
}
...
export async function exportVideoProjectMp4(project) {
  ...
  try {
    ...
    await fs.copyFile(withAudioPath, outputDisk);
