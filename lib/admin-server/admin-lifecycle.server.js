...
export const LIFECYCLE_ACTIONS = new Set(["suspend", "reactivate", "revoke"]);
...
export async function applyAdminPrivateTeacherLifecycle(serviceRole, adminUserId, userId, action, reason) {
  ...
  if (action === "reactivate") {
    ...
    return { ok: true, entitlement: formatAdminEntitlement(entPatch.after), isAccountActive: true };
  }
...
export async function applyAdminSchoolStaffLifecycle(
  ...
  const status = action === "suspend" ? "suspended" : action === "reactivate" ? "active" : "revoked";
  const entPatch = await patchEntitlementStatus(
    serviceRole,
    entResult.entitlement.id,
    adminUserId,
