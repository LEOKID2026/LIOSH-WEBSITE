...
export async function listAllAdminAccounts(serviceRole, actorUserId, actorUser) {
  ...
  if (!userIds.length) {
    return {
      ...
      fullDeleteConfigured: isFullAccountDeleteConfigured(),
      accounts: [],
      total: 0,
    };
  }

  const [entRes, parentProfRes, teacherProfRes, membershipRes, parentSettingsRes, limitsRes, schoolsRes] =
    await Promise.all([
  ...
  for (const user of authUsers) {
    ...
    const protection = await assessDeleteProtection(serviceRole, actorUserId, user);
    const isProtected =
      !protection.ok ||
    ...
    const isSchoolAffiliated = hasSchoolMembership;
    const manageUrls = {
      parent:
