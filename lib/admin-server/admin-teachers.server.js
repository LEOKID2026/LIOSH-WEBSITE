...
async function batchBuildAdminTeacherSummaries(serviceRole, profiles, emailMap) {
  ...
  for (const idChunk of chunkIds(teacherIds, 80)) {
    const { data, error } = await serviceRole
      .from("teacher_limits")
      .select(
        "id, plan_code, student_limit_override, class_limit_override, max_students_per_class_override, notes, feature_flags, is_account_active"
      )
      .in("id", idChunk);
    if (error) {
      if (isDbSchemaNotReadyError(error)) {
    ...
    for (const row of data || []) {
      if (row?.id) limitsById.set(row.id, row);
    }
  }
