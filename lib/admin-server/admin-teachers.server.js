...
export async function buildAdminTeacherDetail(serviceRole, teacherId, emailMap) {
  ...
  const { data: regRow, error: regErr } = await serviceRole
    ...
    .select("status, description, requested_subjects, created_at, updated_at")
...
export async function listAdminTeachers(serviceRole, options = {}) {
  ...
  for (const p of profiles || []) {
    if (affiliated.ids.has(p.id)) continue;
    const entStatus = entMap.get(p.id) ?? null;
    if (statusFilter === "pending" && entStatus !== "pending") continue;
    const detail = await buildAdminTeacherDetail(serviceRole, p.id, emailResult.map);
    if (!detail.ok) continue;
    if (detail.teacher.schoolMembership?.schoolId) continue;
    teachers.push({ ...detail.teacher, entitlementStatus: entStatus });
  }
