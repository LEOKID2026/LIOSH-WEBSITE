...
export function isWorksheetMathLtrExpression(text) {
  ...
  return MATH_OPERATOR_RE.test(value) || BLANK_RE.test(value) || /\//.test(value);
}
...
export function renderWorksheetStemSplitFromParts(split, escapeHtml, opts = {}) {
  ...
  if (split.mode === "mixed-inline" || split.mode === "prose-only") {
    return `<div class="worksheet-prose-math-lines">${proseSlot(`<p class="worksheet-stem worksheet-stem-prose" dir="rtl">${escapeHtml(split.proseHe || "")}</p>`)}${mathSlot("", true)}</div>`;
  }
  return "";
}
