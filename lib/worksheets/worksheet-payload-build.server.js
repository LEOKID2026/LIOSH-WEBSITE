...
import { stripMathLtrMarkers } from "./worksheet-math-display.server.js";
 ...
 * @param {{ subjectId: WorksheetSubjectId, mathPracticeFormat?: string | null }} opts
...
export function buildAnswerKeyPayload(rawQuestions, meta, opts = {}) {
  ...
  const answers = rawQuestions.map((raw, i) =>
    toAnswerKeyEntry(raw, {
      displayIndex: i + 1,
      subject,
      mathPracticeFormat: opts.mathPracticeFormat ?? meta.mathPracticeFormat,
      gradeKey: meta.gradeKey,
...
export function worksheetPayloadToPreviewHtml(payload) {
  ...
  const bodyGridClass = getWorksheetBodyGridClass(questions);
  const header = `
    ...
    .map((q) => {
      ...
      const options =
        q.optionsHe?.length
          ? `<ol class="worksheet-options worksheet-options-grid">${q.optionsHe
              .map((o, i) => {
                const useLtr =
                  q.optionsLatin?.[i] ?? (isMostlyLatinText(o) || isWorksheetNumericOption(o));
                const cls = useLtr ? ' class="worksheet-math-ltr english-ltr" dir="ltr"' : "";
                return `<li class="worksheet-option-cell"><span class="worksheet-option-label">${["א", "ב", "ג", "ד"][i] || i + 1}.</span><span class="worksheet-option-text"><span${cls}>${escapeWorksheetHtml(o)}</span></span></li>`;
              })
              .join("")}</ol>`
          : "";
      ...
      const writing =
        q.writingSpaceLines && q.writingSpaceLines > 0
          ...
          : "";
      const stemClass = q.hasNikud ? "worksheet-stem worksheet-hebrew-nikud" : "worksheet-stem";
      const useFractionStem = q.questionType === "fraction";
      const stemSplit = splitWorksheetStemProseAndMath(q.stemHe);
      const splitStemHtml =
        stemSplit.mode === "split" || stemSplit.mode === "mixed-inline" || stemSplit.mode === "math-only"
      ...
      const proseStemHtml =
        ...
        !isWorksheetMathLtrExpression(q.stemHe) &&
        !q.mathExpressionLtr
          ? `<p class="${stemClass}">${renderStemWithLtrSpansHtml(q.stemHe, q.ltrSpans, escapeWorksheetHtml)}</p>`
          : "";
      const mathExpr =
        !splitStemHtml && q.mathExpressionLtr
          ...
          : "";
      const vertical = q.verticalLayoutLtr
        ? `<div class="worksheet-math-vertical-slot"><div class="worksheet-math-block worksheet-math-block-compact worksheet-math-block-vertical"><pre class="worksheet-math-vertical" dir="ltr">${escapeWorksheetHtml(stripMathLtrMarkers(q.verticalLayoutLtr))}</pre></div></div>`
        : layoutClass === "layout-compact-2" && q.subject === "math"
          ...
          : "";
      const wordBody = q.wordProblemBodyHe
      ...
      const diagram = diagramSvg
        ...
        : "";
      return `
      <article class="worksheet-question ${layoutClass}" data-index="${q.displayIndex}">
...
${header}
<main class="worksheet-body${bodyGridClass ? ` ${bodyGridClass}` : ""}">${body}</main>
</body>
</html>`;
}
