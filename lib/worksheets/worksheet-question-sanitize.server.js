 ...
 * @param {Record<string, unknown>} raw
 * @param {{ displayIndex: number, subject: import("./worksheet-question-types.js").WorksheetSubjectId, mathPracticeFormat?: string | null, gradeKey?: string, topicKey?: string }} ctx
...
export function toPrintableWorksheetQuestion(raw, ctx) {
  ...
  const optionsHe = extractOptionsHe(sanitized);
  ...
  if (ctx.subject === "math") {
    return enrichMathPrintableQuestion(sanitized, base, {
      ...
      gradeKey: ctx.gradeKey || String(sanitized.gradeLevel || ""),
      topicKey: ctx.topicKey || String(sanitized.operation || sanitized.topic || ""),
    });
  }
