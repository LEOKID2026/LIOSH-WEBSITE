...
${dimMeasure(clamp(m20.x - 14, 28, 172), clamp(m20.y, 22, 148), spec.side3, st)}`;
  ...
  // —— Shape template (identify triangles / quads) ——
...
${dimLabel(tx + tileScaled / 2, ty - 4, fmt(tileSide), st)}`;
    ...
    } else if (tile === "rectangle") {
      points = shapeTemplatePointsString("rectangle", { x: cx, y: cy });
    ...
    // Small cluster of 3–4 tiles to show pattern (squares).
    if (tile === "square" || tile === "rectangle") {
      ...
      /** @type {string[]} */
      const tiles = [];
      const maxTiles = tile === "rectangle" ? 4 : 4;
      let n = 0;
      for (let r = 0; r < 2 && n < maxTiles; r += 1) {
        for (let c = 0; c < 2 && n < maxTiles; c += 1) {
...
${angleLabel}`;
  ...
  // —— Solid box (plane-adjacent, keep existing) ——
  if (kind === "solid_box" && spec.length != null && spec.width != null && spec.height != null) {
    ...
    const d = Math.min(32, w * 0.28);
    const x = ORIGIN_X - (w + d) / 2;
    const y = ORIGIN_Y - h - 8;
...
${dimMeasure(x + w / 2, y + h + 14, spec.length, st)}
${dimMeasure(x - 12, y + h / 2, spec.height, st)}
${dimMeasure(x + w + d / 2 + 10, y + h / 2 - d / 2, spec.width, st)}`;
  }

  if (kind === "solid_identify") {
    // Prefer shape-only silhouette without Hebrew name (no answer spoiler).
    const solid = String(spec.solidShape || "");
    if (solid === "cube" || solid === "rectangular_prism") {
      return `<polygon points="70,60 130,60 150,40 90,40" fill="${st.fill}" stroke="${st.stroke}" stroke-width="2"/>
<polygon points="130,60 130,110 150,90 150,40" fill="none" stroke="${st.stroke}" stroke-width="1.5"/>
<polygon points="70,60 70,110 130,110 130,60" fill="none" stroke="${st.stroke}" stroke-width="1.5"/>`;
    }
