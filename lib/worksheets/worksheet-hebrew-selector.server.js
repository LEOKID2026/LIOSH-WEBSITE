...
function hebrewWorksheetItemFromGenerated(q, topicKey, gradeKey) {
  ...
  return {
    ...
    gradeLevel: gradeKey,
    writingSpaceLines: isOpen ? 5 : undefined,
    params: {
      answerMode,
      gradeKey,
...
export function selectHebrewWorksheetQuestions(params) {
  ...
  return withSeededRandom(useSeed, () => {
    ...
    for (let attempt = 0; attempt < maxAttempts && questions.length < n; attempt += 1) {
      ...
      const item = hebrewWorksheetItemFromGenerated(q, topicKey, gradeKey);
      if (!item) continue;
      ...
      const fp = `${printable.stemHe}|${item.correctAnswer}|${item.params?.answerMode || ""}`;
