...
/** Class names created by teacher-portal smoke scripts (exact or prefix). */
...
export function isSmokeClassName(name) {
  ...
  if (/\bSmoke Class\b/i.test(n)) return true;
...
export function isSmokeStudentName(name) {
  const n = String(name || "").trim();
  if (!n) return false;
  if (/\bSmoke\b/i.test(n)) return true;
 ...
 * @param {{ studentId: string, studentFullName?: string|null }[]} students
 */
export function partitionSmokeDashboardRows(classes, students) {
  ...
  const visibleStudents = (students || []).filter((s) => {
    if (isSmokeStudentName(s.studentFullName)) return false;
    return true;
  });
