...
export function buildEnrichedSummaryKV(payload) {
  ...
  const rows = [
    ...
    ["תת-נושא", subtopicHe],
    ["רמת קושי", activityExportDifficultyLabelHe(act.difficultyLevel)],
    ...
    ["", ""],
    ["מספר תלמידים ברשימה", Number(summary.rosterCount) || 0],
...
// ---------------------------------------------------------------------------
// Sheet 4 — תשובות תלמידים (Student Answers — Long Format)
// ---------------------------------------------------------------------------
...
export function buildEnrichedRecommendationsKV(payload) {
  ...
  const rows = [
    ["אחוז השלמה (%)", summary.completionRate != null ? Number(summary.completionRate) : ""],
    ["תלמידים שהגישו", Number(summary.submittedCount) || 0],
    ["תלמידים שלא סיימו", notFinished],
    ["", ""],
  ];
...
export function buildEnrichedActivityReportWorkbook(payload) {
  ...
  const summaryKV = buildEnrichedSummaryKV(payload);
  ...
  const questionsSheet = XLSX.utils.aoa_to_sheet([qHeaders, ...qRows]);
  ...
  const studentRows = buildEnrichedStudentSummaryRows(students, questionCount);
  ...
  XLSX.utils.book_append_sheet(workbook, studentSheet, "סיכום תלמידים");

  // ── Sheet 4: תשובות תלמידים ───────────────────────────────────────────────
  const answerRows = buildEnrichedStudentAnswersRows(students, questions, responses, questionCount);
  ...
  XLSX.utils.book_append_sheet(workbook, answerSheet, "תשובות תלמידים");

  // ── Sheet 5: ניתוח שאלות ──────────────────────────────────────────────────
  const qAnalyticsRows = buildEnrichedQuestionAnalyticsRows(perQuestion, questions);
  ...
  const recKV = buildEnrichedRecommendationsKV(payload);
