...
export function createGameAudioManager(manifestById, getSettings, onSettingsChange) {
  ...
  function applyMusicVolume() {
    if (!musicEl) return;
    const s = settings();
    const base = s.musicEnabled ? s.musicVolume : 0;
  ...
  function resumeMusic() {
    if (musicEl && musicPaused && canPlayMusic()) {
  ...
  async function ensureHebrewStream(text) {
    ...
    try {
      const r = await fetch("/api/hebrew-audio-ensure", {
        ...
        body: JSON.stringify({ narration_plaintext }),
      });
      if (!r.ok) return null;
    } catch {
      return null;
  ...
  function playBrowserTts(text, locale, gen) {
    return new Promise((resolve, reject) => {
      ...
      const voice = lang.toLowerCase().startsWith("he")
        ? pickHebrewTtsVoice(synth)
        : pickEnglishTtsVoice(synth);
  ...
  async function playVoice(assetId, payload = {}, options = {}) {
    ...
    const text = String(payload.text || payload.narration_plaintext || "").trim();
