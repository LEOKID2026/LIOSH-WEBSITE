...
/**
 * Atomically consume one pending box (compare-and-swap on pending_box_count).
...
export async function openSurpriseBox(supabase, studentId, idempotencyKey) {
  ...
  const pickedCards = cardsWanted > 0 ? await pickBoxCards(supabase, preventDup, cardsWanted) : [];
  ...
  if (coinsRewardTotal <= 0 && diamondsRewardTotal <= 0 && cardsGranted <= 0) {
    return failOpenAfterClaim(supabase, studentId, claim, "no_rewards_available");
  }
  ...
  for (let i = 0; i < coinAmounts.length; i += 1) {
    ...
    if (!coinResult.ok) {
      return failOpenAfterClaim(supabase, studentId, claim, coinResult.code || "coin_failed");
    }
  ...
  if (diamondsRewardTotal > 0) {
    ...
    if (!diamondResult.ok && !diamondResult.skipped) {
      return failOpenAfterClaim(supabase, studentId, claim, diamondResult.code || "diamond_failed");
    }
  ...
  for (const card of pickedCards) {
    ...
    if (!grant.ok) {
      return failOpenAfterClaim(supabase, studentId, claim, grant.code || "card_grant_failed", {
  ...
  const { data: opening, error: openErr } = await supabase
    ...
    .single();
  if (openErr) {
    return failOpenAfterClaim(supabase, studentId, claim, "opening_log_failed", {
