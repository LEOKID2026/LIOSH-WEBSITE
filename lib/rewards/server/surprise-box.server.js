...
import { getCardSetting, getDuplicateThreshold } from "./reward-settings.server.js";
import {
...
export async function ensureSurpriseBoxState(supabase, studentId) {
  ...
  const general = parseSurpriseBoxGeneralSettings(generalRaw);
...
export async function getSurpriseBoxStatus(supabase, studentId) {
  ...
  const general = parseSurpriseBoxGeneralSettings(
    await getCardSetting(supabase, "surprise_box_general_settings")
  );
