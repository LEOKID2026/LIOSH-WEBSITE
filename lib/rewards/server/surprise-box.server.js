...
export async function getSurpriseBoxStatus(supabase, studentId) {
  ...
  if (!state) {
    return { ready: false, secondsRemaining: null, pendingBoxCount: 0, maxPendingBoxes: 1 };
  }
  ...
  const maxPendingBoxes = general.max_pending_boxes;
  let secondsRemaining = null;
  if (count < maxPendingBoxes && state.next_available_at) {
