...
async function getStudentContext(supabase, studentId) {
  ...
  try {
    // Prefer the shared 45s derived cache (same numbers as computeStudentLearningDerived).
    const { loadStudentLearningDerivedCached } = await import(
      "../../learning-supabase/student-home-profile-load.server.js"
    );
...
export async function evaluateAndGrantAcquisitionCards(supabase, studentId) {
  ...
  // One batched progress load for the whole grant pass (same cache path as card catalog).
  // Falls back to per-rule queries inside cardRulesAllMatch if cache build fails.
  let progressCache = null;
  try {
    progressCache = await buildStudentRuleProgressCache(supabase, studentId, byCard, ctx);
  } catch {
    progressCache = null;
  }
  ...
  for (const [cardId, cardRules] of byCard) {
    ...
    const { matches } = await cardRulesAllMatch(
      supabase,
      studentId,
      cardRules,
      ctx,
