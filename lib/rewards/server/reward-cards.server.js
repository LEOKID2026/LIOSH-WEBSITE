...
/** Demo read-only shop catalog — same cards/prices as registered student, no ownership writes. */
export async function getDemoCardsShopView(supabase, gradeLevel, coinBalance = 150) {
 ...
 * @param {object[]} cards
 */
function mapDemoCollectionEntries(cards) {
 ...
 * @param {string} studentGradeBand
 */
function loadDemoLockMetaContext(allCards, rulesByCard, ownedIds, studentGradeBand) {
  const ctx = { gradeBand: studentGradeBand, monthlyMinutes: 0 };
...
/** Demo collection — two real active cards for the selected grade (read-only, no DB writes). */
export async function getDemoCardsCollectionView(supabase, gradeLevel) {
  const studentGradeBand = getGradeBand(gradeLevel);
  const allCards = await fetchActiveCardsWithSeries(supabase);
  const picks = pickDemoCollectionCards(allCards, studentGradeBand);
  return { collection: mapDemoCollectionEntries(picks) };
}

/** Demo catalog — all real active cards visible for grade (read-only). */
export async function getDemoCardsCatalogView(supabase, gradeLevel) {
  const studentGradeBand = getGradeBand(gradeLevel);
      ...
      .map((card) => {
        ...
        if (isOwned) {
          ...
          return {
            ...mapped,
            isOwned: true,
            duplicateCount: isAchievement ? 0 : 0,
            canConvert: false,
          };
        }
...
export async function getDemoCardsSeriesView(supabase, gradeLevel) {
  const studentGradeBand = getGradeBand(gradeLevel);
