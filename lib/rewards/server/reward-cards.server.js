...
export async function getStudentCardsSummary(supabase, studentId) {
  ...
  const [coinBalance, ownedRows, shopCardsRes, seriesRows] = await Promise.all([
    ...
    supabase.from("student_reward_cards").select("card_id, owned").eq("student_id", studentId),
    supabase
      .from("reward_cards")
      .select("id, card_type, can_be_purchased, grade_bands, starts_at, ends_at, is_active")
      .eq("is_active", true),
    supabase.from("reward_card_series").select("id").eq("is_active", true),
  ]);
  ...
  const ownedIds = new Set(
    (ownedRows.data || []).filter((r) => r.owned).map((r) => r.card_id)
  );
