...
function mapCardForChild(card, seriesName) {
  return {
    ...
    imageUrl: card.image_url || "/rewards/cards/placeholders/regular/default.svg",
...
export async function getStudentCardsView(supabase, studentId) {
  ...
  for (const card of allCards) {
    if (card.can_be_purchased && card.card_type === "shop" && !ownedIds.has(card.id)) {
  ...
  for (const series of seriesRows.data || []) {
    const inSeries = allCards.filter((c) => c.series_id === series.id);
    const ownedInSeries = inSeries.filter((c) => ownedIds.has(c.id)).length;
    seriesProgress.push({
      seriesId: series.id,
      nameHe: series.name_he,
      ownedCount: ownedInSeries,
      totalCount: inSeries.length,
    });
  }

  return { collection, locked, shop, seriesProgress };
}
