...
export async function generateActivityQuestionSetClient({
  ...
  if (sub === "history") {
    ...
    const filtered = pool.filter((q) => {
      ...
      const lvl = String(q.minLevel || q.assignedLevel || "easy");
      if (levelKey === "easy") return lvl === "easy";
      if (levelKey === "medium") return lvl === "easy" || lvl === "medium";
      return true;
    });

    if (!filtered.length) {
      throw new Error(
        notEnoughQuestionsMessage("היסטוריה", gradeKey, topicKey, levelKey, topicKey)
      );
    }
    ...
    for (const q of shuffled) {
      ...
      if (!fp || fp === "|" || seen.has(fp)) continue;
      const item = frozenHistoryItemFromBankRow(q, topicKey, gradeKey, levelKey);
      if (!item) continue;
      seen.add(fp);
      questions.push(item);
      if (questions.length >= n) break;
