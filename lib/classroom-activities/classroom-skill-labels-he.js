...
const RAW_INTERNAL_KEY_RE =
  /^(?:geo|sci|math|hebrew|eng|mg|moledet)_[a-z0-9_]+$/i;
 ...
 * @param {{ subject?: string|null }} [options]
 * @returns {string}
 */
export function resolveClassroomSkillLabelHe(skillKey, options = {}) {
  ...
  if (GEO_DIAGNOSTIC_SKILL_LABEL_HE[key]) return GEO_DIAGNOSTIC_SKILL_LABEL_HE[key];
 ...
 */
export function decorateWeakSkillsForTeacherDisplay(weakSkills, subject) {
  return (weakSkills || []).map((row) => {
    const skillKey = String(row.skillKey || "general");
    const skillLabelHe = resolveClassroomSkillLabelHe(skillKey, { subject });
    return {
      ...row,
      skillKey,
      skillLabelHe,
