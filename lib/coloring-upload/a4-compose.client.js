 ...
 * Single upscale draw, centered on page.
 * @param {ImageData} lineArt
 */
export async function composeUploadA4Page(lineArt) {
  ...
  const { offsetX, offsetY } = getVisualCenterDrawOffset(lineArt, drawWidth, drawHeight);
 ...
 * @param {ImageData} lineArt
 */
export async function buildUploadFinalA4(lineArt) {
  const { canvas, orientation } = await composeUploadA4Page(lineArt);
