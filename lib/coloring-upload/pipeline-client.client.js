...
export function getPipelineErrorMessageHe(err) {
  ...
  if (err instanceof Error) {
    ...
    if (err.message === "STYLE_TRANSFER_TIMEOUT") return WORKSHEET_UI_HE.coloringUploadStyleTransferTimeout;
    if (err.message === "STYLE_TRANSFER_FAILED") return WORKSHEET_UI_HE.coloringUploadStyleTransferFailed;
    if (err.message === "replicate_not_configured" || err.message === "replicate_unavailable") {
      return WORKSHEET_UI_HE.coloringUploadStyleTransferUnavailable;
    }
    if (err.message === "replicate_rate_limited") return WORKSHEET_UI_HE.coloringUploadStyleTransferRateLimited;
    if (err.message === "upstream_failed") return WORKSHEET_UI_HE.coloringUploadStyleTransferFailed;
  }
