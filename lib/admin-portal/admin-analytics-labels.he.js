...
export function formatAnalyticsPersonaHe(persona) {
  ...
  return personaLabelHe(key) !== "—" ? personaLabelHe(key) : ANALYTICS_ROLE_LABELS_HE[key] || "לא ידוע";
}
...
export function formatAnalyticsGradeHe(grade) {
  ...
  if (!key || key === "unknown") return "לא ידוע";
  return ANALYTICS_GRADE_LABELS_HE[key] || adminGradeLabelHe(key) || grade || "—";
}
...
function formatAnalyticsCoinReasonHe(reason) {
  ...
  if (!key || key === "unknown") return "לא ידוע";
  return ANALYTICS_COIN_REASON_HE[key] || reason;
}
...
function formatAnalyticsTokenHe(token) {
  ...
  const subject = subjectLabelHe(key);
  if (subject !== "—") return subject;

  const persona = formatAnalyticsPersonaHe(key);
  if (persona !== "לא ידוע" && key !== "unknown") return persona;
  ...
  if (key.includes(":")) {
    ...
    return `${formatAnalyticsTokenHe(left)} · ${formatAnalyticsTokenHe(right)}`;
  }
...
export function formatAnalyticsLabelHe(label) {
  ...
  if (raw.includes(" · ")) {
    return raw
      .split(" · ")
      .map((part) => formatAnalyticsTokenHe(part.trim()))
      .join(" · ");
  }

  const mapped = formatAnalyticsTokenHe(raw);
  if (mapped !== raw.replace(/_/g, " ") || looksHebrew(mapped)) return mapped;

  return mapped
    .replace(/\bAuth\b/gi, "אימות")
    .replace(/\bDB\b/g, "מאגר נתונים")
    .replace(/\bpersona\b/gi, "פרסונה")
    .replace(/\bprivate_teacher\b/gi, "מורה פרטי")
    .replace(/\breward_earned\b/gi, "קבלת פרס")
    .replace(/\badmin\b/gi, "מנהל")
    .replace(/\bparent\b/gi, "הורה")
    .replace(/\bstudent\b/gi, "ילד")
    .replace(/\bteacher\b/gi, "מורה")
    .replace(/\banalytics_events\b/gi, "אירועי אנליטיקה")
    .replace(/\bPhase 1\b/gi, "שלב 1")
    .replace(/\bunknown\b/gi, "לא ידוע")
    .replace(/\bunlinked\b/gi, "לא משויך");
}
