...
import { buildClassTeacherGuidance } from "./teacher-recommendations.server.js";
import {
  loadStudentRowForTeacherReport,
} from "./teacher-report.server.js";
...
/** Max parallel student report builds per class (DB + aggregation). */
const CLASS_REPORT_STUDENT_CONCURRENCY = 6;
 ...
 */
export function aggregateClassReportFromStudentPayloads(studentPayloads) {
...
/**
 * Build analytics payload for one class member (class report only — no account attach).
 * @param {{
 ...
 * }} input
 */
async function buildClassReportStudentEntry(input) {
  ...
  const fullName = loaded.student.full_name || "";
  const maskedName = maskStudentFullName(fullName);

  return {
    studentId: member.studentId,
    studentFullName: fullName,
    studentFullNameMasked: maskedName,
...
export async function buildTeacherClassReportPayload(input) {
  ...
  const entries = await mapWithConcurrency(
    members.members,
    CLASS_REPORT_STUDENT_CONCURRENCY,
    (member) => buildClassReportStudentEntry({ serviceRole, member, fromDate, toDate })
  );

  const studentPayloads = entries.filter(Boolean).map((entry) => {
