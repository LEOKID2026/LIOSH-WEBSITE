...
import {
  ...
  sanitizeReportPayloadForTeacher,
} from "./teacher-report.server.js";
import { loadGuardianAccessSummary, maskStudentFullName } from "./teacher-students.server.js";
...
export async function buildTeacherClassReportPayload(input) {
  const { serviceRole, teacherId, classId, fromDate, toDate } = input;
  ...
  for (const member of members.members) {
    ...
    const guardianAccessSummary = await loadGuardianAccessSummary(
      serviceRole,
      teacherId,
      member.studentId
    );
    const payload = sanitizeReportPayloadForTeacher(withAccount, guardianAccessSummary);
    const maskedName = maskStudentFullName(loaded.student.full_name);
    ...
    studentSummaries.push({
      ...
      guardianAccessSummary: payload.guardianAccessSummary || null,
    });
  }
  ...
  await writeTeacherViewClassReportAuditIfNeeded(serviceRole, teacherId, classId);
