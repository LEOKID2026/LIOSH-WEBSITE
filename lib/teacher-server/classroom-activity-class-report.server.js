...
const STATUS_SELECT_COLS =
  "activity_id, student_id, status, submitted_at, answers_count, correct_count";

/**
 * PostgREST encodes `.in()` filters in the URL; large activity lists overflow headers (~16KB).
 * @param {import("@supabase/supabase-js").SupabaseClient} serviceRole
 * @param {string[]} activityIds
 * @param {string[]} studentIds
 */
async function fetchClassroomActivityStatusRows(serviceRole, activityIds, studentIds) {
  const statuses = [];
  if (!activityIds.length || !studentIds.length) return statuses;

  for (const activityIdChunk of chunkIds(activityIds, 40)) {
...
function ensureTopicRollup(subjectRollup, topicKey) {
  if (!subjectRollup.topics[topicKey]) {
    subjectRollup.topics[topicKey] = { answers: 0, correct: 0, wrong: 0, lastActivityAt: null };
  }
...
export function buildClassroomActivityRollupsByStudentId(input) {
  ...
  for (const statusRow of statuses || []) {
    ...
    if (hasRangeFilter) {
      ...
      if (!(referenceIso >= fromIso && referenceIso < toIsoExclusive)) continue;
    }
...
export async function loadClassroomActivityRollupsForClassReport(input) {
  ...
  try {
    ...
    let statuses;
    try {
      statuses = await fetchClassroomActivityStatusRows(serviceRole, activityIds, studentIds);
    } catch (error) {
...
export async function loadClassroomActivityRollupsForMultipleClassReports(input) {
  ...
  try {
    ...
    for (const classIdChunk of chunkIds(classIds, 40)) {
      const { data: activities, error: actErr } = await serviceRole
        .from("classroom_activities")
        .select("id, class_id, subject, topic, closed_at, activated_at, created_at, status")
      ...
      scopedActivities.push(...(activities || []));
    }
...
async function loadStudentClassroomRollupForMemberClassIds(
  ...
  for (const activityIdChunk of chunkIds(activityIds, 80)) {
    for (let offset = 0; ; offset += BATCH_PAGE) {
      const { data, error } = await serviceRole
        .from("classroom_activity_student_status")
