...
import { getCachedLightweightStudentActivityMap } from "../school-server/school-browse-activity-cache.server.js";
import {
 ...
 * @param {Awaited<ReturnType<typeof getCachedLightweightStudentActivityMap>>|null} activity
 */
function mapDashboardStudents(ctx, activity) {
...
export async function buildTeacherDashboardShellPayload(input) {
  ...
  const students = mapDashboardStudents(ctx, null);
...
export async function buildTeacherDashboardActivityPayload(input) {
  ...
  const activity = await getCachedLightweightStudentActivityMap({
    ...
    studentIds: ctx.activityStudentIds.length ? ctx.activityStudentIds : ctx.studentIds,
