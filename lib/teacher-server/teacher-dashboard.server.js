...
import { resolveTopicLabelHe } from "./teacher-guidance-v2.server.js";
import {
 ...
 */
function buildTeacherAttentionSignals(students, byStudentWeakTopic) {
  const candidates = [];
  for (const s of students) {
    const answers = Number(s.totalAnswers) || 0;
    const acc = s.accuracy != null ? Number(s.accuracy) : null;
    if (answers < 5 || acc == null || acc >= 60) continue;
    ...
    candidates.push({
      ...
      riskLevel,
      topWeakSubject: wt?.subject || null,
      topWeakTopic: wt?.topic || null,
      topWeakTopicLabelHe: wt ? resolveTopicLabelHe(wt.subject, wt.topic) : null,
      accuracyPct: acc,
