...
export function parseCreateActivityBody(body) {
  ...
  if (!isUuid(classId)) {
    return { ok: false, code: "validation_failed", message: "classId must be a UUID" };
  }
  ...
  if (!isActivityPreviewSubjectSupported(subject)) {
    return {
      ...
      message: "subject not supported for classroom activity preview",
    };
  }
  ...
  if (!mode) {
    return { ok: false, code: "validation_failed", message: "invalid mode" };
  }
 ...
 */
export async function createClassroomActivity(serviceRole, teacherId, parsed) {
  const owned = await loadTeacherClassOwned(serviceRole, teacherId, parsed.payload.classId);
  if (!owned.ok) {
    return owned;
  }
  ...
  const { data, error } = await serviceRole
    ...
    .insert({
      ...
      status: "draft",
      question_set: jsonSafeCloneForStorage(parsed.payload.questionSet),
    })
    .select("id")
    .maybeSingle();
 ...
 * @param {string} activityId
 */
async function seedStudentStatusRowsForClass(serviceRole, activityId, classId) {
 ...
 * @param {{ questionSet: unknown[], attempts: Array<Record<string, unknown>>, questionCount: number }} input
 */
export function mapTeacherActivityStudentAnswerDetail({ questionSet, attempts, questionCount }) {
  const qSet = Array.isArray(questionSet) ? questionSet : [];
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const att = attemptByIdx.get(i);
    const snap =
      att?.question_snapshot && typeof att.question_snapshot === "object"
        ? att.question_snapshot
        : frozen;

    const question = String(snap.question || snap.prompt || frozen.question || frozen.prompt || "")
      .trim();
    const choicesRaw = Array.isArray(snap.choices)
      ? snap.choices
      : Array.isArray(snap.answers)
        ? snap.answers
        : Array.isArray(frozen.choices)
          ? frozen.choices
          : Array.isArray(frozen.answers)
            ? frozen.answers
            : null;
    const choices = choicesRaw ? choicesRaw.map((c) => String(c)) : undefined;

    const correctFromAttempt =
      att?.correct_answer != null ? String(att.correct_answer).trim() : "";
    const correctAnswer =
      correctFromAttempt || extractCorrectAnswerFromQuestion(frozen) || null;
    ...
    questions.push({
      ...
      subject: snap.subject != null ? String(snap.subject) : frozen.subject,
      topic: snap.topic != null ? String(snap.topic) : frozen.topic,
      params: snap.params && typeof snap.params === "object" ? snap.params : frozen.params,
      shape: snap.shape != null ? snap.shape : frozen.shape,
    });
  }
...
export async function buildActivityStudentAnswersPayload(
  ...
  const { data: attempts, error: attemptsErr } = await serviceRole
    .from("classroom_activity_attempts")
    .select(
      "question_index, selected_answer, correct_answer, is_correct, answered_at, question_snapshot"
    )
    .eq("activity_id", activityId)
    .eq("student_id", studentId)
...
export async function listStudentActivities(serviceRole, studentId) {
  ...
  const { data: activities, error } = await serviceRole
    .from("classroom_activities")
    .select(
      "id, title, mode, question_count, time_limit_seconds, status, class_id, due_at, current_question_idx"
    )
  ...
  const visible = (activities || []).filter((a) => {
    ...
    if (a.status === "closed") {
      ...
      return st && (st.status === "submitted" || st.status === "timed_out" || Number(st.answers_count) > 0);
    }
    return false;
  });
...
export async function startStudentActivity(serviceRole, studentId, activityId) {
  ...
  const questionSet = stripQuestionSetForStudent(row.question_set, row.mode);
 ...
 * @param {string} studentId
 * @param {string} activityId
 * @param {{ questionIndex: number, selectedAnswer: string, timeSpentMs?: number|null, hintsUsed?: number, explanationViewed?: boolean }} input
 */
export async function recordStudentActivityAnswer(serviceRole, studentId, activityId, input) {
  ...
  if (!Number.isFinite(questionIndex) || questionIndex < 0 || questionIndex >= row.question_count) {
    return { ok: false, status: 400, code: "validation_failed", message: "invalid question_index" };
  }
  ...
  const qSet = Array.isArray(row.question_set) ? row.question_set : [];
  const question = qSet[questionIndex];
...
export async function submitStudentActivity(serviceRole, studentId, activityId) {
  ...
  return {
    ...
    questionCount: row.question_count,
    revealAnswers: shouldRevealCorrectAnswerToStudent(row.mode, { submitted: true }),
  };
}
