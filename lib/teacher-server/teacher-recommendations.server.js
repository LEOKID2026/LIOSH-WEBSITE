...
/**
 * @param {number|null|undefined} accuracyPct
 * @returns {"critical_class"|"class_needs_reinforcement"|"class_monitor"|"class_on_track"}
 */
export function deriveClassGuidanceSeverityTier(accuracyPct) {
  const acc = Number(accuracyPct);
  if (!Number.isFinite(acc)) return "class_monitor";
...
/**
 * @param {"critical_class"|"class_needs_reinforcement"|"class_monitor"|"class_on_track"} tier
 */
...
export function buildClassTeacherGuidance(classPayload) {
  ...
  const classHealthSignal =
    cohortAccuracy === null ? "no_data" : mapClassHealthSignalFromTier(guidanceSeverityTier);
