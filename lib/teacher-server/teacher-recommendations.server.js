...
const LOW_ACCURACY_THRESHOLD = 60;       // % — below this is "struggling"
const STRENGTH_THRESHOLD = 80;           // % — above this is "strong"
const MIN_ANSWERS_FOR_STUDENT_SIGNAL = 5;
const MIN_ANSWERS_FOR_TOPIC_SIGNAL = 3;
const MIN_CLASS_ANSWERS_FOR_GUIDANCE = 10;
const INACTIVITY_DAYS_THRESHOLD = 7;     // days without any activity
const STRUGGLING_ACCURACY_CUTOFF = 55;   // for suggestedGroups
const ADVANCED_ACCURACY_CUTOFF = 80;     // for suggestedGroups
const MIN_STUDENT_ANSWERS_FOR_GROUP = 3; // min answers to assign group
...
function rankWeakTopics(subjects, minAnswers = MIN_ANSWERS_FOR_TOPIC_SIGNAL) {
  ...
  for (const subject of REPORT_AGG_SUBJECTS) {
    ...
    for (const [topicKey, topicData] of Object.entries(subj.topics || {})) {
      ...
      const accuracy = safeNum(topicData.accuracy);
      if (answers >= minAnswers && accuracy < LOW_ACCURACY_THRESHOLD) {
        weak.push({
          subject,
          topic: topicKey,
          accuracy,
          wrong: safeNum(topicData.wrong),
          answers,
        });
      }
    }
...
export function buildStudentTeacherGuidance(sanitizedPayload) {
  ...
  const accuracy = totalAnswers > 0 ? safeNum(summary?.accuracy) : null;
  ...
  if (nextPracticeFocus.length === 0 && accuracy !== null && accuracy < LOW_ACCURACY_THRESHOLD) {
    for (const subject of REPORT_AGG_SUBJECTS) {
      const subj = subjects?.[subject];
      if (!subj || typeof subj !== "object") continue;
      const subjAnswers = safeNum(subj.answers);
      const subjAcc = safeNum(subj.accuracy);
      if (subjAnswers >= MIN_ANSWERS_FOR_TOPIC_SIGNAL && subjAcc < LOW_ACCURACY_THRESHOLD) {
        nextPracticeFocus.push({
          subject,
          topic: null,
          accuracy: subjAcc,
          wrong: safeNum(subj.wrong),
          answers: subjAnswers,
...
function buildSuggestedGroups(students) {
  ...
  for (const s of students) {
    ...
    const answers = safeNum(summary?.totalAnswers);
    const accuracy = answers > 0 ? safeNum(summary?.accuracy) : null;
    ...
    if (accuracy < STRUGGLING_ACCURACY_CUTOFF) {
      item.groupReason = "low_accuracy";
      struggling.push(item);
    } else if (accuracy >= ADVANCED_ACCURACY_CUTOFF) {
...
export function buildClassTeacherGuidance(classPayload) {
  ...
  const cohortAccuracy = totalAnswers > 0 ? safeNum(cohortSummary?.accuracy) : null;
  ...
  const classHealthSignal =
    ...
    cohortAccuracy < STRUGGLING_ACCURACY_CUTOFF ? "needs_support" :
    cohortAccuracy >= ADVANCED_ACCURACY_CUTOFF ? "strong" :
    "progressing";
  ...
  if (nextLessonFocus.length === 0 && subjects) {
    for (const subject of REPORT_AGG_SUBJECTS) {
      ...
      const accuracy = safeNum(subj.accuracy);
      if (answers >= MIN_ANSWERS_FOR_TOPIC_SIGNAL && accuracy < LOW_ACCURACY_THRESHOLD) {
        nextLessonFocus.push({
          ...
          errorRate: answers > 0
            ? Number(((safeNum(subj.wrong) / answers) * 100).toFixed(1))
            : 0,
        });
