 ...
 * @param {import('@supabase/supabase-js').SupabaseClient} serviceRole
 */
export async function generateUniqueGuardianUsername(serviceRole) {
  for (let attempt = 0; attempt < 12; attempt += 1) {
    const username = generateGuardianLoginUsername();
...
export async function createTeacherGuardianAccess(input) {
  ...
  const usernameGen = await generateUniqueGuardianUsername(serviceRole);
