...
export async function resolveAuthenticatedTeacherUserId(authHeader) {
  ...
  const teacherPersonas = ["private_teacher", "school_teacher", "school_manager", "school_operator"];
  let matchedEntitlement = null;

  for (const persona of teacherPersonas) {
    const check = await assertActivePersonaEntitlement(serviceRole, userData.user.id, persona);
  ...
  return { ok: true, teacherUserId: userData.user.id, entitlement: matchedEntitlement };
}
