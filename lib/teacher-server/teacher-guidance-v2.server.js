...
function resolveTaxonomyId(subject, topicKey, wrongEvents) {
  ...
  for (const id of candidateIds) {
    ...
    if (!row) continue;
    if (passesEvidenceRecurrenceRules(wrongEvents, row)) return id;
  }
  return null;
}
...
function recurrenceSignalFrom(wrongEvents, taxonomyId, subject, topicKey) {
  ...
  if (taxonomyId) {
    ...
    if (row && passesEvidenceRecurrenceRules(wrongEvents, row)) {
...
export function buildClassTeacherGuidanceV2(classPayload, opts = {}) {
  ...
  for (const wt of weaknessTopics) {
    ...
    let taxonomyId = null;
    const cohortWrongEvents = Array.isArray(wt.wrongEvents) ? wt.wrongEvents.filter((e) => !e.isCorrect) : [];
    for (const id of candidateIds) {
      const row = TAXONOMY_BY_ID[id];
      if (row && cohortWrongEvents.length > 0 && passesEvidenceRecurrenceRules(cohortWrongEvents, row)) {
        taxonomyId = id;
        break;
      }
    }
