...
const INDIVIDUAL_MODES = new Set(["guided_practice", "quiz", "homework"]);
...
export function parseCreateStudentActivityBody(body) {
  ...
  if (!mode) {
    return { ok: false, code: "validation_failed", message: "invalid mode (guided_practice, quiz, homework)" };
  }
...
export async function createStudentActivity(serviceRole, teacherId, parsed) {
  ...
  const { data, error } = await serviceRole
    ...
    .insert({
      ...
      student_id: parsed.payload.studentId,
...
export async function buildStudentActivityReportPayload(serviceRole, teacherId, activityId) {
  ...
  const { data: attempts, error: attemptsErr } = await serviceRole
    ...
    .select(
      "question_index, selected_answer, correct_answer, is_correct, answered_at, question_snapshot, skill_key"
    )
    .eq("activity_id", activityId)
