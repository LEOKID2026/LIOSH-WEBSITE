...
import { addClassMember } from "./teacher-classes.server.js";
...
export async function createTeacherManagedStudent(input) {
  ...
  if ((countResult.count ?? 0) >= studentLimit) {
  ...
  const username = normalizeUsername(`leo-${crypto.randomBytes(4).toString("hex")}`);
