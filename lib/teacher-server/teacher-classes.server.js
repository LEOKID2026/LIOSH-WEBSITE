...
import { hasActiveTeacherStudentLink, maskStudentFullName } from "./teacher-students.server.js";
...
export async function createTeacherClass(serviceRole, teacherId, input) {
  ...
  const countResult = await countActiveTeacherClasses(serviceRole, teacherId);
  if (!countResult.ok) return countResult;
  if (countResult.count >= input.classLimit) {
 ...
 */
export async function addClassMember(serviceRole, teacherId, classId, studentId) {
  if (!isUuid(studentId)) {
    return { ok: false, status: 400, code: "validation_failed" };
  }
