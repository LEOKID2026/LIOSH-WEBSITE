...
function studentRowFromGroupItem(s) {
  ...
  const accuracyNum = s.accuracy != null ? safeNum(s.accuracy) : null;
  
  let statusBadge = "אין מספיק נתונים";
  if (answers > 0 && accuracyNum != null) {
    if (accuracyNum >= 90) statusBadge = "חזק";
    else if (accuracyNum >= 75) statusBadge = "תקין";
    else if (accuracyNum >= 65) statusBadge = "במעקב";
    else if (accuracyNum >= 50) statusBadge = "צריך חיזוק";
    else statusBadge = "דורש התערבות";
  } else if (answers > 0) {
    statusBadge = `${answers} תשובות`;
  }
...
function buildFocusAreasFromWeaknessSource(weaknessSource, guidance, rosterById, subjectLabelFn) {
  ...
  const focusAreas = deduped.map((t) => {
    ...
    focusDrilldowns[t.drilldownKey] = {
      title: t.label,
      subtitle: t.err
        ? `שיעור טעות ${t.err} · ${t.studentCount} תלמידים`
        : null,
...
function classInsightText(body) {
  ...
  if (guidance.version === "v2" && (guidance.classRecommendationUnits || []).length > 0) {
    ...
    if (top.headlineHe) {
      return `${base}. ${top.headlineHe} (${top.affectedStudentCount ?? 0} תלמידים · ${formatPercent(top.cohortAccuracyPct)} הצלחה).`;
    }
    if (top.topicLabelHe) {
...
export function parseClassReportViewModel(body, cls, extras = {}) {
  ...
  return {
    ...
    sections: {
      ...
      focus: {
        title: "נושאים לחיזוק",
        empty: canShowClassCalmWeakTopicsMessage(guidance, body)
          ? HUB_CLASS_WEAK_TOPICS_CALM_MESSAGE
          : CLASS_WEAK_TOPICS_FALLBACK_BANNER,
        items: focusAreas,
      },
...
function flattenPhysicalGuidanceFocusSource(subjectGuidanceBlocks, maxTotal = 10) {
  const out = [];
  for (const block of subjectGuidanceBlocks || []) {
    const units = (block.classRecommendationUnits || []).slice(0, 2);
...
export function parsePhysicalClassReportViewModel(body, ctx = {}) {
  ...
  const physicalGuidance = isPhysicalV2
    ? { version: "v2", guidanceSeverityTier: body.physicalClassGuidanceSeverityTier }
    : { version: "v1" };
  const physicalWeaknessSource = isPhysicalV2
    ? flattenPhysicalGuidanceFocusSource(body.subjectGuidanceBlocks, 10)
  ...
  return {
    ...
    sections: {
      ...
      focus: {
        title: "נושאים לחיזוק",
        empty: canShowClassCalmWeakTopicsMessage(
          isPhysicalV2
            ? {
                guidanceSeverityTier: body.physicalClassGuidanceSeverityTier,
                insufficientData: false,
              }
            : body?.teacherGuidanceBlock || {},
          body
        )
          ? HUB_CLASS_WEAK_TOPICS_CALM_MESSAGE
          : CLASS_WEAK_TOPICS_FALLBACK_BANNER,
