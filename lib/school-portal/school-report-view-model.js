...
function classInsightText(body) {
  ...
  if (guidance.version === "v2" && (guidance.classRecommendationUnits || []).length > 0) {
    ...
    const topic = top.topicLabelHe || "נושא לא מסווג";
    const base = health || "יש נתוני תרגול אחרונים";
    return `${base}. נושא עיקרי לחיזוק: ${topic} (${top.affectedStudentCount} תלמידים · ${formatPercent(top.cohortAccuracyPct)} הצלחה).`;
  }
...
function studentInsightText(body) {
  ...
  if (guidance.version === "v2" && (guidance.recommendationUnits || []).length > 0) {
    ...
    const parts = [
      `מומלץ מעקב ב${u.topicLabelHe || "נושא לא מסווג"} (${formatPercent(ev.accuracyPct)} הצלחה · ${ev.wrongCount ?? 0} טעויות)`,
    ];
...
export function parseClassReportViewModel(body, cls, extras = {}) {
  ...
  const weaknessSource =
    guidance.version === "v2" && (guidance.classRecommendationUnits || []).length > 0
  ...
  const focusAreas = weaknessSource.slice(0, 8).map((t) => {
    const line =
      t.topicLabelHe != null
        ? `${schoolSubjectLabelHe(t.subject) || ""}${t.topicLabelHe ? ` — ${t.topicLabelHe}` : ""}`.trim() ||
          schoolSubjectLabelHe(t.subject)
        : formatTopicLineHe(t.subject, t.topic) || schoolSubjectLabelHe(t.subject);
    const err =
    ...
    focusDrilldowns[drilldownKey] = {
      title: line,
      subtitle: err
        ? `שיעור טעות ${err} · ${safeNum(t.affectedStudentCount ?? t.studentCount)} תלמידים`
        : null,
      items: affectedStudents,
    };
    const studentCount =
      affectedStudents.length || safeNum(t.affectedStudentCount ?? t.studentCount);
    return {
      label: line,
      detail: err ? `שיעור טעות ${err}` : null,
      count: studentCount > 0 ? studentCount : null,
      drilldownKey,
    };
  });
...
export function parseStudentReportViewModel(body, student, ctx = {}) {
  ...
  const focusItems =
    guidance.version === "v2" && (guidance.recommendationUnits || []).length > 0
      ? (guidance.recommendationUnits || []).slice(0, 6).map((u) => {
      ...
      : (guidance.nextPracticeFocus || []).slice(0, 6).map((t) => ({
  ...
  const recommendationItems =
    guidance.version === "v2" && (guidance.supportSuggestionsV2 || []).length > 0
      ? (guidance.supportSuggestionsV2 || []).slice(0, 5).map((s) => {
          const action = actionTypeLabelHe(s.code);
          const topic = s.topicLabelHe || "נושא לא מסווג";
          return {
            label: action ? `${action} ב${topic}` : topic,
            detail: null,
          };
        })
...
function physicalClassInsightText(body) {
  ...
  if (totalAnswers < 10 && studentsWithActivity === 0) {
    return "אין עדיין מספיק נתונים לניתוח עומק בכל המקצועות. מומלץ לעקוב אחרי פעילויות כיתה והגשות תלמידים.";
  }
  if (totalAnswers > 0) {
    return `סיכום כללי לכיתה בכל המקצועות: ${studentsWithActivity} מתוך ${studentCount} תלמידים עם פעילות, ${totalAnswers} תשובות/הגשות.`;
  }
  return "טרם נאספו מספיק נתונים — מומלץ לפתוח פעילויות כיתה.";
}
...
function studentReportActionMeta(studentId) {
  return { id: "student_report", label: "דוח תלמיד", studentId };
}
...
export function parsePhysicalClassReportViewModel(body, ctx = {}) {
  ...
  const subjectItems = subjectBreakdown.map((row) => {
    ...
    return {
      ...
      detail: [
        ...
        `${safeNum(row.memberCount)} תלמידים`,
  ...
  const weaknessSource = body?.weaknessTopics || [];
  const rosterById = new Map(rosterStudentsRaw.map((s) => [s.studentId, s]));
  const focusDrilldowns = {};
  const focusAreas = weaknessSource.slice(0, 12).map((t) => {
    const line = formatTopicLineHe(t.subject, t.topic) || schoolSubjectLabelHe(t.subject);
    const subjectHe = schoolSubjectLabelHe(t.subject);
    const err =
      safeNum(t.answers) > 0
        ? formatPercent((safeNum(t.wrong) / safeNum(t.answers)) * 100)
        : null;
    ...
    const affectedStudents = affectedIds
      ...
      .filter(Boolean)
      .map((s) => ({
        ...studentRowFromPayload(s),
        actions: [studentReportActionMeta(s.studentId)],
      }));
    focusDrilldowns[drilldownKey] = {
      title: line,
      subtitle: err
        ? `${subjectHe} · שיעור טעות ${err} · ${safeNum(t.studentCount)} תלמידים`
        : `${subjectHe} · ${safeNum(t.studentCount)} תלמידים`,
      items: affectedStudents,
    };
    return {
      label: `${subjectHe}: ${line}`,
      detail: err ? `שיעור טעות ${err}` : null,
      count: affectedStudents.length || safeNum(t.studentCount),
      drilldownKey,
    };
  });
  ...
  const navigation = [
    ...
    },
    {
      id: "students",
      label: "תלמידים בכיתה",
      badge: rosterStudents.length ? `${rosterStudents.length} תלמידים` : "אין",
    },
    {
      id: "focus",
      label: "נושאים לחיזוק",
      badge: focusAreas.length ? `${focusAreas.length} נושאים` : "אין",
    },
    {
      id: "attention",
      label: "תלמידים שדורשים תשומת לב",
