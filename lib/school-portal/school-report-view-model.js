...
function classInsightText(body) {
  ...
  if (guidance.version === "v2" && (guidance.classRecommendationUnits || []).length > 0) {
    ...
    if (top.topicLabelHe) {
      return `${base}. נושא עיקרי לחיזוק: ${top.topicLabelHe} (${top.affectedStudentCount} תלמידים · ${formatPercent(top.cohortAccuracyPct)} הצלחה).`;
    }
    return `${base}. יש נתוני תרגול אחרונים — מומלץ להמשיך מעקב שוטף.`;
  }
