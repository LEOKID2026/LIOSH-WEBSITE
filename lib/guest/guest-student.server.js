...
export async function resumeGuestStudent(supabase, resumeToken) {
  ...
  if (!config.enabled) {
    return { ok: false, status: 403, code: "guest_mode_disabled", message: "מצb אורch כבוי כרגע" };
  }

  const tokenHash = hashStudentSecret(raw);
  const binding = await findActiveGuestBinding(supabase, tokenHash);
  if (!binding?.student_id) {
    return { ok: false, status: 401, code: "guest_resume_invalid", message: "לא ניתן לחדש את האורch במכשיר זה" };
  }
...
export async function startGuestStudent(supabase, options = {}) {
  ...
  if (existingToken) {
    const resumed = await resumeGuestStudent(supabase, existingToken);
    if (resumed.ok) return resumed;
    if (resumed.code !== "guest_resume_invalid" && resumed.code !== "guest_already_linked") {
      return resumed;
    }
  }
  ...
  if (!parent.ok) {
    return { ok: false, status: parent.status, code: parent.code, message: "מערכת אורch לא מוכנה" };
  }

  const leoNumber = await generateUniqueLeoNumber(supabase);
  const displayName = `אורch ${leoNumber}`;
  const nowIso = new Date().toISOString();
  ...
  if (studentErr || !student?.id) {
    if (isMissingGuestColumnError(studentErr)) {
      return { ok: false, status: 503, code: "db_schema_not_ready", message: "מערכת אורch לא מוכנה" };
    }
    return { ok: false, status: 500, code: "guest_create_failed", message: "לא ניתן ליצור אורch" };
  }
  ...
  if (bindErr) {
    return { ok: false, status: 500, code: "guest_binding_failed", message: "לא ניתן ליצור אורch" };
  }
