...
function rankSubjectsByAccuracy(subjects, minAnswers = MIN_SUBJECT_ANSWERS) {
  ...
  for (const key of REPORT_AGG_SUBJECTS) {
    ...
    rows.push({
      ...
      accuracy: safeNum(subj.accuracy),
      answers,
    });
  }
...
export function buildParentInsightsHe(payload) {
  ...
  const overallAccuracy = totalAnswers > 0 ? safeNum(summary.accuracy) : null;
  ...
  if (overallAccuracy != null && overallAccuracy < LOW_ACCURACY && totalAnswers >= MIN_SUBJECT_ANSWERS) {
...
export function buildParentFacingBlocks(payload) {
  ...
  const homeRecommendations = buildHomeRecommendationsHe(payload, insights);
...
export async function enrichPayloadWithParentFacing(serviceRole, payload, studentId) {
  ...
  return attachParentFacingToPayload(payload, { insights, homeRecommendations, teacherMessages });
}
