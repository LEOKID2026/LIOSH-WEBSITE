...
export function buildParentInsightsHe(payload) {
  ...
  if (totalAnswers === 0 && totalSessions === 0) {
    insights.push("אין מספיק תשובות אבחוניות בתקופה הזו כדי להציג מסקנה לימודית.");
  } else if (inactiveDays != null && inactiveDays >= INACTIVITY_DAYS) {
    insights.push(insufficientDataInsightHe());
  } else if (totalAnswers > 0 && totalAnswers < PARENT_EVIDENCE_VOLUME.STUDENT_REPORT_THIN_MAX) {
    insights.push(insufficientDataInsightHe());
  }
  ...
  if (allowStrongStudent && !softenStudentInsights) {
    ...
    if (totalAnswers > 0 && totalAnswers < PARENT_EVIDENCE_VOLUME.STUDENT_REPORT_THIN_MAX) {
      insights.push(insufficientDataInsightHe());
    }
  } else if (softenStudentInsights && totalAnswers > 0) {
    insights.push(insufficientDataInsightHe());
  }
  ...
  if (
    ...
    const topicName = String(topicLine || "").replace(/^[^\s]+\s*/, "").trim() || topicLine;
    const line = topicAttentionInsightHe({
      subject: subjectLabelHe(weakTopic.subject),
      subjectId: weakTopic.subject,
      topic: topicName,
      q: weakTopic.answers,
      acc: Math.round(weakTopic.accuracy),
      wrongRatio: Math.max(0, 100 - Math.round(weakTopic.accuracy)),
  ...
  if (!insights.length) {
    insights.push(noUrgentTopicInsightHe());
  }
