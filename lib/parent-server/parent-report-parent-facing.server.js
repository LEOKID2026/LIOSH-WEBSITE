...
function rankSubjectsByAccuracy(subjects, minAnswers = MIN_SUBJECT_ANSWERS) {
  ...
  for (const key of REPORT_AGG_SUBJECTS) {
    ...
    const answers = safeNum(subj.diagnosticAnswers);
...
function rankWeakTopics(subjects) {
  ...
  for (const subject of REPORT_AGG_SUBJECTS) {
    ...
    for (const [topicKey, topicData] of Object.entries(subj.topics)) {
      const answers = safeNum(topicData?.diagnosticAnswers);
...
export function buildParentInsightsHe(payload) {
  ...
  if (allowStrongStudent && !softenStudentInsights) {
    ...
    if (mistakeSubjects.length) {
      const ms = mistakeSubjects[0];
      if (subjectVisibleQuestions(subjectQuestionCounts, ms.subject) > 0) {
        const lab = subjectLabelHe(ms.subject);
    ...
    if (
      ...
      insights.push(`ב${weakest.label} נראה קושי יחסי לפי התרגולים — כדאי לעקוב אחרי עוד תרגול קצר.`);
    }
...
export function buildHomeRecommendationsHe(payload, insights = []) {
  ...
  if (insights.some((t) => t.includes("עברית")) && subjectVisibleQuestions(subjectQuestionCounts, "hebrew") > 0) {
