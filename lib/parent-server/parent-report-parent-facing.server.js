...
export function buildHomeRecommendationsHe(payload, insights = []) {
  ...
  // no LPD/ADC-backed action exists for this student in this range. Do NOT
  // recommend a specific subject/topic from rankSubjectsByAccuracy /
  // rankWeakTopics (raw accuracy-threshold only, no taxonomy/recurrence/
  // guided-exclusion/subskill-safety check) — that would be a home
  // recommendation with no real decision authority behind it. Always use
  // the neutral, non-directive fallback instead.
