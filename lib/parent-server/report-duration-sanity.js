...
export function applyReportDurationSanityToAggregateSubjects(subjects, opts = {}) {
  ...
  for (const subjectKey of Object.keys(subjects)) {
    ...
    for (const topicKey of Object.keys(s.topics || {})) {
      ...
      const topicAnswers = Math.max(0, Math.floor(Number(topic.diagnosticAnswers) || 0));
      ...
      for (const gradeKey of Object.keys(topic.byContentGrade || {})) {
        ...
        const sliceAnswers = Math.max(0, Math.floor(Number(slice.diagnosticAnswers) || 0));
    ...
    const subjectAnswers = Math.max(0, Math.floor(Number(s.diagnosticAnswers) || 0));
