...
/**
 * Independent copy of parentFacing so later topic-engine writes cannot mutate the API body.
 * Preserves shape/types; does not deep-clone arbitrary nested diagnostics.
 * @param {Record<string, unknown>} pf
 * @returns {Record<string, unknown>}
 */
export function cloneParentFacingBlock(pf) {
  if (!pf || typeof pf !== "object") return {};
  /** @type {Record<string, unknown>} */
  const out = { ...pf };
  if (Array.isArray(pf.insights)) out.insights = pf.insights.slice();
  if (Array.isArray(pf.homeRecommendations)) {
    out.homeRecommendations = pf.homeRecommendations.slice();
  }
  if (Array.isArray(pf.teacherMessages)) out.teacherMessages = pf.teacherMessages.slice();
  if (Array.isArray(pf.practiceFocus)) {
    out.practiceFocus = pf.practiceFocus.map((row) =>
      row && typeof row === "object" ? { ...row } : row,
    );
  }
  if (Array.isArray(pf.systemActions)) out.systemActions = pf.systemActions.slice();
  return out;
}
...
export function applyServerParentFacingAuthorityToClientReport(clientReport, apiPayload) {
  ...
  clientReport.parentFacing = cloneParentFacingBlock(serverPf);
