...
import { getIsraelInclusiveDateRangeUtcBounds } from "../learning-supabase/israel-calendar.server.js";
...
import {
  stripInternalKeysFromEvidenceRow,
  stripZeroEvidenceFromPublicReportPayload,
} from "./report-payload-public-sanitize.js";
import {
...
import { buildDiagnosticEvidenceRow, resolveMisconceptionEvidenceAtAggregateRead } from "../../utils/diagnostic-evidence.js";
import {
...
/**
 * Parent report DB filter bounds for an inclusive calendar date range.
 * Civil days are interpreted in Asia/Jerusalem (DST-aware), then converted to UTC ISO.
 * @param {Date} fromDate inclusive calendar day (YMD taken via isoDateOnly)
 * @param {Date} toDate inclusive calendar day
 * @returns {{ fromIso: string, toIsoExclusive: string, fromYmd: string, toYmd: string, timeZone: string }}
 */
export function resolveParentReportRangeUtcBounds(fromDate, toDate) {
  const fromYmd = isoDateOnly(fromDate);
  const toYmd = isoDateOnly(toDate);
  const bounds = getIsraelInclusiveDateRangeUtcBounds(fromYmd, toYmd);
  return {
    fromIso: bounds.fromIso,
    toIsoExclusive: bounds.toIsoExclusive,
...
/**
 * Full engine evidence stream (soft-capped). Do not require isDiagnosticEligible here:
 * DE2 applies independentRecurrenceOnly later. Gating only on eligibility dropped
 * tagged parent-report practice wrongs (e.g. learning_guided) so only the 20-row
 * recentMistakes sample could reach the report builder.
 * @param {{ isDiagnosticEligible?: boolean, evidenceCategory?: string }} classification
 * @param {string} resolvedMode
 */
...
/**
 * Pull write-time evidence fields from answer_payload / assigned snapshot.
 * Without these, historical reclassify needs params; tags already on the row
 * were previously dropped before DE2.
 * @param {Record<string, unknown>|null|undefined} payload
 */
function extractWriteTimeEvidenceFieldsFromPayload(payload) {
  ...
  const answerEvidence =
    (p.answerEvidence && typeof p.answerEvidence === "object" && !Array.isArray(p.answerEvidence)
      ? p.answerEvidence
      : null) ||
    (qe.answerEvidence && typeof qe.answerEvidence === "object" && !Array.isArray(qe.answerEvidence)
      ? qe.answerEvidence
      : null);
  const paramsCandidate =
    (p.params && typeof p.params === "object" && !Array.isArray(p.params) ? p.params : null) ||
    (qe.params && typeof qe.params === "object" && !Array.isArray(qe.params) ? qe.params : null);
  const misconceptionTag =
    safeString(p.misconceptionTag, 120) ||
    safeString(qe.misconceptionTag, 120) ||
    safeString(answerEvidence?.detectedMisconception, 120) ||
    undefined;
  const distractorFamily =
    safeString(p.distractorFamily, 120) || safeString(qe.distractorFamily, 120) || undefined;
  const classifierRuleId =
    safeString(p.classifierRuleId, 160) ||
    safeString(qe.classifierRuleId, 160) ||
    safeString(answerEvidence?.evidenceDetails?.classifierRuleId, 160) ||
    undefined;
  const evidenceReason = safeString(p.evidenceReason, 200) || undefined;
  return {
    ...(paramsCandidate ? { params: paramsCandidate } : {}),
    ...(answerEvidence ? { answerEvidence } : {}),
    ...(misconceptionTag ? { misconceptionTag } : {}),
    ...(distractorFamily ? { distractorFamily } : {}),
    ...(classifierRuleId ? { classifierRuleId } : {}),
...
function buildAggregateWrongAnswerCapture(p) {
  const prelim = {
    ...
    answerEvidence: p.answerEvidence,
    misconceptionTag: p.misconceptionTag,
    distractorFamily: p.distractorFamily,
    classifierRuleId: p.classifierRuleId,
    evidenceReason: p.evidenceReason,
  ...
  const misconceptionEvidence = resolveMisconceptionEvidenceAtAggregateRead({
    ...prelim,
    subject: p.subject,
    topic: p.topic,
    userAnswer: p.userAnswer,
  ...
  if (misconceptionEvidence.misconceptionTag) {
    engineFields.misconceptionTag = misconceptionEvidence.misconceptionTag;
    if (!engineFields.distractorFamily) {
      engineFields.distractorFamily = misconceptionEvidence.distractorFamily;
    }
  }
  if (misconceptionEvidence.classifierRuleId && !engineFields.classifierRuleId) {
    engineFields.classifierRuleId = misconceptionEvidence.classifierRuleId;
  }
  ...
  return {
    ...
    ...(misconceptionEvidence.misconceptionTag
      ? {
          misconceptionTag: misconceptionEvidence.misconceptionTag,
          distractorFamily:
            misconceptionEvidence.distractorFamily || engineFields.distractorFamily || legacyEngine.distractorFamily,
          answerEvidence: misconceptionEvidence.answerEvidence,
          classifierRuleId: misconceptionEvidence.classifierRuleId || p.classifierRuleId,
          evidenceReason: misconceptionEvidence.evidenceReason || p.evidenceReason,
        }
      : {}),
    questionEngine: canonicalBundle.enrichedQuestionEngine || p.questionEngine,
    diagnosticMetadata,
...
export function aggregateReportPayloadFromActivityRows(
  ...
  for (const answer of answers) {
    ...
    if (!isCorrect) {
      ...
      if (!excludeCompetitiveMistake) {
        const writeTimeEvidence = extractWriteTimeEvidenceFieldsFromPayload(payload);
  ...
  for (const attempt of allAssignedActivityAttempts) {
    ...
    if (isCorrect) {
      ...
      if (!excludeCompetitiveMistake) {
        const writeTimeEvidence = extractWriteTimeEvidenceFieldsFromPayload(snapshot);
        const capture = buildAggregateWrongAnswerCapture({
          subject,
          topic,
 ...
 * @param {import("@supabase/supabase-js").SupabaseClient} serviceClient
 * @param {{ id: string, full_name?: string|null, grade_level?: string|null, is_active?: boolean }} student
 * @param {Date} fromDate inclusive calendar day (YMD interpreted as Asia/Jerusalem civil day)
 * @param {Date} toDate inclusive calendar day
...
export async function aggregateParentReportPayload(serviceClient, student, fromDate, toDate, options = {}) {
  ...
  const { fromIso, toIsoExclusive } = resolveParentReportRangeUtcBounds(fromDate, toDate);
 ...
 *
 * Architecture: `diagnosticMistakes` / `recentMistakes` are key-scrubbed only and
 * reattached AFTER public taxonomy-value sanitize, so DE2 / pattern-evidence-gate
 * still receive real `questionId` values (including embedded skill tokens).
 * @param {Record<string, unknown>|null|undefined} payload
 */
export function stripInternalReportPayloadFields(payload) {
  ...
  // Park engine evidence arrays — public taxonomy-value sanitize must not null questionId.
  const diagnosticMistakesParked = Array.isArray(out.diagnosticMistakes)
    ? out.diagnosticMistakes.map((row) => stripInternalKeysFromEvidenceRow(row))
    : null;
  const recentMistakesParked = Array.isArray(out.recentMistakes)
    ? out.recentMistakes.map((row) => stripInternalKeysFromEvidenceRow(row))
    : null;
  delete out.diagnosticMistakes;
  delete out.recentMistakes;
  ...
  if (out.subjects && typeof out.subjects === "object") {
    ...
    for (const [key, subj] of Object.entries(out.subjects)) {
      ...
      // Subject.mistakes (if present) are engine evidence — key-scrub only.
      if (Array.isArray(subjClean.mistakes)) {
        subjClean.mistakes = subjClean.mistakes.map((row) => stripInternalKeysFromEvidenceRow(row));
      }
  ...
  const publicBody = /** @type {Record<string, unknown>} */ (
    stripZeroEvidenceFromPublicReportPayload(out)
  );

  // Reattach forensic evidence after public sanitize (DE2 input on client).
  if (diagnosticMistakesParked) {
    publicBody.diagnosticMistakes = diagnosticMistakesParked;
  }
  if (recentMistakesParked) {
    publicBody.recentMistakes = recentMistakesParked;
  }
