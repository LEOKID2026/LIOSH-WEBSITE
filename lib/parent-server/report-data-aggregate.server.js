...
async function fetchParentActivityAttemptsInRange(supabase, studentId, fromIso, toIsoExclusive) {
  ...
  return { rows: data || [], filterField: "answered_at" };
}
...
export function aggregateReportPayloadFromActivityRows(
  ...
  for (const attempt of parentActivityAttempts) {
    ...
    const timeMs = normalizeFiniteNonNegativeNumber(attempt.time_spent_ms, 36_000_000);
    if (timeMs != null) {
      subjectAgg.timeMsSum += timeMs;
      subjectAgg.timeMsCount += 1;
      overallTimeMsSum += timeMs;
    ...
    // Phase 4: classify parent activity attempt using stored snapshot classification or mode
    const attemptClassification = classifyAnswerForAggregation(
      {
        isDiagnosticEligible: snapshot.isDiagnosticEligible,
        evidenceCategory: snapshot.evidenceCategory,
        contextFlags:
