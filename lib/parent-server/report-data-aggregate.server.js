...
export async function aggregateParentReportPayload(serviceClient, student, fromDate, toDate, options = {}) {
  ...
  let privateTeacherActivityAttempts = [];
  let privateTeacherActivitySchemaUnavailable = false;
  if (options.includePrivateTeacherActivities === true) {
    const privateTeacherAttemptsResult = await fetchPrivateTeacherActivityAttemptsInRange(
      serviceClient,
      student.id,
      fromIso,
      toIsoExclusive
    );
    privateTeacherActivityAttempts = privateTeacherAttemptsResult.rows;
    privateTeacherActivitySchemaUnavailable = privateTeacherAttemptsResult.schemaUnavailable === true;
  }
