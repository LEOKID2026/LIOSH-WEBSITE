...
import { deepStripInternalReportKeys } from "./report-payload-public-sanitize.js";
 ...
 */
function classifyAnswerForAggregation(classificationSource, resolvedMode) {
...
export function aggregateReportPayloadFromActivityRows(
  ...
  for (const session of sessions) {
    ...
    if (
      session.subject === "moledet_geography" &&
      !isMoledetGeographyGradeAllowed(registeredGradeKey)
    ) {
      continue;
    }
  ...
  for (const answer of answers) {
    ...
    const answerClassification = classifyAnswerForAggregation(
      {
    ...
    if (isCorrect) {
      ...
      if (!excludeCompetitiveMistake && recentMistakes.length < RECENT_MISTAKES_LIMIT) {
        const canonicalMeta = resolveMistakeCanonicalMetaIfEnabled(
          payload,
          answerClassification,
          subject,
          topic
        ...
        recentMistakes.push({
          ...
          ...extractRecentMistakeEngineFields(payload.questionEngine),
          ...(canonicalMeta ? { _canonicalMeta: canonicalMeta } : {}),
        });
      }
    }
  ...
  for (const attempt of parentActivityAttempts) {
    const activityMeta =
    ...
    const resolvedLevel = pickEnumString(activityMeta.difficulty_level, ALLOWED_LEVEL_VALUES);
    subjectAgg.modeCounts[resolvedMode] = (subjectAgg.modeCounts[resolvedMode] || 0) + 1;
    subjectAgg.levelCounts[resolvedLevel] = (subjectAgg.levelCounts[resolvedLevel] || 0) + 1;
    overallModeCounts[resolvedMode] = (overallModeCounts[resolvedMode] || 0) + 1;
    overallLevelCounts[resolvedLevel] = (overallLevelCounts[resolvedLevel] || 0) + 1;
    ...
    // Phase 4 + Phase 2: parent attempts always re-derived from assigned_parent source
    ...
    // Phase C: evidence provenance — this attempt came from a parent-assigned activity.
    bumpEvidenceSourceCount(topicAgg.evidenceSourceCounts, EVIDENCE_SOURCE.PARENT_ASSIGNED);
    bumpEvidenceSourceCount(gradeSlice.evidenceSourceCounts, EVIDENCE_SOURCE.PARENT_ASSIGNED);
    ...
    if (isCorrect) {
      ...
      if (!excludeCompetitiveMistake && recentMistakes.length < RECENT_MISTAKES_LIMIT) {
        ...
        recentMistakes.push({
          subject,
          topic,
          questionId: safeString(`par_${attempt.activity_id}_q${attempt.question_index}`, 180),
          ...
          gradeDelta: mistakeEvidence.gradeDelta,
          answeredAt: answerIso,
          ...extractRecentMistakeEngineFields(snapshot.questionEngine),
          ...(canonicalMeta ? { _canonicalMeta: canonicalMeta } : {}),
        });
      }
    }
    ...
    if (dayKey) {
      ...
      if (timeMs != null && timeMs > 0) {
        dailyBySubject[dayKey][subject].durationSeconds += Math.max(0, Math.floor(timeMs / 1000));
      }
    }
  }
 ...
 * @param {{ includeParentActivities?: boolean }} [options]
...
export async function aggregateParentReportPayload(serviceClient, student, fromDate, toDate, options = {}) {
  ...
  let parentActivityAttempts = [];
  let parentActivitySchemaUnavailable = false;
  if (options.includeParentActivities === true) {
    const parentAttemptsResult = await fetchParentActivityAttemptsInRange(
      serviceClient,
      student.id,
      fromIso,
      toIsoExclusive
    );
    parentActivityAttempts = parentAttemptsResult.rows;
    parentActivitySchemaUnavailable = parentAttemptsResult.schemaUnavailable === true;
  }

  const bookVisitsResult = await fetchBookPageVisitsInRange(
    serviceClient,
    student.id,
    fromIso,
    toIsoExclusive
  );
  const bookSessionsResult = await fetchBookReadingSessionsInRange(
    serviceClient,
    student.id,
    fromIso,
    toIsoExclusive
  ...
  if (parentActivitySchemaUnavailable) {
    payload.meta = {
      ...(payload.meta && typeof payload.meta === "object" ? payload.meta : {}),
      _dataHealth: {
        parentActivityAttemptsUnavailable: true,
      },
    };
  }

  return mergeLearningActivityBookData(
    payload,
 ...
 *  - `_dailyBySubject` — internal aggregation helper
...
export function stripInternalReportPayloadFields(payload) {
  ...
  return /** @type {typeof out} */ (deepStripInternalReportKeys(out));
}
