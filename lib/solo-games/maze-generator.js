...
export function openExtraPassages(maze, rng, count) {
  ...
  const candidates = shuffleWithRng(
    wallBreakCandidates(copy).sort((a, b) => b.score - a.score),
    rng
  );
  let opened = 0;
...
/** Keep opening loop walls until iron rules pass or budget exhausted. */
export function reinforceMazePassages(maze, rng, rules, validateCtx, maxOpens = 24) {
  let copy = maze.map((row) => [...row]);
  for (let i = 0; i < maxOpens; i += 1) {
    if (validateIronRules(copy, validateCtx, rules)) return copy;
    const candidates = shuffleWithRng(
      wallBreakCandidates(copy).filter((w) => copy[w.r][w.c] === 1),
      rng
    );
    if (!candidates.length) break;
    copy[candidates[0].r][candidates[0].c] = 0;
  }
  return copy;
}
...
export function buildMazeLevel(rows, cols, starCount, difficulty, withDiamond, settings = {}) {
  ...
  for (let attempt = 0; attempt < 160; attempt += 1) {
    ...
    maze = reinforceMazePassages(maze, rng, rules, validateCtx, rules.extraPassages + 8);
  ...
  for (let extra = 0; extra < 40; extra += 1) {
    ...
    const rng = mulberry32(seed);
    let maze = generateMaze(rows, cols, seed);
    maze = openExtraPassages(maze, rng, rules.extraPassages + 8 + extra);
    ...
    maze = reinforceMazePassages(maze, rng, rules, validateCtx, 32);
