...
const ARCADE_GAME_KEYS = Object.freeze(["catcher", "flyer", "leo-jump", "balloons", "target-tap"]);
const PUZZLE_STYLE_GAME_KEYS = Object.freeze(["puzzle", "maze", "picture-puzzle"]);
...
export function calculateSoloGameCoins(gameKey, difficulty, metrics, rules) {
  ...
  if (isArcadeGame(gameKey)) {
    ...
    if (didWin) {
      ...
      coins = 0;
      breakdown.push("לא הושג ניצחון — אין מטבעות");
    }
    ...
    if (didWin) {
      ...
      if (coins <= 0) breakdown.push("אין מטבעות — ניקוד נמוך מדי");
    } else {
      coins = 0;
      breakdown.push("לא הושג ניצחון — אין מטבעות");
    }
...
export async function finalizeSoloGameSession(supabase, params) {
  ...
  const diamondPayout = calculateSoloGameDiamonds(rules?.diamondRules, metrics);
