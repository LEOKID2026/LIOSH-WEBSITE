...
function resolvePracticeUnits(request) {
  ...
  if (request.writingCategory === "personal_text") {
    if (request.customTextKind === "word_list") {
      return request.customText.split(/[\s,]+/).filter(Boolean);
    }
    return request.customText.split("");
  }
  return [];
}
...
export function buildWritingPayloadFromRequest(request, opts = {}) {
  ...
  const pages = buildPages(request);
  ...
  return {
    ...
    writingCategory: request.writingCategory,
    language: languageForRequest(request),
    pageDirection: pageDirectionForRequest(request),
    scriptStyle: request.scriptStyle,
    tracingMode: request.tracingMode,
    meta: buildWritingMeta(request, opts.titleHe),
    pages,
    requiresAnswerKey,
    answers,
