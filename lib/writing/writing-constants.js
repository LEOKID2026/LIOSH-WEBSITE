...
export const PREWRITING_PATH_LABELS_HE = {
  horizontal: "קווים אופקיים",
  vertical: "קווים אנכיים",
  slants: "קווים אלכסוניים",
  bridges: "גשרים",
  waves: "גלים",
  peaks: "פסגות",
  circles: "עיגולים",
 ...
 * @returns {string}
 */
export function prewritingPathLabelHe(pathId) {
  const key = String(pathId || "").trim();
  return PREWRITING_PATH_LABELS_HE[key] || key;
}

/**
 * @param {Record<string, { titleHe?: string }>} packs
 * @param {string} packId
 * @returns {string}
 */
export function wordPackLabelHe(packs, packId) {
...
export function isTraceableGlyphChar(character, request) {
  ...
  if (isHebrewLetter(ch)) return true;
