...
import {
  ...
  TEACHER_REPORT_PATH,
  toStudentLearningPath,
} from "./pwa/pwa-scope-routes.js";
...
export function isImmersiveGameLayoutPath(pathname) {
  const path = pathname || "";
  const isLearningDev =
    path.startsWith("/learning/dev") || path.startsWith("/student/learning/dev");
  return (
    !isLearningDev &&
    (path.includes("/learning/") ||
      path.includes("/student/learning/") ||
      path.includes("/offline/") ||
      path.includes("/student/offline/"))
  );
}
...
export function shouldShowLayoutThemePicker(pathname) {
  ...
  if (
    ...
    path === PARENT_REPORT_PATH ||
    path === PARENT_REPORT_DETAILED_PATH ||
    path === TEACHER_REPORT_PATH ||
    path === TEACHER_REPORT_DETAILED_PATH
  ) {
    return false;
  }
...
const ENGINE_REVIEW_LINK = {
  href: toStudentLearningPath("/learning/dev/engine-review"),
  label: "סקירת מנוע",
};
...
const STUDENT_NAV = [
  { href: "/", label: "בית" },
  { href: "/student/home", label: "עולם הילדים" },
  { href: STUDENT_GAMES_HUB, label: "משחקים" },
  { href: STUDENT_LEARNING_HUB, label: "לימודים" },
  { href: "/about", label: "אודות" },
  { href: STUDENT_GALLERY, label: "גלריה" },
  { href: "/contact", label: "צור קשר" },
  { href: "/help", label: "מרכז העזרה" },
];
...
export function resolveNavArea(pathname, options = {}) {
  ...
  if (
    path.startsWith("/student") ||
    path === STUDENT_GAMES_HUB ||
    path === "/game" ||
    path === STUDENT_LEARNING_HUB ||
    path === STUDENT_GALLERY ||
    path.startsWith("/student/offline") ||
