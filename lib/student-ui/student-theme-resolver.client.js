...
import { STUDENT_ACTIVITY_LAYOUT } from "../classroom-activities/student-activity-layout.client.js";
import { STUDENT_ACTIVITY_LAYOUT_BRIGHT } from "./student-bright-activity-layout.client.js";
 ...
 * @param {'bright' | 'classic'} theme
 */
export function resolveStudentActivityUi(theme) {
  const isBright = isStudentThemeBright(theme);
  const MB = isBright ? MATH_BRIGHT : MATH_CLASSIC;
  return {
    theme,
    isBright,
    L: isBright ? STUDENT_ACTIVITY_LAYOUT_BRIGHT : STUDENT_ACTIVITY_LAYOUT,
    MB,
  };
}
