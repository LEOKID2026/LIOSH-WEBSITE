...
/** Approximate legal footer block height in Layout (scratchpad dock anchor). */
export const STUDENT_LAYOUT_FOOTER_H_PX = 52;

const WRAP_BASE =
  "w-full shrink-0 flex justify-center px-3 py-1 pointer-events-none bg-transparent";
...
export function getStudentAdSlotClasses(variant, palette = "classic") {
  ...
  if (variant === "dvh") {
    return {
      wrap: `${WRAP_BASE} pb-[max(0.25rem,env(safe-area-inset-bottom,0px))]`,
      slot: t.slot,
      label: t.label,
    };
  }
