...
export function pickUncertaintyReasonScript(dl, intent, ix) {
  ...
  if (cannot) {
    const lines = [
      "לפי המסגרת של הדוח עדיין לא ניתן לסגור מסקנה יציבה — ממשיכים בתרגול ובדיקה חוזרת.",
      "הדוח עדיין לא מאפשר לסגור מסקנה חד משמעית; נכון לעכשיו ממשיכים בתרגול ממוקד ובודקים שוב בהמשך.",
      "כשלא ניתן לסגור עדיין, עדיף לפרק לצעדים קטנים של איסוף מידע מאשר לנחש.",
      "לפי הגבולות שבדוח: אין עדיין «נקודת סיום» למסקנה; ממשיכים בזהירות ובמדידה חוזרת.",
    ];
    return lines[ix % lines.length];
  }
  if (low) {
    const lines = [
      "המסקנה כאן עדיין לא מבוססת מספיק בטווח הזה, ולכן נשמרת זהירות בניסוח.",
