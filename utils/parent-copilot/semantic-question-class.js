...
export function detectAggregateQuestionClass(utterance) {
  ...
  if (
    /(לעומת|מול|בהשוואה|יותר\s+מ|פחות\s+מ).*(מקצוע|חשבון|עברית|גאומטריה|גיאומטריה|הנדסה|אנגלית|מדעים|מולדת)/.test(t) ||
    /(חשבון|עברית|אנגלית|גאומטריה|גיאומטריה|הנדסה|מדעים)\s+מול\s+(חשבון|עברית|אנגלית|גאומטריה|גיאומטריה|הנדסה|מדעים)/.test(t) ||
