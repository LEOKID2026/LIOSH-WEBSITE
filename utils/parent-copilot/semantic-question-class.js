...
export function detectAggregateQuestionClass(utterance) {
  ...
  if (
    /(לעומת|מול|בהשוואה|יותר\s+מ|פחות\s+מ).*(מקצוע|חשבון|עברית|הנדסה|גאומטריה|אנגלית|מדעים|מולדת)/.test(t) ||
    /(חשבון|עברית|אנגלית|הנדסה|גאומטריה|מדעים)\s+מול\s+(חשבון|עברית|אנגלית|הנדסה|גאומטריה|מדעים)/.test(t) ||
