...
const HEBREW_TIME_TRACKING_KEY = "mleo_hebrew_time_tracking";
...
export function trackHebrewTopicTime(topic, grade, level, duration, meta = {}) {
  ...
  if (isTrackingDebugEnabled()) {
    trackingDebugRecordTrack(topic, duration, meta?.mode, "trackHebrewTopicTime");
  }
  ...
  try {
    const saved = JSON.parse(
      localStorage.getItem(HEBREW_TIME_TRACKING_KEY) || "{}"
    );
    ...
    localStorage.setItem(HEBREW_TIME_TRACKING_KEY, JSON.stringify(saved));
  } catch (error) {
    console.error("Error tracking Hebrew topic time:", error);
  }
}

export function getHebrewTimeByCustomPeriod(startDate, endDate) {
  if (typeof window === "undefined") return {};
  ...
  try {
    ...
    console.error("Error getting Hebrew time by period:", error);
    return {};
  }
}
