  ...
  switch (selectedTopic) {
      ...
      switch (shape) {
        case "square": {
          ...
          if (useStory) {
            ...
            if (levelKey === "easy") {
              question = [
                `ריבוע על רשת עם צלע ${side}. כמה משבצות יש בשטחו?`,
                `ריבוע בצלע ${side} על רשת. ספרו כמה משבצות.`,
                `כמה משבצות ממלאות ריבוע בצלע ${side}?`,
                `ריבוע עם ${side} משבצות בכל צלע. כמה משבצות בסך הכל?`,
                `רשת עם ריבוע בצלע ${side}. כמה תאים?`,
                `ריבוע על לוח ריבועים: צלע ${side}. כמה ריבועים קטנים?`,
                `ספרו משבצות בריבוע עם צלע ${side}.`,
                `ריבוע ${side}×${side} על רשת. כמה משבצות?`,
                `כמה יחידות שטח בריבוע עם צלע ${side}?`,
                `ריבוע על רשת — צלע ${side}. כמה תאים בשטחו?`,
              ][earlyW];
        ...
        case "rectangle": {
          ...
          if (useStory) {
            ...
            if (levelKey === "easy") {
              question = [
                `מלבן על רשת: ${length} שורות ו-${width} עמודות. כמה משבצות?`,
                `מלבן על לוח ריבועים: אורך ${length}, רוחב ${width}. כמה תאים?`,
                `מלבן ברשת: ${length} עמודות, ${width} שורות. ספרו משבצות.`,
                `כמה משבצות ממלאות מלבן של ${length}×${width}?`,
                `רשת ${length}×${width}. כמה משבצות?`,
                `מלבן עם ${length} עמודות ו-${width} שורות על רשת. כמה תאים?`,
                `ספרו: מלבן ${length} על ${width} על לוח. כמה משבצות?`,
                `מלבן ${length}×${width} על רשת ריבועים. מה שטחו במשבצות?`,
                `כמה תאים יש במלבן ${length}×${width}?`,
                `מלבן בגודל ${length} על ${width}. כמה משבצות בשטחו?`,
              ][earlyRW];
            } else if (levelKey === "medium") {
              question = [
                ...
                `מלבן ${length} על ${width}. מה שטחו?`,
                `שטח = אורך × רוחב. ${length}×${width} — כמה?`,
                `מלבן: אורך ${length}, רוחב ${width}. מה שטח הפנים?`,
                `חשבו שטח מלבן: ${length} על ${width}.`,
              ...
              question = [
                ...
                `שטח מלבן ${length} על ${width}?`,
                `מלבן: ${length}×${width}. כמה שטח?`,
                `מה שטח מלבן ${length} ו-${width}?`,
                `מלבן ${length}×${width} — מה שטח הפנים?`,
                `שטח = ? מלבן ${length}×${width}.`,
                `מה שטח המלבן? ${length} על ${width}.`,
            ...
            if (levelKey === "easy") {
              ...
              question = [
                ...
                `מלבן ${length} על ${width} יחידות. מה השטח?`,
    ...
    case "parallel_perpendicular": {
      ...
      if (formulaBand === "mid") {
        question =
          levelKey === "easy"
            ? `${gradeTag ? `${gradeTag}: ` : ""}המילה "${selectedType}" מתארת זוג קווים. מה מתאר אותה?`
            : levelKey === "medium"
              ? `${gradeTag ? `${gradeTag} — ` : ""}סיווג ישרים: "${selectedType}" — מה היחס הנכון?`
              : `${gradeTag ? `${gradeTag} | ` : ""}הגדרה "${selectedType}" — איזה יחס בין שני ישרים במישור?`;
      } else {
        question =
          levelKey === "easy"
            ? `זיהוי מהיר: "${selectedType}" — מה היחס בין הישרים?`
            : levelKey === "medium"
              ? `בחירה מדויקת: "${selectedType}" מתאר מצב של שני ישרים. בחרו.`
              : `ניסוח פורמלי: "${selectedType}" כיחס גיאומטרי בין ישרים — מה הוא?`;
      }
      break;
    }
    ...
    case "triangles": {
      ...
      if (formulaBand === "mid") {
        question =
          levelKey === "easy"
            ? [
                `לפי השם "${selectedType}" — מהו סוג המשולש?`,
                `זיהוי משולש: "${selectedType}" = איזה סוג?`,
                `בחרו את הסוג הנכון: "${selectedType}" מתאר איזה משולש?`,
                `מהו סוג המשולש "${selectedType}"?`,
                `חברו בין השם "${selectedType}" לסוג המשולש.`,
                `סיווג: "${selectedType}" מתאים לאיזה סוג משולש?`,
                `זיהוי שם: "${selectedType}" = ?`,
                `מה הסוג של משולש שנקרא "${selectedType}"?`,
              ][triW]
            : levelKey === "medium"
              ? [
                  `סיווג משולש לפי תיאור: "${selectedType}".`,
                  `הגדרת סוג משולש: "${selectedType}" מתאר משולש עם תכונות של —`,
                  `חקר תכונות: למשולש "${selectedType}" יש צלעות —`,
                  `זיהוי לפי הגדרה: "${selectedType}" = משולש עם תכונות ייחודיות של —`,
                  `התאמת תיאור: "${selectedType}" מתאים לאיזה סוג משולש?`,
                  `ניתוח תכונות: משולש "${selectedType}" שייך לקטגוריה —`,
                  `זיהוי סוג: השם "${selectedType}" מצביע על איזה סוג משולש?`,
                  `הגדרה מתמטית: "${selectedType}" הוא משולש המוגדר כ —?`,
                ][triW]
              : [
                  `אתגר סיווג: "${selectedType}" כשם סוג משולש.`,
                  `ניתוח מעמיק: "${selectedType}" — מהי הגדרתו המדויקת של משולש זה?`,
                  `הוכחת זיהוי: מדוע "${selectedType}" שייך לקטגוריה מסוימת?`,
                  `אתגר הגדרות: "${selectedType}" מתאר משולש המקיים תנאים של —`,
                  `ניתוח תכונות מתמטיות: השם "${selectedType}" מרמז על מאפייני הצלעות —`,
                  `סיווג מתקדם: "${selectedType}" = משולש שכל צלעותיו —`,
                  `הבנת מונחים: "${selectedType}" מתייחס למשולש עם תכונת —`,
                  `אתגר הגדרה: מהו "${selectedType}" במונחי סיווג משולשים?`,
                ][triW];
      } else {
        ...
        question =
          levelKey === "easy"
            ? [
                `בחרו סוג למשולש בשם "${selectedType}":`,
                `זיהוי: "${selectedType}" מתאים לסוג משולש —`,
                `מהו הסוג: "${selectedType}" = איזה משולש?`,
                `התאמה: השם "${selectedType}" מתאר איזה משולש?`,
                `סיווג פשוט: "${selectedType}" שייך לאיזה קטגוריה?`,
                `זיהוי בסיסי: משולש "${selectedType}" הוא מסוג?`,
                `בחירת סוג: "${selectedType}" מתאים לאיזה סוג משולש?`,
                `הגדרה: "${selectedType}" מתאר משולש עם מאפייני —`,
                `זיהוי לפי שם: "${selectedType}" הוא משולש —`,
                `סיווג: "${selectedType}" = איזה סוג משולש?`,
                `תיאור: "${selectedType}" מתאר משולש —`,
                `מה סוג המשולש "${selectedType}"?`,
                `חברו שם לסוג: "${selectedType}" = משולש —`,
                `זיהוי קל: "${selectedType}" הוא סוג משולש —`,
                `סימון סוג: "${selectedType}" — מהו סוג המשולש?`,
                `בדיקה: "${selectedType}" מתאים לסוג —`,
              ][triLateW]
            : levelKey === "medium"
              ? [
                  `התאמת מונח: "${selectedType}" = איזה סוג משולש?`,
                  `מונח מתמטי: "${selectedType}" מגדיר משולש עם תכונות של —`,
                  `הגדרה וסיווג: "${selectedType}" מתייחס למשולש המקיים —`,
                  `זיהוי מונח: "${selectedType}" = משולש בעל צלעות —`,
                  `התאמת הגדרה: "${selectedType}" מתאר משולש שצלעותיו —`,
                  `מונח גיאומטרי: "${selectedType}" — מהו סוג המשולש?`,
                  `סיווג לפי שם: "${selectedType}" מצביע על איזה סוג משולש?`,
                  `הבנת מונח: "${selectedType}" מתייחס למשולש עם מאפייני צלעות —`,
                  `ניתוח: "${selectedType}" מתאר משולש עם צלעות —`,
                  `זיהוי סוג: "${selectedType}" = סוג משולש —`,
                  `הגדרה: "${selectedType}" מתאים למשולש —`,
                  `סיווג משולש: "${selectedType}" הוא —`,
                  `מאפייני צלעות: "${selectedType}" = משולש —`,
                  `חקר משולשים: "${selectedType}" שייך לסוג —`,
                  `בדיקת הבנה: "${selectedType}" סוג משולש —`,
                  `מיון משולשים: "${selectedType}" = קטגוריה —`,
                ][triLateW]
              : [
                  `ניסוח מדויק: "${selectedType}" — מהו סוג המשולש?`,
                  `הגדרה מדויקת: "${selectedType}" — מהי הגדרת סוג המשולש?`,
                  `ניתוח מונחי: השם "${selectedType}" מגדיר איזה סוג משולש?`,
                  `התאמה מדויקת: "${selectedType}" מתאר משולש עם תכונות ייחודיות של —`,
                  `ביטוי מתמטי: "${selectedType}" הוא מונח המתייחס למשולש שבו —`,
                  `זיהוי מדויק: "${selectedType}" = משולש המקיים תנאים ספציפיים של —`,
                  `הגדרה מתקדמת: "${selectedType}" מוגדר כמשולש שכל צלעותיו —`,
                  `ניסוח מתמטי: "${selectedType}" מתייחס למשולש בעל תכונות גיאומטריות של —`,
                  `אתגר: "${selectedType}" — מהו סוג המשולש?`,
                  `חקר מעמיק: "${selectedType}" מתאר משולש —`,
                  `ניתוח מתקדם: "${selectedType}" = סוג משולש —`,
                  `הוכחה: "${selectedType}" שייך לקטגוריה —`,
                  `אתגר הגדרות: "${selectedType}" — מהו סוג המשולש?`,
                  `חקר תכונות: "${selectedType}" מאפייני צלעות —`,
                  `ניתוח מדויק: "${selectedType}" קטגוריית משולש —`,
                  `הסבר מילולי: "${selectedType}" סוג משולש —`,
                ][triLateW];
      }
      break;
    }
    ...
    case "rotation": {
      ...
      if (levelKey === "easy") {
        ...
        question = [
          `${angleLabel} במישור — כמה מעלות בדיוק?`,
    ...
    case "tiling": {
      ...
      if (tilingSubtype === 0) {
        ...
        question = [
          ...
          `ריצוף במישור: מה הזווית הפנימית ב${selectedShape}?`,
        ...
        question = [
          ...
          `כמה אריחים בצלע ${tileSide} נדרשים לכיסוי רצפה ${floorL} על ${floorW}?`,
          ...
          `חדר ${floorL} על ${floorW} אמות. אריחי רצפה בצלע ${tileSide}. כמה אריחים?`,
          ...
          `חדר בגודל ${floorL} על ${floorW}. אריח הוא ${tileSide}×${tileSide}. כמה אריחים דרושים?`,
