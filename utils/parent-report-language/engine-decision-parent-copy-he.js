...
export function buildExplainIdentifiedLineHe(engineCopy, label) {
  ...
  switch (engineCopy.engineDecision) {
    case "mastery_stable":
      return `מה זוהה: שליטה טובה ב«${t}».`;
    case "partial_stable":
      return `מה זוהה: שליטה חלקית ב«${t}».`;
    case "clear_topic_gap":
      return `מה זוהה: פער ברור בתרגול ב«${t}».`;
    case "topic_needs_strengthening":
      return `מה זוהה: נקודת חיזוק ב«${t}».`;
    ...
    case "speed_pressure_pattern":
      return `מה זוהה: חלק מהטעויות קשורות למהירות ב«${t}».`;
    default:
      return `מה זוהה: מיקוד ב«${t}».`;
  }
}
