...
function buildDiagnosticBodyByDecision(p) {
  ...
  if (decision === "partial_stable") {
    let body =
      `ב${subj} בנושא ${topic} נראית הבנה חלקית, אך הביצוע עדיין אינו יציב. ` +
  ...
  if (decision === "topic_needs_strengthening") {
    let body =
      `ב${subj} בנושא ${topic} יש חלק שדורש חיזוק. ` +
      `הילד פתר ${q} שאלות בדיוק של ${acc}%. ` +
      "הביצוע בנושא עדיין אינו יציב, ולכן כדאי להמשיך בתרגול ממוקד.";
  ...
  if (decision === "clear_topic_gap") {
    let body =
      `ב${subj} בנושא ${topic} נראה קושי שדורש חיזוק. ` +
  ...
  return (
    `ב${subj} בנושא ${topic} כדאי להמשיך לעקוב אחר הביצוע בתרגולים הבאים. ` +
    `הילד פתר ${q} שאלות בדיוק של ${acc}%.`
  );
}
...
export function buildExplainIdentifiedLineHe(engineCopy, label) {
  ...
  switch (engineCopy.engineDecision) {
    ...
    case "clear_topic_gap":
      return "מה כדאי לחזק: כדאי להמשיך לתרגל את הנושא.";
