...
function generateUniquePassageItems(count, topic, level, patternFamily, subtype, subtopicId) {
  ...
  const templates = [
    (name, obj, place, g, when) => ({
      ...
      wrong: [
        ...
        `החליף${G5_FEMALE_NAMES.has(name) ? "ה" : ""} את ${obj} בלי לשאול`,
      ],
    }),
    (name, obj, place, g, when) => ({
      passage: `לפני השיעור ${name} ${g.read} ב${obj} ו${g.wrote} רשימת מילים חשובות. ${when} ${g.explained} למורה את העבודה.`,
      prompt: `מה עש${G5_FEMALE_NAMES.has(name) ? "תה" : "ה"} ${name} לפני השיעור?`,
      answer: `קרא${G5_FEMALE_NAMES.has(name) ? "ה" : ""} ב${obj} וכתב${G5_FEMALE_NAMES.has(name) ? "ה" : ""} רשימת מילים`,
    ...
    (name, obj, place, g, when) => ({
      passage: `${name} ${G5_FEMALE_NAMES.has(name) ? "הכינה" : "הכין"} מצגת על ${obj} ב${place}. ${when} ${G5_FEMALE_NAMES.has(name) ? "הציגה" : "הציג"} בפני הכיתה בביטחון.`,
  ...
  for (let i = 0; i < count; i += 1) {
    const name = G5_NAMES[i % G5_NAMES.length];
    const place = G5_PLACES[(i * 2) % G5_PLACES.length];
    const obj = G5_OBJECTS[(i * 3) % G5_OBJECTS.length];
    const g = g5GenderForms(name);
    const when = G5_DAY_WORDS[i % G5_DAY_WORDS.length];
