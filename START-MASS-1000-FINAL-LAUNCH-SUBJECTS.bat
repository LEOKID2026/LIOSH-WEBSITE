...
set "LOG=reports\mass-simulation\_logs\mass-1000-final-launch-subjects-run.log"

if not exist reports\mass-simulation\_logs mkdir reports\mass-simulation\_logs

echo.
echo ============================================================
echo   MASS 1000 — FINAL LAUNCH SUBJECTS (new runId)
echo   subjects: %SUBJECTS%
echo   history key: history (catalog/registry confirmed)
echo   NOT touching closed PASS runs:
echo     mass-2026-06-28T06-22-20
echo     mass-2026-06-29T03-58-49
echo   Preflight runs automatically before seed.
...
echo [%date% %time%] final-launch-subjects 1000 start subjects=%SUBJECTS%>> "%LOG%"
