...
--
-- Rollback (manual):
--   delete from public.site_game_catalog where game_key = 'leo-word-detective';
--   delete from public.reward_economy_educational_game_rules where game_key = 'leo-word-detective';
...
insert into public.site_game_catalog (
  ...
  coalesce(
    (select is_active from public.reward_economy_educational_game_rules where game_key = 'leo-word-detective'),
    true
  ),
  75,
  '🕵️',
